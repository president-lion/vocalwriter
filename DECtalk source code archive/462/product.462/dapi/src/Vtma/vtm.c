...
/*	 14.  8-OCT-96 EAB Fixed three problems 1. initialize lacking the   */
/*							initialization of filter param one_minus_decay	 */
/*							2.Intialiaze variables when a spdef is loaded 	 */
/*							otherwise filters can overload. 3. One packet from*/
/* 						last frame stuck so set vocal tract gains to 	*/
/*							zero for the first frame following a speaker def */
...
#include "port.h"
#include "phinst.h"     /* MVP : Now phinst.h includes phdefs.h */
...
void read_speaker_definition(LPTTS_HANDLE_T phTTS) 
{
  ...
  /********************************************************************/
	...
	pVtm_t->one_minus_decay = 0;    /* Second sample of the tilt filter.
								10/96 eab this filter parameter needs to be zeroed
								also*/
  ...
  */
  ...
  phTTS->pKernelShareData->uiCurrentSpeaker = ((SP_CHIP *)spdeftochip)->speaker;
