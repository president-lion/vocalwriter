...
int ls_suff_suffix_find(LPTTS_HANDLE_T phTTS,unsigned char      __far *str_end,short which_dic)
{
...
#ifdef LSSUFFDEBUG
		FNX_PRINTF("ls_suff_suffix_find; comp_str = %s str_end = %s\n",pLts_t->comp_str,str_end);
#endif
	...
	while(si != 0xffff)
	{
					...
					if(*sp++ == SF_REPLACE_WITH)
					{
						...
						*np++ = 0;
						...
						if(*sp == SF_RECURSE)
						{
							sp++;
#ifdef LSSUFFDEBUG
								FNX_PRINTF("suffix_find recursion bp = %s np = %s save_str = %s\n",bp,np,save_str);
#endif
							stat = ls_suff_suffix_find(phTTS,np-1,which_dic);
#ifdef LSSUFFDEBUG
								FNX_PRINTF("out of recursion\n");
#endif
						}
						else
						{
							...
							if(pKsd_t->udic_entries[phTTS->cur_lts_lang] != 0)
#else
		...
		si = stp->next;
#ifdef LSSUFFDEBUG
			FNX_PRINTF("suffix_find new si = %04X\n",si); 
#endif
	}/*     while(si != 0xffff)*/
	return(MISS);
}
		...
		if(pLts_t->fc_struct[j])
		{
			...
			for(i=0;i<32;i++)
			{
				if(pLts_t->fc_struct[j] & fc_mask)
					FNX_PRINTF("%s fc ",form_class_strings[i]);
				fc_mask = fc_mask*2;
			}
		}
		else
...
#ifndef LDS_BUILD
#ifdef NEW_MULTI_LANG_ARCH
		phTTS->ph_funcs.TextToSpeechErrorHandler( phTTS,
			      ERROR_WRITING_FILE,
			      0L );
#else
...
#ifndef MSDOS
			if (pKsd_t->dbglog)	/*mfg 04/28/98 added debug support*/
				FNX_FPRINTF((FILE *)pKsd_t->dbglog," %s fc ",form_class_strings[i] );
#endif

			if (pKsd_t->logflag & LOG_FORM_TYPES)
			{
				if ( FNX_FPRINTF( phTTS->pLogFile, "%s fc ",form_class_strings[i] ) < 0 )
				{
#ifndef LDS_BUILD
#ifdef NEW_MULTI_LANG_ARCH
					phTTS->ph_funcs.TextToSpeechErrorHandler( phTTS,
