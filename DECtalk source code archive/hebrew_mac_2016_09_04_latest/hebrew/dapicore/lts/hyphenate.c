			...
			int *separationdigits)
{
  ...
  // for (j=0;j<ndigits; j++) FNX_PRINTF("  (%i %i)",pos[j],digits[j]);
  // FNX_PRINTF("\n");
		 ...
		 int *affectedtokens)
{
  ...
  for (ic=0; ic<le; ic++)
	...
	{
	  letok = FNX_STRLEN(TOK[it].str);  // token length
	  if (letok<=FNX_STRLEN(&word[ic]))
	    if (FNX_STRNCMP(&word[ic],TOK[it].str,letok)==0)
	      {
...
GERLETTER *geminyrepairs(GERLETTER *pin, GERLETTER *pout)
{
  ...
  po = FNX_STRCPY(pout, pin);     // copy to out so it can be changed.

repairagain:
  p = (GERLETTER *) FNX_STRSTR(pout, "-ck");
  if (p) 
    {
      p[0] = 'k';
      p[1] = '-';
...
puckels:  
 ...
 tzte:
  p = (GERLETTER *) FNX_STRSTR(pout, "-tz");
  if (p) 
    {
      p[0] = 't';
      p[1] = 'z';
 ...
 nge: 
  p = (GERLETTER *) FNX_STRSTR(pout, "n-g");
  if (p) 
    {
      p[0] = 'n';
      p[1] = 'g';
