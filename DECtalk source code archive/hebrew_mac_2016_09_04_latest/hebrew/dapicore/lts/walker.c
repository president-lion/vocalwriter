     ...
     /* referes to structures defined in ltrie.c */
{
  ...
  return (GERLETTER *)(Mdict->bigbuffer + Mdict->ptarray[index-1]);  // has to be zero terminated, and is.
}
...
void init_dkstate(MinPathState *dj, const int nnodes)
{
  ...
  if (nnodes > MAXPATHNODES) 
    {
#ifndef ARM7_NOSWI
      FNX_PRINTF("Too many nodes in initializing graph structure\n");
#endif
      return;
    }
...
void path_addedge(MinPathState *dj, const SHORT from, const SHORT to, const SHORT w, const EdgeInfo info)
{
  ...
  if(n<MAXEDGES) dj->nedges += 1;
    ...
    FNX_PRINTF("Too many edges in graph\n");
#endif
}

#ifdef WIN32
int __cdecl edge_compare(const void *a, const void *b)
#else
...
void print_edges(const MinPathState *dj)
{
  ...
  for (i=0; i<dj->nedges; i++)
    {
#ifndef ARM7_NOSWI
      FNX_PRINTF(" %3i:   %3i -> %3i w= %3i info: %i \n",i,
	     dj->edges[i].source,
	     dj->edges[i].destin,
	     dj->edges[i].weight,
	     dj->edges[i].info);
...
#ifndef ARM7_NOSWI
  int i;
  FNX_PRINTF("dist "); for (i=0;i<dj->nnodes;i++) FNX_PRINTF("%3i ",dj->dist[i]); FNX_PRINTF("\n"); 
  FNX_PRINTF("pred "); for (i=0;i<dj->nnodes;i++) FNX_PRINTF("%3i ",dj->pred[i]); FNX_PRINTF("\n"); 
  FNX_PRINTF("in S "); for (i=0;i<dj->nnodes;i++) FNX_PRINTF("%3i ",dj->inS[i]);  FNX_PRINTF("\n"); 
  if (how==0) return;
  FNX_PRINTF("adj  ");  for (i=0;i<dj->nnodes;i++) FNX_PRINTF("%3i ",dj->adj[i]); FNX_PRINTF("\n"); 
  FNX_PRINTF("nadj ");  for (i=0;i<dj->nnodes;i++) FNX_PRINTF("%3i ",dj->nadj[i]); FNX_PRINTF("\n"); 
#endif
}
...
GERLETTER vowel_lts_rule(GERLETTER *word, SHORT from, SHORT wordlen, SHORT *explain)
{
  ...
  *explain=0;
  ...
  if (cr == 'e' && is_a_consonantgrapheme(crr))
      ...
      if (cin=='a' || cin =='o' || cin=='u')
	{
	  ...
	  *explain += 1;
	  switch (cin)
	    {
	    case 'a': cin=0xe4; break;	//0xe4='ä'
	    case 'o': cin=0xf6; break;	//0xf6='ö'
	    case 'u': cin=0xfc; break;	//0xfc='ü'
	    }
	}
      else if (cin=='i')
	{
  ...
  // FNX_PRINTF("lang=%i letters: [%c][%c][%c]\n",lang,cin,cr,crr);
  if (lang==1)
    {
      switch (cin)
	{
...
#else
	case 0xe4:  cout='V'; break;	//0xe4='ä'
	case 0xf6:  cout='Q'; break;	//0xf6='ö'
	case 0xfc:  cout='Y'; break;	//0xfc='ü'
#endif
	}
    }
  else if (lang==2)
...
#else
	case 0xe4:  cout='7'; break;	//0xe4='ä'
	case 0xf6:  cout='q'; break;	//0xf6='ö'
	case 0xfc:  cout='6'; break;	//0xfc='ü'
#endif
	}
    }
  // FNX_PRINTF("out lang=%i letters: [%c][%c][%c][%c]\n",lang,cin,cout,cr,crr);
  return cout;
}
			  ...
			  const int newweight)
{
  ...
  //  FNX_PRINTF("trying to enter: %s %s with weight %i\n",newstr,newphon,newweight);

  for (i=0; i < candlist->nlist; i++) 
    {
      /*  if (FNX_STRLEN(newstr) > FNX_STRLEN(candlist->cand[i].str)) */
      if (newweight <  candlist->cand[i].weight)
	{
      ...
      /* else if (FNX_STRLEN(newstr) == FNX_STRLEN(candlist->cand[i].str)) return 0;*/
    }
  ...
  FNX_STRCPY(candlist->cand[ncan-1].str, newstr);
  FNX_STRCPY(candlist->cand[ncan-1].phon, newphon);
...
void add_path_candidates(canditlist *candlist, canditcollection *collected,  MinPathState *dj, const int wordlength)
{
  ...
  */
  ...
  for (i=0;i<candlist->nlist; i++)
    {
	  if (collected->nlist>=MAXCANDCOLLECTION)
	  {
		  return;
	  }
...
void print_candidate_list(canditlist *candlist, const char how)
{
  ...
  switch (how)
	...
	{
	  FNX_PRINTF("%2i",i+1);
	  for (j=0;j<candlist->cand[i].from;j++) FNX_PRINTF(" ");
	  FNX_PRINTF("%s   /%s/%i/  [%i,%i] w=%i type=%i\n", candlist->cand[i].str,candlist->cand[i].phon, 
      ...
      for (i=0;i<candlist->nlist; i++)
	{
	  FNX_PRINTF("%2i",i+1);
	  for (j=0;j<candlist->cand[i].from;j++) FNX_PRINTF(" ");
	  len = FNX_STRLEN(candlist->cand[i].str);
...
int  match_whatever(PLTS_T pLts_t, GERLETTER *word, 
...
//  MinPathState DIJ;
  ...
  //memset(&pLts_t->wunion,0,sizeof(union walkerunion));

//  FNX_PRINTF("number of ltries is %i\n",ntries);
  ...
  len = FNX_STRLEN(word);
  pathnodes=len+1;
  init_dkstate(&pLts_t->wunion.path_data.DIJ, pathnodes);                 // minimum path problem with len+1 nodes 
  pLts_t->wunion.path_data.candcollection.nlist = 0;                  // snippet candidate list empty
  ...
  for (currpos= 0 ; currpos < len; currpos++)
    {
      ...
      init_candidate_list(&pLts_t->wunion.path_data.candlist);  
      ...
      if (is_a_vowelgrapheme(word[currpos])) 
	{
	  ...
	  // FNX_PRINTF("Letter %c  -> /%c/ Explains: %i\n",word[currpos],cout,explain);
	  ...
	  add_to_candidate_list(&pLts_t->wunion.path_data.candlist, outstr, outphon, 0 , from, to , 0 , weight);
	}
	  ...
	  if (nalt > 0)
	      ...
	      for (jj=0; jj<nalt;jj++)
		{
		  ...
		  if (type==1 && (to-from)>=2 )       // minium of three letters and root morpheme: likes stress.
		    {
		      // FNX_PRINTF("This applies to %s\n",  phonemestring);
		      outphon[0]=':'; 
		      FNX_STRCPY(&outphon[1],phonemestring);    // ugly fix for the fact that German roots morphemes have no stress in table.
		    }
		  else
		    {
		      FNX_STRCPY(outphon,phonemestring);
		    }
		  rulenumber = ruleindex[jj];
		  weight = edge_weight(from , to , type , len);       
		  // FNX_PRINTF("at position %i with type %i found %i alternatives %s -> %s with weight %i\n",
		  //	  currpos,type, nalt,outstr, phonemestring,weight);

		  add_to_candidate_list(&pLts_t->wunion.path_data.candlist, outstr, outphon, rulenumber, from, to,type,weight);
		}
	    }
	}
      if (hit) 
	{
	  //      for (i=0;i<currpos;i++) FNX_PRINTF(" ");
	  //	  FNX_PRINTF("%s /%s/%i/\n",outstr,outphon,rulenumber); 
	  //      hit=0
#ifdef FROMTYPED
	  print_candidate_list(&pLts_t->wunion.path_data.candlist,'P');
#endif 
	  add_path_candidates(&pLts_t->wunion.path_data.candlist,&pLts_t->wunion.path_data.candcollection, &pLts_t->wunion.path_data.DIJ, len);
	  hit =0;
	}
    }
  
  // print_edges(&pLts_t->wunion.path_data.DIJ);

  if (sort_edges(&pLts_t->wunion.path_data.DIJ) == 0 ) return 0;

  // print_edges(&pLts_t->wunion.path_data.DIJ);
 
  process_graph(&pLts_t->wunion.path_data.DIJ,pathnodes+1);    // find shortest path algorithm.
  ...
  jj = track_path(&pLts_t->wunion.path_data.DIJ, pathnodes-1 , pLts_t->wunion.path_data.path, pLts_t->wunion.path_data.edgeinfos);

  if (jj<2) return 0;

#ifdef FROMTYPED
  for (j=0; j<jj-1;j++) FNX_PRINTF(" %i Cand %i : %s -> /%s/ %2i  \n",
			       j, pLts_t->wunion.path_data.edgeinfos[j], 
			       pLts_t->wunion.path_data.candcollection.cand[pLts_t->wunion.path_data.edgeinfos[j]].str,  
			       pLts_t->wunion.path_data.candcollection.cand[pLts_t->wunion.path_data.edgeinfos[j]].phon,
			       pLts_t->wunion.path_data.candcollection.cand[pLts_t->wunion.path_data.edgeinfos[j]].rule);

#endif

  for (j=0; j<jj-1;j++)
    {
      candyPt = &(pLts_t->wunion.path_data.candcollection.cand[pLts_t->wunion.path_data.edgeinfos[j]]);    // point to a successful candidate
      MorphPt = &(Mcumulate.morph[Mcumulate.nparts]);
      FNX_STRCPY(MorphPt->wordpart, candyPt->str);
      FNX_STRCPY(MorphPt->phonemepart, candyPt->phon);
      FNX_STRCPY(MorphPt->morphname, "Dijkstra");
  ...
  for (p=word,pp=outword; (c = *p) != 0; p++)
    {
      switch (c)
	{
	case 0xc4:  cc = 0xe4; break;	//0xc4='Ä' 0xe4='ä'
	case 0xe4:  cc = 0xe4; break;	//0xe4='ä' 
	case 0xd6:  cc = 0xf6; break;	//0xd6='Ö' 0xf6='ö'
	case 0xf6:  cc = 0xf6; break;	//0xf6='ö'
	case 0xdc:  cc = 0xfc; break;	//0xdc='Ü' 0xfc='ü'
	case 0xfc:  cc = 0xfc; break;	//0xfc='ü'
	case 0xdf:  cc = 0xdf; break;	//0xdf='ß'
	default:
  ...
  for (p=word,pp=outword; (c = *p) != 0; p++)
    {
      switch (c)
	{
	case 0xc4:  cc = 0xc4; break;	//0xc4='Ä'
	case 0xe4:  cc = 0xc4; break;	//0xe4='ä' 0xc4='Ä'
	case 0xd6:  cc = 0xd6; break;	//0xd6='Ö'
	case 0xf6:  cc = 0xd6; break;	//0xf6='ö' 0xd6='Ö'
	case 0xdc:  cc = 0xdc; break;	//0xdc='Ü'
	case 0xfc:  cc = 0xdc; break;	//0xfc='ü' 0xdc='Ü'
	case 0xdf:  cc = 0xdf; break;	//0xdf='ß'
	default:
	  if (c>='a'&& c<='z') cc = (GERLETTER) (c-32);
	  else  cc = c;
...
{
    int i;
    FNX_PRINTF("Call statistics for rule lines\n");

    for (i=0; i< number ; i++)
      {
	if (Nodes[i].typeinfo == ALTNODE)
	  {
	    FNX_PRINTF("Line %4d succeeded %8d times.\n",Nodes[i].info.parameter,Nodes[i].info.counter);
	...
	if (Nodes[i].typeinfo == ALTNODE)
	  {
	    if (Nodes[i].info.counter > 0) 
	      {
		FNX_PRINTF("%4d ",Nodes[i].info.parameter);
		Nodes[i].info.counter=0;
	      }
	  }
      }
      ...
      */
{
  ...
  //  FNX_PRINTF("Registering %s with rule %d\n",phone,rule);
  ...
  switch (action)
      ...
      for (i=from;i<=to;i++) 
	{
      ...
      FNX_STRCPY(M->morph[n].phonemepart,phone);
      M->morph[n].rulenumber = rule;
      break;
    case 1:
      FNX_STRCPY(M->morph[n].wordpart,name);
      FNX_STRCPY(M->morph[n].morphname,name);        // morphname in this case is an inserted string such as "+"
      FNX_STRCPY(M->morph[n].phonemepart,name);
...
void split_inline_morph_phonemes(const GERLETTER *name, U16 *comparelength, U16 *phonestart, U16 *phonelength, U16 *rule)
...

{
  ...
  *rule=0;
  ...
  if ((*phonestart + *phonelength+1)<len)
    {
      *rule = FNX_ATOI(&name[*phonestart+*phonelength+1]);
    }
}
...
void display_separated_word(GERLETTER *word, MorphemeStruct *M)
{
  ...
  for (i=M->nparts; i>0; i--)
    {
      FNX_PRINTF("%s", &(M->morph[i-1].wordpart));
    }
  //  FNX_PRINTF("\n");
}
#endif
...
void process_stress(GERLETTER *phonemes)
{
...
#ifdef FROMTYPED   
  // for (i=0;i<npos;i++) FNX_PRINTF("pos: %3i %3i \n",address[i]+1, stress[i]);
#endif
  ...
  //  FNX_PRINTF("fixed stress: %i.  Focus %i  Focus2: %i Focus 3: %i\n",fixedstress, focus,focus2, focus3);
...
BOOL morphemize_word(void * pLts_t,const GERLETTER *word, 
		     GERLETTER *outword, 
		     GERLETTER *outphonemes, 
		     WalkCNTXT *Contxt,
                                                ...
                                                /* const HPatterns *TOK) no longer */
{
  ...
  // FNX_PRINTF("pathmethod: %s -> /%s/ \n",wordbuff,outphonemes);
  // #endif 
#ifdef FROMTYPED     
  FNX_PRINTF("Gtwalker Iterations: %i Maxlevel: %i \n", Contxt->counter,Contxt->maxlevel); 
#endif
  if (walki.succeeded == 1) 
    {
	  ...
	  FNX_PRINTF("Phoneme string : /%s/.\n",wordbuff);
#endif
      ret=1;
    }
  else
    {
      ret = match_whatever(pLts_t,wordbuff, outphonemes, Wt->Ltreelist, Wt->notries,  Wt->MorphDict);
#ifdef FROMTYPED     
      FNX_STRCPY(outword,Contxt->theword);
      if (ret) FNX_PRINTF("pathmethod: %s -> /%s/ \n",wordbuff,outphonemes);
      else     FNX_PRINTF("pathmethod failed\n");
#endif
    }
  return ret;
}
