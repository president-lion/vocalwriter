...
#ifdef DEBG
    if (dbgls1) {
		FNX_PRINTF ("ls2.c do2digits entrée %c %c\n",lp->l_ch,(lp+1)->l_ch);
    }
#endif	// DEBG
...
#ifdef DEBG
    if (dbgls1) FNX_PRINTF ("ls2.c do2digits sortie\n");
#endif	// DEBG
}
...
int ls_proc_do_number(LPTTS_HANDLE_T phTTS, LETTER *llp, LETTER *rlp, int oflag)
{
	...
	ndig  = 0;
#ifdef LS2DEBUG
	FNX_PRINTF("In ls_proc_do_number\n");
...
int ls_proc_is_time(PLTS_T pLts_t, LETTER *llp, LETTER *rlp)
{
...
#ifdef DEBG
    if (dbgls1) {
		//FNX_PRINTF ("ls2.c istime entr‚e : "); Ecrit (llp, rlp);
    }
#endif	// DEBG
    ...
    if (dbgls1) 
	{
		FNX_PRINTF ("l_fr_pr1.c ls_proc_is_time sortie Resu=%d\n", Resu);
    }
#endif	// DEBG
