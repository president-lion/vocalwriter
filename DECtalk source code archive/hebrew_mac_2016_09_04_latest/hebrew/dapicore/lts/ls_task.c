...

{
	...
	/* FNX_PRINTF("this is the start of lts\n"); */
...
#ifdef FRENCH
	FNX_STRCPY(pLts_t->precedent, "");
	pLts_t->nitem.i_nword = 0; /* nothing in nitem */
#endif

}

#if (!defined WIN32_STATIC && !defined EPSON_ARM7) || defined MULTIPLE_LANGUAGES_LOADED
...
//    FNX_PRINTF("l_input = 0x%04X 0x%04X 0x%04X\n", input[0], input[1], input[2]);
	...
	if ((input[0]&PFONT)==(PFASCII<<PSFONT))
	{
		...
		if ( (((phTTS->ph_funcs.char_types[pLts_t->input_array[temp]])&MARK_space) && 
			  ((phTTS->ph_funcs.char_types[pLts_t->input_array[temp-1]])&MARK_clause)) ||
			 (temp>400 && ((phTTS->ph_funcs.char_types[pLts_t->input_array[temp]])&MARK_space)) ||
			 (temp>480) ||
	...
	else if ((input[0]&PFONT)==(PFCONTROL<<PSFONT))
	{
		...
		switch (input[0]&(PFONT|PVALUE))
		{
			...
			case INDEX_VOLUME:
			case INDEX_NOISE:	
			case INDEX_REPLACE_START:	
			case INDEX_REPLACE_END:	
				if (pLts_t->cur_input_pos==0)
				{
					goto parse_label;
				}
				//handle asian langauge index marks
#ifdef NEW_MULTI_LANG_ARCH
				if (phTTS->cur_lts_lang==LANG_japanese || phTTS->cur_lts_lang==LANG_chinese ||
					phTTS->cur_lts_lang==LANG_korean)
				{
					goto parse_label;
				}
#else
...
#else
#ifdef NEW_MULTI_LANG_ARCH
			phTTS->ph_funcs.ph_loop(phTTS,input);
#else
			ph_loop(phTTS,input);
#endif
#endif
...
void lts_main_loop(LPTTS_HANDLE_T phTTS)
{
...
#endif
#ifdef CHINESE
    char  outphonemes[1000];
    char  junkbuffer[600];
#endif
	pLts_t->cur_read_pos=0;

#ifdef JAPANESE
	ls_rule_do_japanese(phTTS, NULL, TRUE,FALSE);
	return;
#endif
#ifdef CHINESE
	if(chinese_pinyin2arpa(pLts_t->input_array, outphonemes,junkbuffer, NULL))
	{
		mandarin_tone_process(phTTS,outphonemes);
	}
	
	return;
#endif
#ifdef KOREAN
	ls_rule_korean_lts(phTTS);
	return;
#endif
		 ...
		 second_time==0)
	{
		...
		if (/*(windbg_flag == 0) && ET 6/9/1998*/(*phTTS->ph_funcs.Thread_Alive == 0) && 
			(*phTTS->ph_funcs.in_winmain==0) && (pKsd_t->debug_switch != 0 || pKsd_t->logflag != 0))
		{
			phTTS->ph_funcs.WINstart_thread();
		...
		{
			FNX_PRINTF("LTS debug switch description:\n");
			FNX_PRINTF("4001 -- Incoming data string  4002 -- Dictionary HIT/MISS (0) for miss\n");
			FNX_PRINTF("4004 -- Suffix stripping HIT  4008 -- Display MISS word, not speaking\n");
			FNX_PRINTF("4010 -- Dict. search trace    4020 -- Homograph rule HIT\n");
			FNX_PRINTF("4040 -- Form class array dump 4080 -- ACNA name type\n");
			FNX_PRINTF("4100 -- log form class        4200 -- dump data before write_pipe\n");
			FNX_PRINTF("4400 -- Spell it rule HIT     4800 -- drain data before write_pipe\n");
			FNX_PRINTF("1002 -- Parser Output\n");
		...
		{
#ifndef ARM7_NOSWI
			ls_util_dump_cword (phTTS,pLts_t->cword, "LTS Input:");
#endif
		}
#ifdef DUMP_WORD
		ls_util_dump_cword (phTTS,pLts_t->cword, "ltsinput:");
#endif
#endif
		done=KEEP_SEARCHING;
		/* 
...
#ifdef LS1DEBUG
			FNX_PRINTF("after find end of word ");
...
#ifdef GERMAN
		if (!done)
			done=ls_task_check_german_gender(phTTS,llp,rlp);
#endif
		...
		{   /* no break : we have to do 1 then 2, etc */
    		...
    		*/
      		...
      		case 4:
    			if (!(pLts_t->contgc)) 
					FNX_STRCPY(pLts_t->precedent, ""); /* don't keep cword as left context */
    	} /* switch */
#endif	// FRENCH
...
//#endif				
		...
		case '?':
			...
			{
#ifdef LS1DEBUG
				FNX_PRINTF("Changing question mark to period! \n");
#endif
...
int ls_task_set_what_state(LPTTS_HANDLE_T phTTS, PLTS_T pLts_t)  
{
	LETTER *llp;
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_set_what_state\n");
#endif
...
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_spell_mode\n");
#endif
	if ((pKsd_t->modeflag&MODE_SPELL)!=0) 
	{

#ifdef LS1DEBUG
		FNX_PRINTF(" mode spell spell\n");
#endif
...
int ls_task_minidic_search(LPTTS_HANDLE_T phTTS, LETTER *llp, LETTER *rlp)
{
	...
	pKsd_t = phTTS->pKernelShareData;   
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_minidic_search\n");
#endif
...
/* ******************************************************************
 *  Function Name:  
 *      ls_task_check_german_gender
 *
 *  Description:              
 *      search the mini dictionary for german gender words
 *
 *  Arguments:                                                      
 *		LPTTS_HANDLE_T phTTS	Text-to-speech handle
 *      LETTER *llp				the left bounding pointer of the word
 ...
 * *****************************************************************/
#ifdef GERMAN
int ls_task_check_german_gender(LPTTS_HANDLE_T phTTS, LETTER *llp, LETTER *rlp)
{
	char    *cp;
	PLTS_T  pLts_t;
	PKSD_T  pKsd_t;
	...
	pKsd_t = phTTS->pKernelShareData;   
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_check_german_gender\n");
#endif
	// CAB 08/15/2002	Removed warning by typecast
	if ( (cp = (char *)ls_task_wlookup(pLts_t, llp, (unsigned char *)&maledic[0])) != NULL)
	{       /*eab mini dictionary lookup */
...
//	PKSD_T pKsd_t = phTTS->pKernelShareData;
#ifdef DEBG
    //if (DbgLs1) FNX_PRINTF ("ls1.c categgram entree\n");
#endif
...
#ifdef DEBG
    if (DbgLs1) FNX_PRINTF ("ls_task.c ls_task_categ_gram sortie %4x\n", (FRGC << 8) | *pt);
#endif
...
int ls_task_dictionary_search(LPTTS_HANDLE_T phTTS, LETTER *llp, LETTER *rlp)
{
...
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_dictionary_search\n");
#endif
	...
	else 
	{
		...
		if (ls_util_is_clause(phTTS)==TRUE && rlp>=llp+2 && (rlp-1)->l_ch=='.')
		{
			context=FABBREV;
			has_dot=1;
			(rlp-1)->l_ch=0;
...
#endif
#if (defined SPANISH) || (defined ITALIAN)|| (defined JAPANESE)|| (defined HEBREW)|| (defined CHINESE) || (defined KOREAN)
...
void ls_task_strip_left_punctuation(PLTS_T pLts_t, LETTER **llp, LETTER *rlp)        
{
	 ...
	 */
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_strip_left_punctuation\n");
#endif
	 pLts_t->lflag  = 0;                             /* Clear all flags.     */

#ifdef LS1DEBUG
	FNX_PRINTF("before punctuation stripping ");
	FNX_PRINTF("left=%u  right=%u\n",(*llp),rlp);
#endif
	pLts_t->lbphone = WBOUND;
	pLts_t->rbphone = WBOUND;
	...
	{
#ifdef LS1DEBUG
			FNX_PRINTF("in left stripping loop %c\n",(*llp)->l_ch);
#endif
		pLts_t->lflag |= LSTRIP;
...
{
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_strip_right_punctuation\n");
#endif
	/* Change this "while" to an "if" to strip only 1 layer. */
	while ((*rlp)!=llp && (lsctype[((*rlp)-1)->l_ch]&RS)!=0) 
	{
#ifdef LS1DEBUG
			FNX_PRINTF("in right stripping loop %c\n",llp->l_ch);
#endif
		pLts_t->lflag |= RSTRIP;
		--(*rlp);
...
int ls_task_spell_all_punct(LPTTS_HANDLE_T phTTS,LETTER *llp, LETTER *rlp, LETTER *lp1, LETTER *lp2)
{
	...
	pLts_t = phTTS->pLTSThreadData;
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_spell_all_punct\n");
#endif
	if (llp == rlp) 
	{                       /* Null word            */
		ls_spel_spell(phTTS,lp1, lp2);  /* Always slowly.       */
#ifdef LS1DEBUG
			FNX_PRINTF("null word spell\n");
#endif
...
#ifdef LS1DEBUG
	FNX_PRINTF("after punctuation stripping ");
	FNX_PRINTF("left=%u  right=%u\n",llp,rlp);
#endif
...
int ls_task_dictionary_after_punct(LPTTS_HANDLE_T phTTS, LETTER *llp, LETTER **rlp)
{
	...
	pLts_t = phTTS->pLTSThreadData;
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_dictionary_after_punct\n");
#endif
 ...
 */
...
#ifdef LS1DEBUG
	FNX_PRINTF("entering ls_task_spell_vs_speak\n");
	FNX_PRINTF("ls_spel_say_it ");
	FNX_PRINTF("left=%u  right=%u\n",llp,rlp);
#endif
	...
	{
#ifdef LS1DEBUG
			FNX_PRINTF("ls_spel_say_it spell\n");
#endif
				/* debug switch */
...
int ls_task_single_letter_spell_rules(LPTTS_HANDLE_T phTTS, LETTER *llp, LETTER *rlp)
{
...
#ifdef LS1DEBUG
	FNX_PRINTF("entering ls_task_single_letter_spell_rules");
	FNX_PRINTF("left=%u  right=%u\n",llp,rlp);
#endif 
	...
	if (llp+1 == rlp)
	{
		...
		{
#ifdef LS1DEBUG
			FNX_PRINTF("single letter spell\n");
#endif
...
int ls_task_set_sign_flag(LPTTS_HANDLE_T phTTS, LETTER *llp, LETTER *rlp, LETTER **lp1)
{
...
#ifdef LS1DEBUG       
	FNX_PRINTF("entering ls_task_set_sign_flag\n");
	FNX_PRINTF("beginnning number processing   ");
	FNX_PRINTF("left=%u  right=%u\n",llp,rlp);
#endif
	...
	||  (*lp1)->l_ch == 0xB6)          			/* Item.                */
		...
		++(*lp1);
#ifdef LS1DEBUG
		FNX_PRINTF("num processing sign found\n");
#endif
		if ((*lp1) == rlp)
		{
#ifdef LS1DEBUG
			FNX_PRINTF(" spell from num processing\n");
#endif
...
{   
...
#ifndef FRENCH
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_currency_processing\n");
#endif
	/*
	...
	if (llp->l_ch=='$' || llp->l_ch==0xA3 || llp->l_ch==0x80) 
	{
		...
		{
#ifdef LS1DEBUG
			FNX_PRINTF("currency mark spell \n");
#endif
		...
		{        						/* Plus-minus.          */
			...
			{
#ifdef LS1DEBUG
			FNX_PRINTF(" spell from num sign proc \n");
#endif
		...
		{
#ifdef LS1DEBUG
			FNX_PRINTF("ls_task_parse_number spell \n");
#endif
		...
		*/
		...
		else
		{
			if (lp2==rlp-1) { /* number from lp1 to rlp-1 */
				...
				if (DbgLs1)
		 		{
					FNX_PRINTF ("ls1.c Somme_en_frcs nombre suivi de F pour francs\n");
					//Ecrit (llp, rlp);
				}
#endif
			} /* elp exists or not */
...
#ifdef LS1DEBUG
	FNX_PRINTF("date time processing ");
	FNX_PRINTF("left=%u  right=%u\n",llp,rlp);
#endif
...
/* GL 02/13/1997, support am, pm for spanish as well since sometime they do use */
			...
			{
#ifdef LS1DEBUG
			FNX_PRINTF("ls_proc_is_am_pm spell\n");
#endif
...
int ls_task_frac_processing(LPTTS_HANDLE_T phTTS, LETTER *llp, LETTER *rlp)
{
	...
	pLts_t = phTTS->pLTSThreadData;
#ifdef LS1DEBUG
		FNX_PRINTF("entering ls_task_frac_processing\n");
#endif             
...
//	int Fini=0;
	...
	pLts_t = phTTS->pLTSThreadData;
	 ...
	 EAB-4-11-89*/
#ifdef LS1DEBUG
	FNX_PRINTF("entering ls_task_plain_number_processing\n");
	FNX_PRINTF("number procesing ");
	FNX_PRINTF("left=%u  right=%u\n",llp,rlp);
#endif
      ...
      */
      ...
      if (llp==rlp-1                   /* one digit */
        ...
        */
        pLts_t->contgc = TRUE;
        switch ( (rlp-1)->l_ch) {
          case '1': FNX_STRCPY( pLts_t->precedent, "un");     break;
          case '2': FNX_STRCPY( pLts_t->precedent, "deux");   break;
          case '3': FNX_STRCPY( pLts_t->precedent, "trois");  break;
          case '4': FNX_STRCPY( pLts_t->precedent, "quatre"); break;
          case '5': FNX_STRCPY( pLts_t->precedent, "cinq");   break;
          case '6': FNX_STRCPY( pLts_t->precedent, "six");    break;
          case '7': FNX_STRCPY( pLts_t->precedent, "sept");   break;
          case '8': FNX_STRCPY( pLts_t->precedent, "huit");   break;
          case '9': FNX_STRCPY( pLts_t->precedent, "neuf");   break;
        }
        ...
        switch ( (rlp-2)->l_ch) {
          case '1':
          case '7':
          case '9': FNX_STRCPY( pLts_t->precedent, "dis");    break;
          case '2': FNX_STRCPY( pLts_t->precedent, "vingt");  break;
          case '8': FNX_STRCPY( pLts_t->precedent, "vingts"); break;
        }
      } else if ( llp<=(rlp-3) && (rlp-3)->l_ch!='0') {
        if ( (rlp-2)->l_ch=='0' && (rlp-1)->l_ch=='0' ) {
          pLts_t->contgc = TRUE;
          if ( (rlp-3)->l_ch=='1' ) {
        FNX_STRCPY( pLts_t->precedent, "cent"); /* 100 */
          } else if ( (rlp-3)->l_ch!='0' ) {
        FNX_STRCPY( pLts_t->precedent, "cents");  /* 800 */
          }
        }
      }
#endif // FRENCH
...
/* JDB:  Spanish doesn't use this section! */                        
		...
		{             
			...
			{
				if (pLts_t->sign==0 && ls_util_is_year(llp, lp2)!=FALSE)
				{
					ls_proc_do_4_digits(phTTS,llp);
				}
				else
...
{              
...
#ifdef LS1DEBUG
	FNX_PRINTF("entering ls_task_part_number\n");
	FNX_PRINTF("no more heuristics ");
	FNX_PRINTF("left=%u  right=%u\n",llp,rlp);
#endif        
	lp1=llp;
	while (lp1 != rlp) 
	{
		...
		{
#ifdef LS1DEBUG
			FNX_PRINTF(" drop through spell %c  \n",c);
#endif
...
#ifdef LS1DEBUG
	FNX_PRINTF("entering ls_task_process_word ");
	FNX_PRINTF("left=%u  right=%u\n",llp,rlp);
#endif
...
FNX_PRINTF("sending hypen");
		...
		if (lp2!=lp1)
		{
			...
			{       /* If no consonant, the */
				...
				{ */
#ifdef LS1DEBUG
					FNX_PRINTF(" spell no cons. \n");
#endif
#ifdef NEW_MULTI_LANG_ARCH
if(phTTS->cur_lts_lang != LANG_italian)
#else
...
#ifdef DEBG
			if (DbgLs1) {
				FNX_PRINTF ("ls1.c Word_or_Compound LTS for word without - : ");
				//Ecrit (llp, rlp);
			}
#endif
			/* copyword after FNX_PRINTF : nword in cword for next loop */
...
#ifdef DEBG
			if (DbgLs1) {
				FNX_PRINTF ("ls1.c Word_or_Compound LTS for word with - : ");
				//Ecrit (llp, rlp);
			}
#endif
...
#ifdef DEBG
			if (DbgLs1) {
				FNX_PRINTF ("ls1.c Word_or_Compound spell or lts? : ");
				//Ecrit (lp2, lp1);
			}
#endif
...
#ifdef DEBG
				if (DbgLs1) {
					FNX_PRINTF ("ls1.c Word_or_Compound hyphen  LTS : ");
					//Ecrit (lp2, lp1);
				}
#endif
				
				/* don't keep DOS for next time */
				FNX_STRCPY(pLts_t->precedent, "");       /* nothing on the left */
...
//#if (defined ARM7 || defined _UNIX_LIKE_) && !defined OS_PALM
...
int ls_task_find_verb_particles(LPTTS_HANDLE_T phTTS,LETTER *word_start,LETTER *word_end)
{
	...
	{
		mid=(high+low)>>1;
		value=FNX_STRICMP(word,&(verb_pairs_words[verb_pairs_index[mid]]));
		if (value==0)
		{
			found=1;
			break;
...
void ls_task_parse_sentence(LPTTS_HANDLE_T phTTS)
{
	...
	for (i=0;i<pLts_t->cur_input_pos;i++)
	{
		for (j=i;(j<pLts_t->cur_input_pos && 
			((phTTS->ph_funcs.char_types[pLts_t->input_array[j]])&MARK_space) ); j++,i++);

		k=0;
		for (j=i;(j<pLts_t->cur_input_pos && 
			(!((phTTS->ph_funcs.char_types[pLts_t->input_array[j]])&MARK_space)) && k<199);j++,k++,i++)
		{
	...
	if (DT_DBG(VTM_DBG,0x002))
	{
		for (i=0;i<pLts_t->cur_input_pos;i++)
		{
			FNX_PRINTF("%c",pLts_t->input_array[i]);
		}
		FNX_PRINTF("\n");
		i=1;
		for (j=0;j<pLts_t->cur_input_pos;j++)
		{
			if (((phTTS->ph_funcs.char_types[pLts_t->input_array[j]])&MARK_space))
			{
				FNX_PRINTF(" ");
			}
			else
			{
				FNX_PRINTF("%c",pLts_t->input_array[j]);
			}
			if (((phTTS->ph_funcs.char_types[pLts_t->input_array[j]])&MARK_space) && 
				((phTTS->ph_funcs.char_types[pLts_t->input_array[j-1]])&MARK_space)==0)
			{
				FNX_PRINTF("      ");
				...
				for (k=0;k<32;k++)
				{
					...
					{
						FNX_PRINTF("%s ",form_class_strings[k]);
					}
					start<<=1;
				}
				i++;
