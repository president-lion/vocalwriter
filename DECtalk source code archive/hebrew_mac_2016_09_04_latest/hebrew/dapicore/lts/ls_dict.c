...
int ls_dict_find_word(LPTTS_HANDLE_T phTTS, int nosend)
{
	...
	do
	{
		...
		if (stat == LOOK_HIGHER)
		{
			...
			{
#ifndef ARM7_NOSWI
				FNX_PRINTF("ls_dict_find_word:  up \n");
#endif
			}

#ifdef DICDEBUG
			FNX_PRINTF("LSDIC.C; ls_dict_find_word;  up \n");
#endif /* DICDEBUG */
			base += offset;
		}
	    else
	    {
			...
			{
#ifndef ARM7_NOSWI
				FNX_PRINTF("ls_dict_find_word:  down \n");
#endif
			}
#ifdef DICDEBUG
			FNX_PRINTF("LSDIC.C; ls_dict_find_word;  down \n");
#endif /* DICDEBUG */
		}
	} while (offset != 0);
	...
	if (stat != HIT)
	{
		...
		{
#ifndef ARM7_NOSWI
			FNX_PRINTF("ls_dict_find_word:  crawling, stat=%x\n",stat);
#endif
		}
#ifdef DICDEBUG
		FNX_PRINTF("LSDIC.C; ls_dict_find_word;  crawling, stat=%x \n",stat);
#endif /* DICDEBUG */
		if (stat == LOOK_HIGHER)
		{
	...
	{
#ifndef ARM7_NOSWI
		FNX_PRINTF("ls_dict_find_word:  miss \n");
#endif
	}
#ifdef DICDEBUG
		FNX_PRINTF("LSDIC.C; ls_dict_find_word;  miss. \n");
#endif /* DICDEBUG */
	if (stat != HIT)
	...
	{
#ifndef ARM7_NOSWI
		FNX_PRINTF("ls_dict_find_word:  cap:%x\n",cap);
#endif
	}
#ifdef DICDEBUG
	FNX_PRINTF("LSDIC.C; ls_dict_find_word;  cap:%x \n",cap);
#endif /* DICDEBUG */
	...
	{
#ifndef ARM7_NOSWI
		FNX_PRINTF("ls_dict_find_word:  capit. homograph reverse");
#endif
	}
#ifdef DICDEBUG
		FNX_PRINTF("LSDIC.C; ls_dict_find_word;  capit. homograph reverse \n");
#endif /* DICDEBUG */								     
	...
	{
#ifndef ARM7_NOSWI
		FNX_PRINTF("ls_dict_find_word:  HIT complete.\n");
#endif
	}
#ifdef DICDEBUG
	FNX_PRINTF("LSDIC.C; ls_dict_find_word;  HIT complete. \n");
#endif /* DICDEBUG */
...
//		}
	...
	{
#ifndef ARM7_NOSWI
		FNX_PRINTF("ls_dict_find_word:  HIT");
#endif
	}
#ifdef DICDEBUG
	FNX_PRINTF("*");
...
int ls_dict_dlook(LPTTS_HANDLE_T phTTS, long index, int *pLocaloff, struct dic_entry far **ppent)
{
	...
	if (index<0)
	{
		...
		{
#ifndef ARM7_NOSWI
			FNX_PRINTF("ls_dict_dlook:  limit. LOOK_HIGHER\n");
#endif
		}
#ifdef DICDEBUG
		FNX_PRINTF("LSDIC.C; ls_dict_dlook;  limit, LOOK_HIGHER \n");
#endif /* DICDEBUG */
		return(LOOK_HIGHER);
	}
	if (index>limit)
	{
		...
		{
#ifndef ARM7_NOSWI
			FNX_PRINTF("ls_dict_dlook:  limit. LOOK_LOWER\n");
#endif
		}
#ifdef DICDEBUG
		FNX_PRINTF("LSDIC.C; ls_dict_dlook;  limit. LOOK_LOWER \n");
#endif /* DICDEBUG */
		return(LOOK_LOWER);
	}
	...
	*ppent = (struct dic_entry far *)DICT_ACCESS(index);
	...
	{
#ifndef ARM7_NOSWI
		FNX_PRINTF("ls_dict_dlook:%d:",index);
		for (i=0;(*ppent)->text[i];i++)
		FNX_PRINTF("%c",(*ppent)->text[i]);
		FNX_PRINTF("\n");
#endif
	}
#ifdef DICDEBUG
	FNX_PRINTF("LSDIC.C; ls_dict_dlook;");
	for (i=0;(*ppent)->text[i];i++)
		FNX_PRINTF("%c",(*ppent)->text[i]);
	FNX_PRINTF("\n");
#endif /* DICDEBUG */
...
//#endif
	...
	{
#ifndef ARM7_NOSWI
		FNX_PRINTF("ls_dict_dlook:  short string. LOOK_LOWER\n");
#endif
	}
#ifdef DICDEBUG
			FNX_PRINTF("LSDIC.C; ls_dict_dlook;  short string, LOOK_LOWER\n");
#endif /* DICDEBUG */
			pLts_t->hit_type = MISS;
			 ...
			 * to decide the LOOK_LOWER or LOOK_UPPER.(need LOOK_LOWER here)
                         ...
                         * "ms." if possible
			 */
			if (pLts_t->abbrev_look==1)
			{
				if (((*ppent)->text[i]=='-') && (pLts_t->comp_str[i] == '\0'))
				{
					return(LOOK_HIGHER);
				}
			}
		...
		{
#ifndef ARM7_NOSWI
			FNX_PRINTF("ls_dict_dlook:  limit. LOOK_HIGHT\n");
#endif
		}
#ifdef DICDEBUG
			FNX_PRINTF("LSDIC.C; ls_dict_dlook;  Limit, LOOK_HIGHER \n");
	...
	{
#ifndef ARM7_NOSWI
		FNX_PRINTF("ls_dict_dlook:  long string. LOOK_HIGHER\n");
#endif
	}
#ifdef DICDEBUG
	FNX_PRINTF("LSDIC.C; ls_dict_dlook;  long string, LOOK_HIGHER \n");
#endif /* DICDEBUG */
	return(LOOK_HIGHER);
}
/* ******************************************************************
...
{	
	...
	if ( (pLts_t->comp_str[i]=='\0') && (pent->text[i]=='\0'))
	{
		...
		{
#ifndef ARM7_NOSWI
			FNX_PRINTF("ls_dict_where_to_look:  w-t-l match. LOOK_HIGHER\n");
#endif
		}
#ifdef DICDEBUG
		FNX_PRINTF("LSDIC.C; ls_dict_where_to_look;  w-t-l match: LOOK_HIGHER \n");
#endif /* DICDEBUG */
		return(LOOK_HIGHER);
	}
	if(ls_upper[pLts_t->comp_str[i]] > pivot_char)
	{
		...
		{
#ifndef ARM7_NOSWI
			FNX_PRINTF("ls_dict_where_to_look:  w-t-l. LOOK_HIGHER\n");
#endif
		}
#ifdef DICDEBUG
		FNX_PRINTF("LSDIC.C; ls_dict_where_to_look;  w-t-l: LOOK_HIGHER \n");
#endif /* DICDEBUG */
		return(LOOK_HIGHER);
	}
	/* debug switch */
	...
	{
#ifndef ARM7_NOSWI
		FNX_PRINTF("ls_dict_where_to_look:  w-t-l. LOOK_LOWER\n");
#endif
	}
#ifdef DICDEBUG
	FNX_PRINTF("LSDIC.C; ls_dict_where_to_look;  w-t-l: LOOK_LOWER \n");
#endif /* DICDEBUG */
	return(LOOK_LOWER);
}
...
int ls_dict_user_dict_look(LPTTS_HANDLE_T phTTS, long uindex, short abbr, short nosend)
{
	...
	/*	pLts_t->hit_type=HIT; */
#ifdef DICDEBUG
		FNX_PRINTF("LSDIC.C; ls_dict_user_dict_look;  %d in lk\n",uindex);
#endif
	...
	for(i=0;ent[i] != '\0';i++)
	{
		...
		{   
#ifdef DICDEBUG
			FNX_PRINTF("leaving ls_dict_user_dict_look lower 1\n");
#endif
			return(LOOK_LOWER);
		}
		if(IS_LOWER(ent[i])  && (pLts_t->comp_str[i] == ls_upper[(int)ent[i]]))
			continue;                       
#ifdef DICDEBUG
		FNX_PRINTF("leaving ls_dict_user_dict_look  where to look\n");
#endif
	...
	if(pLts_t->comp_str[i] == '\0')
	{
...
#endif
		{
			...
			if(two_byte_phon)
			{
				if (ent[i]>0x18) // HACK HACK!! This should be changed after version 5.1
								// this is to enable the use of older user dictionaries on the new architecture
				{
					temp = ((short)(0x001f-ent[i])<<PSFONT);
				}
				else
				{
		...
		two_byte_phon = 0;
#ifdef DICDEBUG
		FNX_PRINTF("leaving ls_dict_user_dict_look hit\n");
#endif /* DICDEBUG */
		return(HIT);
	}
#ifdef DICDEBUG
	FNX_PRINTF("leaving ls_dict_user_dict_look higher\n");
#endif // DICDEBUG
	return(LOOK_HIGHER);
}
...
{	
...
#ifdef DICDEBUG
	FNX_PRINTF("in ls_dict_where_to_ulook\n");
#endif
	for(i=0;pLts_t->comp_str[i];i++)
	{
	...
	{
#ifdef DICDEBUG
		FNX_PRINTF("leaving ls_dict_where_to_ulook higher\n");
#endif
		return(LOOK_HIGHER);                         
	}
#ifdef DICDEBUG
		FNX_PRINTF("leaving ls_dict_where_to_ulook lower\n");
#endif
	return(LOOK_LOWER);
}
...
long DictionaryHit(LPTTS_HANDLE_T phTTS, struct dic_entry *entry)
{
	...
	if (FNX_STRCMP(entry->text, DICT_ACCESS(first)->text) == 0)
		return first; /* a form of the word is in the dictionary */

	if (FNX_STRCMP(entry->text, DICT_ACCESS(last)->text) == 0)
	...
	FNX_STRCPY(pLts_t->comp_str, DICT_ACCESS(first)->text);
	if (ls_dict_where_to_look(phTTS, entry) == LOOK_HIGHER)
		return -1;

	FNX_STRCPY(pLts_t->comp_str, DICT_ACCESS(last)->text);
	...
	{
		FNX_STRCPY(pLts_t->comp_str, DICT_ACCESS(mid)->text);
		temp = ls_dict_where_to_look(phTTS, entry);
		if (FNX_STRCMP(entry->text, pLts_t->comp_str) == 0)
...
MMRESULT DumpDictionary(LPTTS_HANDLE_T phTTS, char *filename)
{
	...
	if ((outfile = FNX_FOPEN(filename, "w")) == NULL)
		return MMSYSERR_ERROR;

	pKsd_t = phTTS->pKernelShareData;

	FNX_FPRINTF(outfile, "Total dictionary entries: %d\n\n", DICT_ENTRY);
	for (lcv = 0; lcv < DICT_ENTRY; lcv++)
	{
		for (phoneme_ptr = DICT_ACCESS(lcv)->text + (FNX_STRLEN(DICT_ACCESS(lcv)->text) + 1), lcv2 = 0;
		...
		FNX_FPRINTF(outfile, "%s, %s, %s,", DICT_ACCESS(lcv)->text, arpabet_ph, ascky_ph);
		if (DICT_FC_ACCESS(DICT_ACCESS(lcv)->fc[0]))
		{
			...
			for (lcv2 = 0; lcv2 < 32; lcv2++)
			{
				if (DICT_FC_ACCESS(DICT_ACCESS(lcv)->fc[0]) & fc_mask)
					FNX_FPRINTF(outfile, "%s", form_class_strings[lcv2]);
				fc_mask *= 2;
			}
		}
		else
			FNX_FPRINTF(outfile, " none");
		FNX_FPRINTF(outfile, "\n");
	}
...
long UserDictionaryHit(LPTTS_HANDLE_T phTTS, struct dic_entry *entry)
{
	...
	if (FNX_STRCMP(entry->text, UDICT_ACCESS(first)->text) == 0)
		return first; /* a form of the word is in the dictionary */

	if (FNX_STRCMP(entry->text, UDICT_ACCESS(last)->text) == 0)
		return last; /* a form of the word is in the dictionary */

	/* first we check to see if the entry is not within the range of the current entries */
	FNX_STRCPY(pLts_t->comp_str, UDICT_ACCESS(first)->text);
	if (ls_dict_where_to_ulook(pLts_t, entry->text) == LOOK_HIGHER)
		return -1;

	FNX_STRCPY(pLts_t->comp_str, UDICT_ACCESS(last)->text);
	...
	while ((last - first) >> 1 != 0)
	{
		FNX_STRCPY(pLts_t->comp_str, UDICT_ACCESS(mid)->text);
...
MMRESULT DumpUserDictionary(LPTTS_HANDLE_T phTTS, char *filename)
{
	...
	FNX_FPRINTF(outfile, "Total user dictionary entries: %d\n\n", UDICT_ENTRY);
	for (lcv = 0; lcv < UDICT_ENTRY; lcv++)
	{
		for (phoneme_ptr = UDICT_ACCESS(lcv)->text + (FNX_STRLEN(UDICT_ACCESS(lcv)->text) + 1), 
...
int GetUserEntrySize(struct dic_entry *entry)
{
	...
	phoneme_ptr = entry->text + (FNX_STRLEN(entry->text) + 1);

	/* add the lengths of the grapheme and phoneme strings (including the null characters) */
	entry_size = FNX_STRLEN(entry->text) + FNX_STRLEN(phoneme_ptr) + 2;
...
MMRESULT AddUserEntry(LPTTS_HANDLE_T phTTS, struct dic_entry *entry)
{
	...
	if (UDICT_INDEX == NULL)
	{
...
#else
		if (!(UDICT_INDEX_NC = (S32 *) FNX_MALLOC(4)))
#endif
			return MMSYSERR_NOMEM;
#if (defined WIN32) && (!defined UNDER_CE)
		if (!(UDICT_DATA = (unsigned char *) mallocLock(entry_size)))
#else
		if (!(UDICT_DATA = (unsigned char *) FNX_MALLOC(entry_size)))
#endif
			return MMSYSERR_NOMEM;
		...
		{
			char szTemp[256];
			FNX_SPRINTF(szTemp, "Allocated user dictionary; UDICT_INDEX:%08lx\n",
				UDICT_INDEX);
...
#else
	if ((UDICT_INDEX_NC = (U32*)FNX_REALLOC(UDICT_INDEX, (UDICT_ENTRY+1) * sizeof(S32))) == NULL)
	{
		return MMSYSERR_NOMEM;
	}
	if ((UDICT_DATA = FNX_REALLOC((unsigned char *)UDICT_DATA, new_size)) == NULL)
	{
		return MMSYSERR_NOMEM;
	}
		...
		{
			char szTemp[256];
			FNX_SPRINTF(szTemp, "Reallocated user dictionary; UDICT_INDEX:%08lx\n",
				UDICT_INDEX);
			OutputDebugString(szTemp);
		}
#endif // LTS_DEBUG
...
MMRESULT DeleteUserEntry(LPTTS_HANDLE_T phTTS, struct dic_entry *entry)
{
					      ...
					      memory block it's occupying and return. */
	{
		...
		{
			char szTemp[256];
			FNX_SPRINTF(szTemp, "Freeing user dictionary; UDICT_INDEX:%08lx\n",
				UDICT_INDEX);
			OutputDebugString(szTemp);
		}
#endif // LTS_DEBUG
		...
		freeLock(UDICT_DATA);
#else
		FNX_FREE(UDICT_INDEX);
		FNX_FREE((unsigned char *)UDICT_DATA);
#endif
		UDICT_INDEX_NC = NULL;
		UDICT_DATA=NULL;
		UDICT_ENTRY = 0;
...
#else
	UDICT_INDEX_NC = (U32 *)FNX_REALLOC(UDICT_INDEX, (UDICT_ENTRY-1) * sizeof(S32));
	UDICT_DATA = FNX_REALLOC((unsigned char *)UDICT_DATA, new_size);
#endif
						   ...
						   unsigned char *new_phoneme)
{
	...
	phoneme_ptr = entry->text + (FNX_STRLEN(entry->text) + 1);
	FNX_STRCPY(phoneme_ptr, new_phoneme);
...
int GetUserEntry(LPTTS_HANDLE_T phTTS, char *gr_ph, int index)
{
	...
	FNX_STRCPY(gr_ph, UDICT_ACCESS(index)->text); /* copy the grapheme */

	for (phoneme_ptr = UDICT_ACCESS(index)->text + (FNX_STRLEN(UDICT_ACCESS(index)->text) + 1),
	...
	phoneme_ptr = gr_ph + (FNX_STRLEN(gr_ph) + 1);
	FNX_STRCPY(phoneme_ptr, arpabet_ph); /* copy the phoneme */
	
	return FNX_STRLEN(gr_ph) + 1; /* returns the starting index of the phoneme */
}
...
MMRESULT SaveUserDictionary(LPTTS_HANDLE_T phTTS, char *filename)
{
	...
	if ((outfile = FNX_FOPEN(filename, "wb")) == NULL)
		return MMSYSERR_ERROR;

	/* output number of entries */
	FNX_FWRITE((S32 *)&UDICT_ENTRY, 4, 1, outfile);

	pointer_list_size = sizeof(void *) * UDICT_ENTRY;
	bytes = UDICT_BYTES;

	/* output number of bytes */
	FNX_FWRITE(&bytes, 4, 1, outfile);

	/* output the offsets */
	for (lcv = 0; lcv < UDICT_ENTRY; lcv++)
	{
		offset = UDICT_INDEX[lcv];
		FNX_FWRITE(&offset, 4, 1, outfile);
	}

	/* output all of the entries */
	FNX_FWRITE((unsigned char *)&(UDICT_DATA[0]), bytes, 1, outfile);
