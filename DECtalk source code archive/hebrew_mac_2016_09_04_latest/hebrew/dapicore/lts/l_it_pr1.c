...
//		lbuf[1].l_ip = NULL;
		...
		{
			ls_util_send_phone(phTTS, IT_A);         /* Eh?                  */
		}
		ls_util_send_phone(phTTS, WBOUND);
	}
}
...
FNX_PRINTF("oflag is %d\n",pLts_t->ord);
FNX_PRINTF("pflag is %d\n",pLts_t->pflag);
*/
	switch(lp->l_ch)
	{
		case 0 :
#ifdef NUMDEBUG
	FNX_PRINTF("In do2spell() \n");
#endif
			ls_spel_spell(phTTS, lp, lp+2);
			break;
		case '1':
#ifdef NUMDEBUG
	FNX_PRINTF("In do2case 1() \n");
#endif
		...
		else if (lp->l_ch=='1')
		{
			if ((lp+1)->l_ch=='0' && (lp+2)->l_ch=='0')
			{
				if (pLts_t->ord == 1)
				{
					...
					ls_util_send_phone(phTTS, IT_O);
					pLts_t->ord = 0;
				}
				else if (pLts_t->ord == 2)
				{
					ls_util_send_phone_list(phTTS, pordonehundred);
					ls_util_send_phone(phTTS, IT_A);
					pLts_t->ord = 0;
				}
				else
				{
...
*/
	...
	FNX_PRINTF("In docase oo & !'1' \n");
		...
		ls_util_send_phone(phTTS, WBOUND);
#ifdef NUMDEBUG
	FNX_PRINTF("In do4case going to do3 \n");
#endif
		ls_util_send_phone_list(phTTS, pthousand);
...
//   int i; // NAL warning removal
   PLTS_T pLts_t = phTTS->pLTSThreadData;                                 
/*                                    
FNX_PRINTF("B:NUMBER is -%c-%c-%c-",buf[0],buf[1],buf[2]);
FNX_PRINTF("oflag is %d %d\n",oflag,pLts_t->ord);
FNX_PRINTF("pflag is %d %d\n",pflag,pLts_t->pflag);
*/
	/* 1xx is a special case and must be handled first*/
#ifdef NUMDEBUG
	FNX_PRINTF("In dogroup() buf0=%c buf1=%c buf2=%c buf3=%c ord=%d\n",buf[0],buf[1],buf[2],buf[3],oflag);
#endif                       
	if (buf[0] != '0' )
	{
		switch(buf[0])
		{
			...
			case '1' :
#ifdef NUMDEBUG
	FNX_PRINTF("In dogroup case 1xx \n");
	FNX_PRINTF("case1 adn \n");         
#endif                  
				if (pLts_t->ord == 1 && oflag == 1)
				{
					ls_util_send_phone_list(phTTS, pordonehundred);
				...
				{
/*					FNX_PRINTF("send phone for 100 \n");  */
				    if (pLts_t->ord != 0 && oflag == 0)
					   ls_util_send_phone_list(phTTS, phundredp);
					else
						if (buf[1] == '0' && buf[2] == '0')
					   		ls_util_send_phone_list(phTTS, ponehundred);
						else if (buf[1] == '0' && buf[2] == '1')
					   		ls_util_send_phone_list(phTTS, phundredp);
						else
					   		ls_util_send_phone_list(phTTS, phundred);
			...
			case '2' :
#ifdef NUMDEBUG
	FNX_PRINTF("In dogroup case 200 \n");
	FNX_PRINTF("case2 adn \n");  
#endif
				if (pLts_t->ord == 1 && oflag == 1)
				{
...
/*						FNX_PRINTF("send phone for 200 \n");  */
			...
			case '3' :
#ifdef NUMDEBUG
	FNX_PRINTF("In dogroup case 300 \n");
	FNX_PRINTF("case3 adn \n");  
#endif
				if (pLts_t->ord == 1 && oflag == 1)
				{
...
/*						FNX_PRINTF("send phone for 300 \n");  */
			...
			case '4' :
#ifdef NUMDEBUG
	FNX_PRINTF("In dogroup case 400 \n");
	FNX_PRINTF("case4 adn \n");  
#endif
				if (pLts_t->ord == 1 && oflag == 1)
				{
...
/*						FNX_PRINTF("send phone for 400 \n");  */
			...
			case '5' :
#ifdef NUMDEBUG
	FNX_PRINTF("In dogroup case 500 \n");
	FNX_PRINTF("case5 adn \n");  
#endif
				if (pLts_t->ord == 1 && oflag == 1)
				{
...
/*						FNX_PRINTF("send phone for 100 \n");  */
			...
			case '6' :
#ifdef NUMDEBUG
	FNX_PRINTF("In dogroup case 600 \n");
	FNX_PRINTF("case6 adn \n");  
#endif
				if (pLts_t->ord == 1 && oflag == 1)
				{
...
/*						FNX_PRINTF("send phone for 600 \n");  */
			...
			case '7' :
#ifdef NUMDEBUG
	FNX_PRINTF("In dogroup case 7xx \n");
	FNX_PRINTF("case7 adn \n");           
#endif
				if (pLts_t->ord == 1 && oflag == 1)
				{
...
/*						FNX_PRINTF("send phone for 700 \n");  */
			...
			case '8' :
#ifdef NUMDEBUG
	FNX_PRINTF("In dogroup case 800 \n");
	FNX_PRINTF("case8 adn \n");  
#endif
				if (pLts_t->ord == 1 && oflag == 1)
				{
...
/*						FNX_PRINTF("send phone for 800 \n");  */
			...
			case '9' :
#ifdef NUMDEBUG
	FNX_PRINTF("In dogroup case 9xx \n");
	FNX_PRINTF("case9 adn \n");
#endif
				if (pLts_t->ord == 1 && oflag == 1)
				{
	...
	FNX_PRINTF("In dogroup xyx y!=0 \n");
		...
		else
		{
			...
			*/
			...
			if (buf[2]=='8' || buf[2]=='1')
			{
				ls_util_send_phone_list(phTTS, ptens_x[buf[1]-'0']);
			}
			else
			{
...
void ls_proc_do_frac(LPTTS_HANDLE_T phTTS, LETTER * llp, LETTER * rlp)
{
	...
	*/
	...
	{
		ls_util_send_phone(phTTS, IT_U);
		ls_util_send_phone(phTTS, IT_N);
	...
	else 
	{
		...
		/* GL 11/05/1996, use IT_S to mark plural. have no idea what "ud" do */
		/* also comment out the sending of "ppercent" */
		if (pflag != FALSE) ls_util_send_phone(phTTS, IT_S);
#if 0
