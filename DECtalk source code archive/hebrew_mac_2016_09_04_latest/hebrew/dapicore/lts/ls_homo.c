	...
	if(try_other)
	{
		...
		if(DICT_FC_ACCESS((*entry_secondary).fc[0]) & FC_HOMOGRAPH)
		{
			...
			for(i=0;(*entry_secondary).text[i];i++)
			{
				...
				{
#ifdef HDEBUG
					FNX_PRINTF("plus and minus failed???");
#endif
					break;
...
/*					if( (homo_table[i].h_select & DICT_FC_ACCESS((*entry_primary).fc)) == 0)
					...
					{
#ifdef HDEBUG
						FNX_PRINTF("changing primary to secondary1. rule i %d\n",i);
						FNX_PRINTF(" %d %d \n",entry_primary,entry_secondary);
#endif
#ifdef NEW_LTS
						pLts_t->word_info[pLts_t->cur_word_index].homograph|=HOMO_SECONDARY;
				...
				if(homo_table[i].h_elim)
				{
					...
					{
#ifdef HDEBUG
						FNX_PRINTF("changing primary to secondary2. rule %d\n",i);
#endif
#ifdef NEW_LTS
						pLts_t->word_info[pLts_t->cur_word_index].homograph|=HOMO_SECONDARY;
#endif
	...
	if (printit && (DT_DBG(LTS_DBG,0x020)))
	{
		...
		if (pKsd_t->dbglog)			/*mfg 04/28/98 added debug support*/
			FNX_FPRINTF((FILE *)pKsd_t->dbglog,"HOMO:(%d)\n",i);
#endif    
		FNX_PRINTF("HOMO:(%d)\n",i);
#endif
	}
...
#ifndef MSDOS
						if (pKsd_t->dbglog)			/*mfg 04/28/98 added debug support*/
							FNX_FPRINTF((FILE *)pKsd_t->dbglog,"HOMO:(H1)\n",i);
#endif    
						FNX_PRINTF("HOMO:(H1)\n");
#endif
					}
...
#ifndef MSDOS
						if (pKsd_t->dbglog)			/*mfg 04/28/98 added debug support*/
							FNX_FPRINTF((FILE *)pKsd_t->dbglog,"HOMO:(H2)\n",i);
#endif    
						FNX_PRINTF("HOMO:(H2)\n",i);
#endif
					}
