...
#if defined (VMS) || defined (LDS_BUILD)
			if (rflag != FALSE)
				FNX_PRINTF("Impossible, shift right\n");
#endif
		}
#if defined (VMS) || defined (LDS_BUILD)
		if (rflag != FALSE)
			FNX_PRINTF("Primary on sylable %d\n", pLts_t->psyl+1);
#endif
		pLts_t->sylp[pLts_t->psyl]->p_stress = pstype;
		csyl = pLts_t->psyl+1;
		isreduced = TRUE;
...
#if defined (VMS) || defined (LDS_BUILD)
	if (rflag != FALSE)
		FNX_PRINTF("Best default on sylable %d\n", pLts_t->psyl+1);
#endif
	pLts_t->sylp[pLts_t->psyl]->p_stress = pstype;
	csyl = pLts_t->psyl+1;
	isreduced = TRUE;
