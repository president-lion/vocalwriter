...
// Prototypes
void ls_rule_to_arpabet(LPTTS_HANDLE_T phTTS, char *fonixbet);

/*FUNCTION_HEADER**********************
 * NAME:	;CorrectPronunciation
 * DESC: 	sPronAll was created with syllable boundaries in place which
			may have affected the pronunciation model.  sPron was created 
			without the syllable boundaries so the pronunciation should be 
			most correct.  Add syllable boundaries into sPron.
 * IN:		
 * OUT:		Correct pronunciation with syllable boundaries
 * RETURN:	
 * NOTES:	
 *END_HEADER***************************/
void CorrectPronunciation(char *sPronAll, char *sPron)
{
	char sTmp[MAX_PHONEMES],
		 sLocalPron[MAX_PHONEMES],
		 *sTok, *sTokAll;

	sTmp[0] = 0x00;
	strcpy(sLocalPron, sPron);
	sTok = strtok(sLocalPron, " ");
	sTokAll = strtok(sPronAll, " ");
	while(sTokAll && sTok)
	{
		while( *sTokAll == '-' )		// Copy syllable breaks from sPronAll
		{
			FNX_STRCAT(sTmp, sTokAll);
			FNX_STRCAT(sTmp, " ");
			sTokAll = strtok(sTokAll+strlen(sTokAll)+1, " ");
		}

		FNX_STRCAT(sTmp, sTok);			// Copy pronunciation from sPron
		FNX_STRCAT(sTmp, " ");
		sTok = strtok(sTok+strlen(sTok)+1, " ");
		sTokAll = strtok(sTokAll+strlen(sTokAll)+1, " ");
	}

	FNX_STRCPY(sPronAll, sTmp);
}

/*FUNCTION_HEADER**********************
 * NAME:	;ls_rule_korean_lts
 * DESC: 	Translate the input word into phonemes and then send to PH
 * IN:		phTTS
 * OUT:		none
 * RETURN:	none
...
void ls_rule_korean_lts(LPTTS_HANDLE_T phTTS)
{
	...
	char	sWord[MAX_PHONEMES],
			sWordAll[MAX_PHONEMES];
	int		nWordLen,
			nWordAll;
	char	sPron[MAX_PHONEMES];
	char	sPronAll[MAX_PHONEMES];
	char	iStop;
		   ...
		   !pKsd_t->halting )
	{
		...
		while( pLts_t->cur_read_pos <= pLts_t->cur_input_pos &&		// Skip leading spaces
			   pLts_t->input_array[pLts_t->cur_read_pos] <= ' ') 
		{
			pLts_t->cur_read_pos++;
		}
		...
		while( pLts_t->cur_read_pos <= pLts_t->cur_input_pos && 
			   !pKsd_t->halting)
		{
			if( pLts_t->input_array[pLts_t->cur_read_pos] != SYLLABLE_MARKER_CHAR )
			{
				// Don't copy syllable boundary markers.  We don't want to confuse the LTS system
				sWord[nWordLen++] = pLts_t->input_array[pLts_t->cur_read_pos];
			}
			sWordAll[nWordAll++] = pLts_t->input_array[pLts_t->cur_read_pos];
			pLts_t->cur_read_pos++;

			iStop = 0;

			switch( pLts_t->input_array[pLts_t->cur_read_pos] )
			{
				case ' ':
				case '\0':
				case '\n':
		...
		if( sWord[0] )
		{
			if (pKsd_t->halting) return;

			// Apply letter to sound rules
			if( translate(pLts_t->pDccInfo->pGraph2Phon, sWord, sPron) )	// Get pronunciation without syllable boundaries
			{
			//	SelectSinglePron(sPron,0);						// Eliminate optional and alternate pronunciations
				if( translate(pLts_t->pDccInfo->pGraph2Phon, sWordAll, sPronAll) )	// Get pron. with syllable boundaries
				{
			//		SelectSinglePron(sPronAll,0);				// Eliminate optional and alternate pronunciations
					CorrectPronunciation(sPronAll, sPron);		// Make sure we have the correct phonemes and syllable breaks.	
				}
				...
				{
					FNX_PRINTF("Korean LTS Input: %s Output: %s\n", sWord, sPronAll);
				}
				else if(DT_DBG(LTS_DBG,0x002))
				{
					FNX_STRCPY(sPronAll, sPron);
					FNX_PRINTF("Korean LTS Input: %s Output: %s\n", sWord, sPronAll);
				}
#endif

				ls_rule_to_arpabet(phTTS,sPronAll);				//send the phonemes to PH
...
const char *const fonixbet_strings[]= {
	"",
	"i",
	"E",
	"@",
...
void ls_rule_to_arpabet(LPTTS_HANDLE_T phTTS, char *fonixbet)
{
	...
	while (!pKsd_t->halting && fonixbet[pos])
	{
		while (fonixbet[pos]==' ') 
		{
			pos++;
		...
		{
			length=strlen(fonixbet_strings[i]);
			match=strncmp(fonixbet+pos,fonixbet_strings[i],length);
			if (match==0)
			{
		...
		if (best_length<=0)
		{
			while (fonixbet[pos] && fonixbet[pos]!=' ') 
			{
				pos++;
			}
		}
		...
		{
			pos+=best_length;
			ls_util_send_phone(phTTS,best_match);
		}
	}
}
...
//				gp1->g_ip    = NULL;
			...
			//	FNX_PRINTF("%5d ",rline);
				...
				if (xflag == TRUE)
					FNX_PRINTF("%s",get_rule_data(rline));
				else
				    FNX_PUTCHAR('\n');
			}
#endif
