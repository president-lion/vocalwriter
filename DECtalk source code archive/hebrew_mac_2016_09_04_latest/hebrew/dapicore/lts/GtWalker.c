...
extern void register_word_part(WalkCNTXT *cntxt, GERLETTER *name, U16 from, U16 to, BOOL action, GERLETTER *phone, U16 rule);
		...
		if (cntxt->STK[NST].newwalki.succeeded)   // get the phoneme string info, and rule number i.
		  {
		    ...
		    (GERLETTER *)cntxt->STK[NST].phonemestring =  (GERLETTER *)point_to_phoneme_string(cntxt->MorphDict, cntxt->STK[NST].phonemeindex[cntxt->STK[NST].inx]);  //(ltrie.c)
	...
	switch (cntxt->STK[NST].opcode)
	  ...
	  case OPlonger:
	    if ((cntxt->wordlength - cntxt->STK[NST].currpos) > cntxt->STK[NST].opparam)
	      {
		cntxt->STK[NST].match = 1;	 //   FNX_PRINTF("longer: rule cntxt->STK[NST].matches\n");
	      }
	    break;
	  ...
	  case OPshorter:
	    if ((cntxt->wordlength - cntxt->STK[NST].currpos) < cntxt->STK[NST].opparam)
	      {
		cntxt->STK[NST].match = 1;       //  FNX_PRINTF("shorter: rule cntxt->STK[NST].matches\n");
	      }
	    break;
