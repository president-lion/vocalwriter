 ...
 *	067	MGS		12/24/2004	copy the resulting set match to $8 for later use
 *	068	MGS		01/18/2005	Fixed _stricmp for ARM7
 */
...
#endif
{
	FNX_STRCPY(output_array+ret_value->output_pos,"I am a pud-fart. ");
	ret_value->output_offset=FNX_STRLEN("I am a pud-fart. ");
	return;
}
...
const void (*CONSTCONST(perform_action_funcs[0x20]))(unsigned char *current_rule,
									 unsigned char *input_array,
									 unsigned char *output_array,
									 pindex_data_t input_indexes,
									 pindex_data_t output_indexes,
...
int main(void)
{
...
#ifdef TIMING
	start=clock();
	FNX_FPRINTF(stderr,"start clock=%d\n",start);
#endif
	...
	FNX_FPRINTF(stderr,"start clock=%d\n",start);
	FNX_FPRINTF(stderr,"end   clock=%d\n",end);
	FNX_FPRINTF(stderr,"difference =%d\n",end-start);
	FNX_FPRINTF(stderr,"seconds =%6.3f\n",(double)(end-start) / CLOCKS_PER_SEC);
#endif
#ifdef PARSER_DEBUG
...
//			i=0;
...
#ifdef PARSER_DEBUG
			FNX_PRINTF("the language flag is 0x%08X\n",*rule_modes);
#endif
...
#ifdef PARSER_DEBUG
			FNX_PRINTF("the mode flag is 0x%08X\n",rule_modes[1]);
#endif
			/*	044	MGS		09/24/1997	BATS#469 Fix for NWS parser problem */
			if (last_rule_was_hit==0)
			{
			...
			if ((current_value & (BIN_DICT_HIT | BIN_DICT_MISS)))
			{
				...
				{
#ifdef PARSER_DEBUG
					FNX_PRINTF("processing rule because of dictionary ABBREV on the word\n");
#endif			
				}
				else
				{	
	...
	* should remain unchanged for the following rules to use 
	...
	if (new_ret.value!=OPT_FAIL)
	{
		...
		if (new_ret.value==FATAL_FAIL)
		{
			ret_value->value=FATAL_FAIL;
#ifdef PARSER_DEBUG
			FNX_PRINTF("leaving par_match_rule fatal failure of the rule\n");
#endif
			return;
		}
		if (new_ret.value==FAIL)
					   ...
					   int insert_operation_flags)
{
...
#ifdef PARSER_DEBUG
	FNX_PRINTF("entering par_compound_break\n");
#endif

	if (noun_num_character_in_mapping==0)
	{
#ifdef PARSER_DEBUG
		FNX_PRINTF("leaving par_compound_break, no table loaded\n");
#endif
		return;
	}
	...
	else
	{
		...
		{
#ifdef PARSER_DEBUG
			FNX_PRINTF("set optional to opt_fail\n");
#endif
			ret_value->value=OPT_FAIL;
		}
	}
...
#ifdef PARSER_DEBUG
	FNX_PRINTF("leaving par_compound_break\n");
#endif
...
#ifdef PARSER_DEBUG
	FNX_PRINTF("entering par_break_down_word %s\n",input);
#endif
	result=par_find_word_in_dict(head,input,positions,0,&number_of_pos);
	if (result==0)
	{
#ifdef PARSER_DEBUG
		FNX_PRINTF("leaving par_break_down_word no matches\n");
#endif
		return(-1);
	}
			...
			for (j=0;j<noun_number_of_conjunctions;j++)
			{
				if (noun_conjunction[j].length==0)
				{
					...
					if (result>0)
					{
						output[positions[i]]='-';
#ifdef PARSER_DEBUG
						FNX_PRINTF("leaving par_break_down_word no conj %s %s %d\n",input,output,result+positions[i]+1);
#endif
...
#ifdef PARSER_DEBUG
							FNX_PRINTF("leaving par_break_down_word conj %s %s %d\n",input,output,result+positions[i]+1);
#endif
...
#ifdef PARSER_DEBUG
	FNX_PRINTF("leaving par_break_down_word failure %s %s\n",input,output);
#endif
	return(-1);
...
#endif
{
	...
	if (ret_value->input_offset>=PAR_MAX_MATCH_ARRAY)
	{
		ret_value->value=FATAL_FAIL;
#ifdef PARSER_DEBUG
		FNX_PRINTF("leaving par_dom_dict_search fatal failure of the rule\n");
#endif
		return;
	}
	if (ret_value->output_offset>=PAR_MAX_MATCH_ARRAY)
	...
	if (result==1)
	{
		match_array->array_lengths[9]=FNX_STRLEN(match_array->array[9]);
						 ...
						 int dict_num, int dict_state_flag)
{
	...
	while ((for_same<=high) && (FNX_STRICMP(input,(unsigned char *)(dict_data_table+dict_index_table[for_same]))==0))
	{
		for_same++;
	}                  
	rev_same++;
...
//		FNX_STRCPY(output,((unsigned char *)(dict_data_table+dict_index_table[save_for]))+(FNX_STRLEN((unsigned char *)(dict_data_table+dict_index_table[save_for]))+1));
		FNX_STRCPY(output,((unsigned char *)(dict_data_table+dict_index_table[save_for]))+(input_length+1));
#ifdef PARSER_DEBUG
					...
					preturn_value_t ret_value)
{
	...
	if (char_type==BIN_EXACT)
	{
		...
		if (current_rule[find_index] & BIN_CASE_INSEN)
		{
			...
			{
#ifdef PARSER_DEBUG
				FNX_PRINTF("leaving par_look_ahead; exact failure\n");
#endif
				return(0);
			}
			while (ipos<value)
...
#ifdef PARSER_DEBUG
			FNX_PRINTF("leaving par_look_ahead; exact success\n");
#endif
			return(1);
		}
			...
			else /* if (char_type==BIN_HEXADECIMAL) */
			{
				...
				else if (char_type==BIN_SETS)
				{
					...
					if (new_ret.value==SUCCESS)
					{
						...
						{
#ifdef PARSER_DEBUG
							FNX_PRINTF("leaving par_look_ahead;sets success\n");
#endif
							return(1);
						}
					}
#ifdef PARSER_DEBUG
					FNX_PRINTF("leaving par_look_ahead;sets fail\n");
#endif
					return(0);
				}
...
//	int find_type= -2;
	...
	if (temp & BIN_LARGE_DESC)
	{
		large_desc=1;
#ifdef PARSER_DEBUG
		FNX_PRINTF("match_standard using large descriptors\n");
#endif
	}
							   ...
							   prange_value_t range_value, int lookahead, int break_on_min_match)
{
	...
	ret_value->rule=rule_p;
	/* copy the resulting set match to $8 for later use */
	if (total_length>=1 && total_length<PAR_MAX_MATCH_ARRAY)
	{
		memcpy(match_array->array[8],(input_array+(ret_value->input_pos)),total_length);
		match_array->array_lengths[8]=total_length;
		/* null terminate the string */
		match_array->array[8][total_length]='\0';
	}
