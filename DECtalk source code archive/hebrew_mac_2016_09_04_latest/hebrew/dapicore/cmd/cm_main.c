...
#ifdef WIN32 
	if((pCmd_t= (PCMD_T) FNX_MALLOC(sizeof(CMD_T))) == NULL)
	{
			return(MMSYSERR_NOMEM);
	}        
	/***************Thread specific structure initialization MVP ***************/
	if((pCmd_t->cm = (short *)FNX_MALLOC(sizeof(int)*total_commands))== NULL)
	{
			return(MMSYSERR_NOMEM);
    } 
#ifdef ESCAPE_SEQ
    if((pCmd_t->esc_seq = (INPUT_SEQ *)FNX_MALLOC(sizeof(INPUT_SEQ)))== NULL)
	{
			return(MMSYSERR_NOMEM);
    } 
#endif
...
/* GL 04/21/1997  add this for OSF build */
#if defined _UNIX_LIKE_
	if((pCmd_t= (PCMD_T) FNX_CALLOC(1,sizeof(CMD_T))) == NULL)
	        phTTS->uiThreadError = MMSYSERR_NOMEM;
        else
		/***************Thread specific structure initialization MVP ***************/
			if((pCmd_t->cm = (int*)FNX_CALLOC(total_commands,sizeof(int) ))== NULL)
				phTTS->uiThreadError = MMSYSERR_NOMEM;
#ifdef ESCAPE_SEQ
			else
				if((pCmd_t->esc_seq = (INPUT_SEQ *) FNX_CALLOC(1,sizeof(INPUT_SEQ)))== NULL)
					phTTS->uiThreadError = MMSYSERR_NOMEM;
#endif
#endif
#ifdef ARM7
	...
	pCmd_t = &Cmd_t; 
#else
	if((pCmd_t = GetGlobalDTPtr()->Cmd_t) == NULL)
	{
		pCmd_t = FNX_MALLOC(sizeof(CMD_T));
		GetGlobalDTPtr()->Cmd_t = pCmd_t;
	}
#endif
#endif
