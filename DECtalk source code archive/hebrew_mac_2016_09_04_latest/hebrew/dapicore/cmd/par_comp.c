...
{
  rule_node p;
  p=(rule_node)FNX_MALLOC(sizeof(struct rule_node_s));
  if (p==NULL)
	{
	  FNX_FPRINTF(stderr,"add_node;out of memory\n");
	  free_rule_list(list);
	  return(NULL);
	}
  p->data=info;
...
{
	dict_node p;
	p=(dict_node)FNX_MALLOC(sizeof(struct dict_entry_s_comp));
    if (p==NULL)
	{
	  FNX_FPRINTF(stderr,"add_entry_to_dict;out of memory\n");
	  return(0);
	}
	p->data=entry;
...
{
	dict_names p;
	p=(dict_names)FNX_MALLOC(sizeof(struct dict_names_s));
	if (p==NULL)
	{
	  FNX_FPRINTF(stderr,"add_dictionary_to_list;out of memory\n");
	  free_dict_names_list(dict_list);
	  return(NULL);
	}
	FNX_STRCPY(p->dict_name,dict_name);         
...
{
	return(FNX_STRICMP(p->data->search_graph,q->data->search_graph));
}
...
{
	return(FNX_STRCMP(p->data->search_graph,q->data->search_graph));
}
...
prule_comp_t convert_rule_line(unsigned char *line)
{
  ...
  rule=(prule_comp_t)FNX_MALLOC(sizeof(rule_comp_t));
  if (rule==NULL)
	{
	  FNX_FPRINTF(stderr,"out of memory in convert line on line number %d\n",line_number);
	  return(NULL);
	}
  ...
  if (line[index]!=HEXADECIMAL_DELIM)
	{
	  if (FNX_STRNCMP(line+index,"STOP",4)==0)
		{
		  rule->special_rule=STOP_TAG_VALUE;
		}
	  else if (FNX_STRNCMP(line+index,"RETURN",6)==0)
		{
		  rule->special_rule=RETURN_TAG_VALUE;
		}
	  else if (FNX_STRNCMP(line+index,"GOTO",4)==0)
		{
		  index+=4;
		  rule->special_rule=GOTO_TAG_VALUE;
		  rule->special_value=FNX_ATOI(line+index);
		}
	  else if (FNX_STRNCMP(line+index,"GORET",5)==0)
		{
		  index+=5;
		...
		{
		  FNX_FPRINTF(stderr,"unrecognized tag at the beginning of line number %d\n",line_number);
	  ...
	  if (line[index]!=RULE_NUMBER_DELIM)
		{
		  FNX_FPRINTF(stderr,"no rule number given for special rule on line %d\n",line_number);
		  par_print_rule_error(line,index);
		  FNX_FREE(rule);
		  return(NULL);
		}
	  index++;
	  rule->rule_number=FNX_ATOI(line+index);
  ...
  else
	{
	  ...
	  if (temp==-1)
		{
		  FNX_FPRINTF(stderr,"error in hex number in line number %d at position %d\n",line_number,index);
		  par_print_rule_error(line,index);
		  FNX_FREE(rule);
		  return(NULL);
		}
	  ...
	  if (line[index]!='-')
		{
		  FNX_PRINTF("- not found at position %d in line number %d\n",index,line_number);
		  par_print_rule_error(line,index);
		  FNX_FREE(rule);
		  return(NULL);
		}
	  ...
	  while (line[index]!=',')
		{
		  switch(line[index])
			...
			case HIT_NUMBER_DELIM:
			  index++;
			  rule->next_hit_rule=FNX_ATOI(line+index);
			  index+=par_get_int_length(rule->next_hit_rule);
			  break;
			case MISS_NUMBER_DELIM:
			  index++;
			  rule->next_miss_rule=FNX_ATOI(line+index);
			...
			case GORET_NUMBER_DELIM:
			  ...
			  if (line[index]==GORET_HIT_DELIM)
				{
				  index++;
				  rule->next_goret_hit=FNX_ATOI(line+index);
				  index+=par_get_int_length(rule->next_goret_hit);
				}
			  else if (line[index]==GORET_MISS_DELIM)
				{
				  index++;
				  rule->next_goret_miss=FNX_ATOI(line+index);
				  index+=par_get_int_length(rule->next_goret_miss);
				}
			  else
				{
				  FNX_FPRINTF(stderr,"unrecognized goret delimiter at position %d in line %d\n",index,line_number);
				  par_print_rule_error(line,index);
				  FNX_FREE(rule);
				  return(NULL);
				}
			...
			case DICTIONARY_DELIM:
			  ...
			  if (line[index]==DICTIONARY_HIT)
				...
				{
				  FNX_FPRINTF(stderr,"unrecognized dictionary tag at position %d in line %d\n",index,line_number);
				  par_print_rule_error(line,index);
				  FNX_FREE(rule);
				  return(NULL);
				}   
			  ...
			  break;
			default:
			  FNX_FPRINTF(stderr,"unrecognized delimiter at position %d in line %d\n",index,line_number);
			  par_print_rule_error(line,index);
			  FNX_FREE(rule);
			  return(NULL);
			}
	  ...
	  index++;
	  FNX_STRCPY(rule->rule,line+index);
	  index=FNX_STRLEN(rule->rule)-1;
	  ...
	  if (rule->rule_number==-1)
		{
		  FNX_FPRINTF(stderr,"no rule number found in line number %d\n",line_number);
		  par_print_rule_error(line,index);
		  FNX_FREE(rule);
		  return(NULL);
		}
...
#ifdef DEBUG
FNX_PRINTF("entering match_rule\n");
par_print_rule_error(rule,(*index));
	...
	if (state==SAVE_STATE)
	{
		...
		{
			FNX_PRINTF("invalid save state found in rule R%d\n",current_rule_number);
			par_print_rule_error(rule,(*index));
			exit(1);      
		}
		end=End_Is_Paren;
	...
	if (state==DICTIONARY_STATE)
	{
		...
		{
			FNX_PRINTF("unexpected end of rule R%d\n",current_rule_number);
			par_print_rule_error(rule,keep_index);
			exit(1);      
		}
		p=dict_list;
		while (p!=NULL && FNX_STRICMP(dict_name,p->dict_name))
		{
			p=p->next;
		}
		if (p==NULL)
		{
			FNX_PRINTF("dictionary name %s not found from rule R%d\n",dict_name,current_rule_number);
			par_print_rule_error(rule,(*index));
			exit(1);      
		}
		new_num=p->dic_number;
		save_len=FNX_STRLEN(dict_name);
		...
		if (new_len>save_len)
		{
			...
			for (i=FNX_STRLEN(rule);i>((*index)+save_len-1);i--)
			{
				rule[i+diff]=rule[i];
			}
		}
		else
		{
			if (new_len<save_len)
			{
				...
				for (i=(*index)+diff+1;i<=FNX_STRLEN(rule);i++)
				{
					rule[i-diff]=rule[i];
				}
			}
		}
		FNX_SPRINTF(temps,"%d",new_num);
		FNX_STRNCPY(rule+(*index),temps,FNX_STRLEN(temps));
		(*index)+=new_len;
	...
	if (state==MACRO_STATE)
	{
		...
		{
			FNX_PRINTF("no rule delimiter found in a macro state in rule R%d\n",current_rule_number);
			par_print_rule_error(rule,(*index));
			exit(0);
		}
		(*index)++;/* skip the R */
		save_num=FNX_ATOI(rule+(*index));
		new_num=rule_indexes[save_num];
		if (new_num==-1)
		{
			FNX_PRINTF("no such rule number R%d in rule R%d\n",save_num,current_rule_number);
			par_print_rule_error(rule,(*index));
			exit(0);
		}
		...
		while (rule[(*index)]!=end)
		{
			if ((new_char=par_get_char_type(rule[(*index)]))!=NULL_TYPE)
			{
				if (new_char==NO_LOOKAHEAD)
				{
					...
					{
						FNX_PRINTF("no character type found in rule R%d\n",current_rule_number);
						par_print_rule_error(rule,(*index));
						exit(1);
					}
			...
			else
			{
				if ((new_state=par_get_state(rule[(*index)]))!=NULL_STATE)
				{
					...
					if (new_state!=SAVE_STATE)
					{
						...
						{
							FNX_PRINTF("no state delimiter found in rule R%d\n",current_rule_number);
							par_print_rule_error(rule,(*index));
							exit(0);
						}
				...
				{
					FNX_PRINTF("no action state or character type found in rule R%d\n",current_rule_number);
					par_print_rule_error(rule,(*index));
					exit(1);
				}
			}
	...
	if (rule[(*index)]!=end)
	{
		FNX_PRINTF("end of state slash not found in rule R%d\n",current_rule_number);
		par_print_rule_error(rule,(*index));
		exit(0);
	}
	if (state!=NULL_STATE)
...
{
#ifdef DEBUG
FNX_PRINTF("entering perform_action\n");
par_print_rule_error(rule,(*index));
#endif
	switch(state)
	{
...
#ifdef DEBUG
			FNX_PRINTF("in dict_state perform_action\n");
			par_print_rule_error(rule,(*index));
#endif
			if (FNX_STRNCMP(rule+(*index),"FAIL",4)==0)
			{
				if (rule[(*index)+4]!='|')
				{
					FNX_PRINTF("no vertical bar found in dictioanry_state\n");
...
#ifdef DEBUG
			FNX_PRINTF("in dict_state perform_action 2\n");
			par_print_rule_error(rule,(*index));
#endif
			if (FNX_STRNCMP(rule+(*index),"FAIL",4)==0)
			{
				if (rule[(*index)+4]!='/')
				{
					FNX_PRINTF("no slash found in dictioanry_state\n");
					par_print_rule_error(rule,(*index)+4);
					exit(1);
				}
				(*index)+=5;
...
{
#ifdef DEBUG
FNX_PRINTF("entering match_string\n");
par_print_rule_error(rule,(*index));
#endif
	switch(char_type)
	{
		...
		case	HEXADECIMAL_TYPE:
			...
			{
				FNX_PRINTF("error in hexadecimal type in rule R%d\n",current_rule_number);
				par_print_rule_error(rule,(*index));
			}
			(*index)+=4;
			break;
        ...
        case	SOME_PUNCT_TYPE:
			...
			break;
		default:
			FNX_PRINTF("character type specifier not found in rule R%d\n",current_rule_number);
			par_print_rule_error(rule,(*index));
			exit(-1);
			break;
	}
...
void match_standard(char *rule,int *index)
{
	...
	if (rule[(*index)]!='<')
	{
		FNX_PRINTF("no < found at the beginning of the character type in rule R%d\n",current_rule_number);
		par_print_rule_error(rule,(*index));
		exit(1);
	}           
	(*index)++;
	while (rule[(*index)]!='>')
	{
		switch(rule[(*index)])
		{
			case '*':
				...
				if (rule[(*index)]!='>')
				{
					FNX_PRINTF("> not found after * in rule R%d\n",current_rule_number);
					par_print_rule_error(rule,(*index));
					exit(1);
				}
				break;
			...
			case ',':
				...
				break;
			default:
				(*index)+=par_get_int_length(FNX_ATOI(rule+(*index)));
				switch(rule[(*index)])
				{
					case ',':
						(*index)++;
					...
					case '-':
						...
						switch (rule[(*index)])
						{
							...
							case '+':
								...
								if (rule[(*index)]!='>')
								{
									FNX_PRINTF("> not found after * or + in rule R%d\n",current_rule_number);
									par_print_rule_error(rule,(*index)); 
									exit(1);
								}
								break;
							default:
								...
								{
									FNX_PRINTF("expected > or , after range in rule R%d\n",current_rule_number);
									par_print_rule_error(rule,(*index));
									exit(1);
								}
						}
						break;
					default:
						FNX_PRINTF("error in rule syntax in rule R%d\n",current_rule_number);
						par_print_rule_error(rule,(*index));
						exit(1);
				}               
		}
...
void match_digit(char *rule,int *index)
{
	...
	if (rule[(*index)]!='[')
	{
		FNX_PRINTF("no [ found at the beginning of the character type in rule R%d",current_rule_number);
		par_print_rule_error(rule,(*index));
		exit(1);
	}           
	(*index)++;
...
void match_set(char *rule,int *index)
{
	...
	if (rule[(*index)]!='{')
	{
		FNX_PRINTF("no { found at the beginning of the set in rule R%d\n",current_rule_number);
		par_print_rule_error(rule,(*index));
		exit(0);
	}   
	(*index)++;                
	...
	if (rule[(*index)]!='<')
	{
		FNX_PRINTF("no < found at the befinning of the character type in rule R%d",current_rule_number);
		par_print_rule_error(rule,(*index));
		exit(1);
	}           
	(*index)++;
...
dict_entry_comp convert_dict_line(unsigned char *line)
{
	...
	int i,j;
	entry=(dict_entry_comp)FNX_MALLOC(sizeof(struct dict_entry_s_comp));
	if (entry==NULL)
	{
		FNX_PRINTF("out of memory in convert_dict_line\n");
		return(NULL);
	}
	...
	{
		FNX_PRINTF("no dictionary delimiter found on line %d\n",line_number);
...
int find_longest_from(dict_node dict)
{
	...
	while (dict!=NULL)
	{
		temp=FNX_STRLEN(dict->data->search_graph);
		if (temp>length)
			length=temp;
		dict=dict->next;
	}
...
int find_longest_to(dict_node dict)
{
	...
	while (dict!=NULL)
	{
		temp=FNX_STRLEN(dict->data->out_graph);
		if (temp>length)
			length=temp;
		dict=dict->next;
	}
...
int fill_binary_rule_table(rule_node list)
{
  ...
  rule_offsets=(short *)FNX_MALLOC((rule_count+1)*sizeof(short)+16);
  if (rule_offsets==NULL)
	{
	  FNX_FPRINTF(stderr,"could not allocate rule offsets\n");
	  return(0);
	}
  p=list;
  while (p!=NULL)
	{
	  ...
	  total_rule_length=total_rule_length+((((FNX_STRLEN(p->data->rule)+1+8*sizeof(S16)+2*sizeof(S32))+3)>>2)<<2);
	  p=p->next;
	}
  rule_binary_data=(unsigned char *)FNX_MALLOC(total_rule_length*sizeof(unsigned char )+16);
  if (rule_binary_data==NULL)
	{
	  FNX_FPRINTF(stderr,"could not allocate rule data array\n");
	  return(0);
	}
  p=list;
  while (p!=NULL)
	{
	  ...
	  rule_length=FNX_STRLEN(p->data->rule)+10*sizeof(short)+1;
	  ...
	  FNX_STRCPY(rule_binary_data+rule_index,p->data->rule);
	  rule_index+=FNX_STRLEN(p->data->rule);
...
int fill_binary_dictionary_table(dict_names dict_list)
{
  ...
  {
	dictionary_offsets=(short *)FNX_MALLOC(sizeof(short));
	if (dictionary_offsets==NULL)
	{
		FNX_FPRINTF(stderr,"could not allocate dictionary_offsets\n");
		return(0);
	}
	dictionary_offsets[0]=0x0000;
	dictionary_binary_data=(unsigned char *)FNX_MALLOC(sizeof(unsigned char) *4);
	if (dictionary_binary_data==NULL)
	{
		FNX_FPRINTF(stderr,"could not allocate dictionary_binary_data\n");
		return(0);
	}
  ...
  else
  {
	while (p!=NULL)
	{
		...
		while (q!=NULL)
		{
			from_length=FNX_STRLEN(q->data->search_graph)+1;
			to_length=FNX_STRLEN(q->data->out_graph)+1;
	...
	dictionary_offsets=(short *)FNX_MALLOC((dict_entry_count)*sizeof(short));
	if (dictionary_offsets==NULL)
	{
		FNX_FPRINTF(stderr,"could not allocate dictionary_offsets\n");
		return(0);
	}
	dictionary_binary_data=(unsigned char *)FNX_MALLOC(sizeof(unsigned char)*total_dict_length);
	if (dictionary_binary_data==NULL)
	{
		FNX_FPRINTF(stderr,"could not allocate dictionary_binary_data\n");
		return(0);
	...
	while (p!=NULL)
	{
		...
		while (q!=NULL)
		{
			...
			FNX_STRCPY(dictionary_binary_data+dict_index,q->data->search_graph);
			dict_index+=FNX_STRLEN(q->data->search_graph);
			/* insert the null */
			dictionary_binary_data[dict_index++]=0x00;
			/* copy the out_graph to the array */
			FNX_STRCPY(dictionary_binary_data+dict_index,q->data->out_graph);
			dict_index+=FNX_STRLEN(q->data->out_graph);
			/* insert the null */
			dictionary_binary_data[dict_index++]=0x00;
			q=q->next;
		}
...
void write_output(rule_node list,dict_names dict_list,int *rule_sections,char*infile_name,char *outfile_name)
{
	...
	FNX_FPRINTF(outfile_asc,"/* source file %s */\n",infile_name);
	FNX_FPRINTF(outfile_asc,"/* output file %s */\n",outfile_name);
	FNX_STRCPY(char_time,ctime(&ltime));
	char_time[FNX_STRLEN(char_time)-1]='\0';
	FNX_FPRINTF(outfile_asc,"/* file created on %s */\n",char_time);
	FNX_FPRINTF(outfile_asc,"\n");
	FNX_FPRINTF(outfile_asc,"S16 num_rule_sections=%d;\n\n",rule_sections[0]);
	FNX_FPRINTF(outfile_asc,"S16 rule_sections[%d]={",rule_sections[0]);
	for (i=1;i<=rule_sections[0];i++)
	  if (i==rule_sections[0])
		FNX_FPRINTF(outfile_asc,"\t%d",rule_sections[i]);
	  else
		FNX_FPRINTF(outfile_asc,"\t%d,",rule_sections[i]);
	FNX_FPRINTF(outfile_asc,"\t};\n\n");
	FNX_FPRINTF(outfile_asc,"S16 num_rules=%d;\n\n",rule_count+1);

	FNX_FPRINTF(outfile_asc,"S16 rule_index_table[%d] = {\n",rule_count+1);
	for (i=0;i<rule_count+1;i++)
	  {
		if (i==rule_count)
		{
			FNX_FPRINTF(outfile_asc,"\t0x%04X",rule_offsets[i]);
		}
		else
		{
			if (i!=0 && (i+1)%6==0)
			{
				FNX_FPRINTF(outfile_asc,"\t0x%04X,\n",rule_offsets[i]);
			}
			else
			{
				FNX_FPRINTF(outfile_asc,"\t0x%04X,",rule_offsets[i]);
	...
	FNX_FPRINTF(outfile_asc,"\n};\n\n");
	FNX_FPRINTF(outfile_asc,"/*\n");
	FNX_FPRINTF(outfile_asc," * in this file, the rule in its old form is printed before the\n");
	FNX_FPRINTF(outfile_asc," * the rule in its new form\n");
	FNX_FPRINTF(outfile_asc," */\n\n");
	FNX_FPRINTF(outfile_asc,"unsigned char rule_data_table[%d] = {\n",total_rule_length);
	r=list;
	for (i=0,j=0,k=0;i<total_rule_length;i++,k++)
	{
		if ((i==rule_offsets[j]))
		{
			...
			FNX_FPRINTF(outfile_asc,"//{\t%d,\t%d,\t0x%08lX,\t0x%08lX,\t%5d,\t%2d,\t%2d,\t%2d,\t%2d,\t%2d,\t\"%s\"\t}\n",
		...
		{
			FNX_FPRINTF(outfile_asc,"  0x%02X",rule_binary_data[i]);
		}
		else
		{
			if (k!=0 && (k+1)%10==0)
			{
				FNX_FPRINTF(outfile_asc,"  0x%02X,\n",rule_binary_data[i]);
			}
			else
			{
				FNX_FPRINTF(outfile_asc,"  0x%02X,",rule_binary_data[i]);
	...
	FNX_FPRINTF(outfile_asc,"\n};\n\n");
	list=r;
	FNX_FFLUSH(outfile_asc);

	FNX_FPRINTF(outfile_asc,"/* this is table of indexes in the dict_index_table of */\n");
	FNX_FPRINTF(outfile_asc,"/* start, end, and the size each dictionary */\n");
	p=dict_list;
	i=0;
	total=0;
	longest_from=0;
	...
	if (num_dictionaries>0)
	  {
		s=(dict_pointers_t *)FNX_MALLOC(sizeof(struct dict_pointers_s)*num_dictionaries);
		if (s==NULL)
		  {
			FNX_PRINTF("not enough memory to allocate the dictionary pointers\n");
			return;
		  }
		while (p!=NULL)
		  {
	...
	if (num_dictionaries==0)
	  {
		FNX_FPRINTF(outfile_asc,"dict_pointers_t dict_point[1]={{0,0,0}};\n\n");
	  }
	else
	  {
		FNX_FPRINTF(outfile_asc,"dict_pointers_t dict_point[%d]={\n",i);
		for (j=0;j<i;j++)
		  {
			if (j+1==i)
			  FNX_FPRINTF(outfile_asc,"{\t%5d,\t%5d,\t%5d\t}};\n\n",s[j].start,s[j].end,s[j].num_entries);
			else
			  FNX_FPRINTF(outfile_asc,"{\t%5d,\t%5d,\t%5d\t},\n",s[j].start,s[j].end,s[j].num_entries);
		  }
	  }
	...
	if (num_dictionaries==0)
	{
	  FNX_FPRINTF(outfile_asc,"S16 dict_index_table[%d] = {\n",1);
		FNX_FPRINTF(outfile_asc,"\t0x0000");
	}
	else
	{
	  FNX_FPRINTF(outfile_asc,"S16 dict_index_table[%d] = {\n",dict_entry_count);
		for (i=0;i<dict_entry_count;i++)
		{
			...
			{
				FNX_FPRINTF(outfile_asc,"\t0x%04X",dictionary_offsets[i]);
			}
			else
			{
				if (i!=0 && (i+1)%6==0)
	...
	if (num_dictionaries==0)
	{
	    FNX_FPRINTF(outfile_asc,"unsigned char dict_data_table[%d] = {",2);
		FNX_FPRINTF(outfile_asc,"\n//{\t\"\",\"\"}\n");
		FNX_FPRINTF(outfile_asc,"  0x00,  0x00");
	}
	else
	{
	    FNX_FPRINTF(outfile_asc,"unsigned char dict_data_table[%d] = {",total_dict_length);
		p=dict_list;
		q=p->dict_entries;
		for (i=0,j=0,k=0;i<total_dict_length;i++,k++)
		  {
			...
			if (i==dictionary_offsets[j])
			  {
				if (q==NULL)
				...
				else
				  {
					...
					for (l=0;l<(longest_from-FNX_STRLEN(q->data->search_graph));l++)
					  {
						FNX_FPRINTF(outfile_asc," ");
					  }
					FNX_FPRINTF(outfile_asc,",\t\"%s\"",q->data->out_graph);
					for (l=0;l<(longest_to-FNX_STRLEN(q->data->out_graph));l++)
					  {
						FNX_FPRINTF(outfile_asc," ");
					  }
					FNX_FPRINTF(outfile_asc,"\t}\n");		  
			...
			if (i==total_dict_length-1)
			  {
				FNX_FPRINTF(outfile_asc,"  0x%02X",dictionary_binary_data[i]);
			  }
			else
			  {
				if (k!=0 && (k+1)%10==0)
...
void write_binary(rule_node list,dict_names dict_list,int *rule_sections,char*infile_name,char *outfile_name)
{
	...
	FNX_FWRITE(rule_binary_data,1,total_rule_length,outfile);
	...
	else
	{
		...
		FNX_FWRITE(&temp,2,1,outfile);
		FNX_FWRITE(dictionary_binary_data,1,total_dict_length,outfile);
	}
}
...
void add_define(char defines[50][50],char *sym)
{
	...
	if (i>49)
	{
		FNX_PRINTF("too many defines\n");
		exit(3);
	}
	FNX_STRNCPY(defines[i],sym,49);
}
...
int compare_to_defines(char *line,char defines[50][50])
{
	int i=0,j=0;
	char *temp;
	temp=FNX_STRTOK(line," \t\n\r");
	while (defines[i][0]!='\0')
	{
		if (FNX_STRCMP(defines[i],temp)==0)
		{
			return(1);
		}
		i++;
...
int main(int argc,char *argv[])
{
  ...
  if (argc<2)
	{
	  FNX_PRINTF("usage; par_comp input_file_name <output_file_name> /D symbol\n");
	  FNX_PRINTF("                output_file is optional\n");
	  exit(1);
	}
  if ((infile=FNX_FOPEN(argv[1],"r"))==NULL)
	{
	  FNX_PRINTF("could not open the input file %s\n",argv[1]);
	  exit(2);
	}
  j=2;
  while (j<argc)
  {
	  ...
	  {
		  FNX_STRCPY(output_file_temp,argv[j]);
	  }
	  j++;
  }

  if (FNX_STRLEN(output_file_temp)>0)
	{	
	  if (FNX_STRCMP(FNX_STRRCHR(output_file_temp,'.'),".h")==0)
	  {
		  FNX_STRCPY(output_file,output_file_temp);
		  FNX_STRCPY(FNX_STRRCHR(output_file,'.'),".bin");
		  FNX_STRCPY(output_file_asc,output_file_temp);
	  }
	  else
	  {
		  FNX_STRCPY(output_file_asc,output_file_temp);
		  FNX_STRCPY(FNX_STRRCHR(output_file_asc,'.'),".h");
		  FNX_STRCPY(output_file,output_file_temp);
	  }
	  if ((outfile=FNX_FOPEN(output_file,"wb"))==NULL)
		{
		  FNX_PRINTF("could not open the output file %s\n",argv[2]);
		  exit(2);
		}
	  if ((outfile_asc=FNX_FOPEN(output_file_asc,"w"))==NULL)
		{
		  FNX_PRINTF("could not open the output file %s\n",output_file_asc);
		  exit(2);
		}
...

	{
	  ...
	  FNX_STRCPY(output_file_asc,argv[1]);
	  FNX_STRCPY(FNX_STRRCHR(output_file_asc,'.'),".h");
	  if ((outfile_asc=FNX_FOPEN(output_file_asc,"w"))==NULL)
		{
		  FNX_PRINTF("could not open the output file %s\n",output_file_asc);
  ...
  while(FNX_FGETS(input_line,MAX_INPUT_LINE-2,infile)!=NULL)
	{
	  line_number++;	  
	  if (FNX_STRLEN(input_line)<2) /* blank or mostly blank lines */
		{
		  continue;
		}
	  if (input_line[0]=='#')
	  {
		  if (FNX_STRNCMP(input_line,"#ifdef ",7)==0)
		  {
			  if (ifdef_level>=9)
			  {
				  FNX_PRINTF("too many nested ifdefes in line %d\n",line_number);
				  exit(7);
			  }
			  ifdef_level++;
		  ...
		  else if (FNX_STRNCMP(input_line,"#ifndef ",8)==0)
		  {
			  if (ifdef_level>=9)
			  {
				  FNX_PRINTF("too many nested ifdefes in line %d\n",line_number);
		  ...
		  else if (FNX_STRNCMP(input_line,"#endif",6)==0)
		  {
			  if (ifdef_level==-1)
			  {
				  FNX_PRINTF("unmatched #endif found on line %d\n",line_number);
				  exit(2);
			  }
			  skip_ifdef[ifdef_level]=0;
			  ifdef_level--;
		  }
		  else if (FNX_STRNCMP(input_line,"#else",5)==0)
		  {
			  ...
			  {
				  FNX_PRINTF("#else found with out and matching #ifdef\n");
				  exit(4);
			  }
		  }
		  else
		  {
			  FNX_PRINTF("# directive %s not understood at line %d\n",input_line,line_number);
			  exit(1);
		  }
	  ...
	  if (FNX_STRLEN(input_line)>MAX_INPUT_LINE-10 && input_line[FNX_STRLEN(input_line)-1]!='\n')
		{
		  FNX_PRINTF("rule to long in line number %d line %s\n",line_number,input_line);
		  free_rule_list(list);
		  free_dict_names_list(dict_list);
		  exit(3);
		}
	  if (input_line[FNX_STRLEN(input_line)-1]=='\n')
		  input_line[FNX_STRLEN(input_line)-1]='\0';
	  if (input_line[0]==';')
		{
		  continue;
		}
	  ...
	  {
		  if (FNX_STRNICMP(input_line+1,"rule",4)==0)
		  {
		  ...
		  if (FNX_STRNICMP(input_line+1,"dict",4)==0)
		  {
			  if (input_line[FNX_STRLEN(input_line)-1]!=']')
			  {
				  FNX_PRINTF("invalid dictionary section marker found on line %d",line_number);
				  free_rule_list(list);
				  free_dict_names_list(dict_list);
				  exit(8);
			  }
  ...
  {
	  FNX_PRINTF("no sections found in the rule file\n");
	  exit(6);
  }
  if (list!=NULL)  
  ...
  FNX_FCLOSE(outfile);
  FNX_FCLOSE(outfile_asc);
  free_rule_list(list);
  free_dict_names_list(dict_list);
  FNX_PRINTF("total_rule_length=%d\n",total_rule_length);
  FNX_PRINTF("total_dict_length=%d\n",total_dict_length);
  FNX_PRINTF("total data used = %d\n",total_rule_length+total_dict_length+num_dictionaries*sizeof(short)*3+dict_entry_count*sizeof(short)+(rule_count+1)*sizeof(short)+sizeof(short)*4);
  FNX_PRINTF("the number of rules is %d in %d sections\n",rule_count+1,rule_sections[0]);
  FNX_PRINTF("the number of dictionary entries is %d in %d sections\n",num_dict_entries,dict_sections);
  return(0);
}
