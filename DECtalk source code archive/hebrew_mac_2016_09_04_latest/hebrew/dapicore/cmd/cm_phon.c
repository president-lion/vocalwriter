...
extern unsigned int lang_fonts_shift[];
...
void cm_phon_flush(LPTTS_HANDLE_T phTTS)
{
	...
	if(pCmd_t->param_index && (pKsd_t->phoneme_mode & PHONEME_SPEAK))
	{
		...
		else
		{
			...
			pCmd_t->params[0] = pCmd_t->params[0] | (lang_fonts_shift[pKsd_t->lang_curr]<<PSFONT) | 
		...
		phTTS->lts_funcs[phTTS->cur_lts_lang]->ml_lts_loop(phTTS,pipe_values);
#else
