...
int cm_cmd_phoneme(LPTTS_HANDLE_T phTTS)
{
	...
	{
		value = cm_util_string_match((const unsigned char **)phoneme_modes,pCmd_t->pString[i]);
...
int cm_cmd_log(LPTTS_HANDLE_T phTTS)
{
	...
	for(i=0; i < (int)pCmd_t->param_index; i++)
	{
		...
		value = cm_util_string_match((const unsigned char **)log_options,pCmd_t->pString[i]);
		if(value == NO_STRING_MATCH)
		{
			return(CMD_bad_string);
		}
...
static int OpenDbgLogFile(LPTTS_HANDLE_T phTTS)
{
	...
	if ((pKsd_t->dbglog = FNX_FOPEN("dbglog.txt","w"))==NULL)
		return(FALSE);
#endif
	return( TRUE );
...
int OpenLogFile(LPTTS_HANDLE_T phTTS)
{
  ...
  */
  ...
  if (( phTTS->pLogFile = FNX_FOPEN( "log.txt", "w" )) == NULL )
  {
    OP_UnlockMutex( phTTS->pcsLogFile );
...
int cm_cmd_say(LPTTS_HANDLE_T phTTS)
{
	...
	value = cm_util_string_match((const unsigned char **)say_options,pCmd_t->pString[0]);
	if(value == NO_STRING_MATCH)
		return(CMD_bad_string);
	switch(value)
	{
...
int cm_cmd_error(LPTTS_HANDLE_T phTTS)
{
	...
	value = cm_util_string_match((const unsigned char **)error_options,pCmd_t->pString[0]);
	if(value == NO_STRING_MATCH)
		return(CMD_bad_string);
	switch(value)
	{
...
{   
	...
	pCmd_t->params[0] = cm_util_string_match((const unsigned char **)flush_options, pCmd_t->pString[0]);
...
int cm_cmd_enable(PKSD_T pKsd_t)
{
	...
	if(pKsd_t->halting == FALSE)
	{
		...
		pipe_value = (PFASCII<<PSFONT)+0xb;
#ifdef NEW_MULTI_LANG_ARCH
		phTTS->lts_funcs[phTTS->cur_lts_lang]->ml_lts_loop(pKsd_t->phTTS,&pipe_value);
#else
...
int cm_cmd_punct(LPTTS_HANDLE_T phTTS)
{
	...
	value = cm_util_string_match((const unsigned char **)punct_options,pCmd_t->pString[0]);
	if(value == NO_STRING_MATCH)
		return(CMD_bad_string);
	switch(value)
    {
...
int cm_cmd_skip(LPTTS_HANDLE_T phTTS)
{
	...
	value = cm_util_string_match((const unsigned char **)skip_options,pCmd_t->pString[0]);
	if(value == NO_STRING_MATCH)
		return(CMD_bad_string);
	switch(value)
    {
...
/* GL 04/21/1997  change this for OSF build */
   ...
   temp[j] = '\0';
   FNX_STRCPY(pCmd_t->setv[pCmd_t->cmd_number].cmd,temp); 
   return(CMD_success);
}
...
int cm_cmd_volume(LPTTS_HANDLE_T phTTS)
{
	...
	cmd_type = cm_util_string_match((const unsigned char **)volume_options, pCmd_t->pString[0]);
	...
	switch(cmd_type)
	{
		...
		case DCS_VOLUME_TONE:
			phTTS->pKernelShareData->vol_tone=cmd_value;
			break;
		default:
			return(CMD_bad_value);
	};
...
{   
	...
	{
		cmd_type = cm_util_string_match((const unsigned char **)lang_options,pCmd_t->pString[0]);
		if(cmd_type == NO_STRING_MATCH)
			return(CMD_bad_string);
	}
	else
...
{   
	...
	TextToSpeechSetLanguage(phTTS,lang);
#endif
	
	return(CMD_success);
}
int cm_cmd_ltslanguage(LPTTS_HANDLE_T phTTS)
{   
	PKSD_T pKsd_t = phTTS->pKernelShareData;
	PCMD_T pCmd_t = phTTS->pCMDThreadData;
	...
	phTTS->lts_funcs[phTTS->cur_lts_lang]->ml_lts_loop(phTTS,&pipe_value);
#else
	// second parameter needs '&'!
	// pipe_value is an unsigned short and lts_loop takes an unsigned short*
	lts_loop(phTTS,&pipe_value);
#endif
...
//		return(CMD_flushing);
	...
	TextToSpeechSetLTSLanguage(phTTS,lang);
#endif
...
#ifdef MSDOS
	pKsd_t->lang_ready[phTTS->cur_lts_lang] = 0;
#else
...
#ifdef DEBUG
			FNX_PRINTF("[:rem interval %d]",pKsd_t->power_interval);
#endif /*debug*/
			return(CMD_success);
			break;
		case 2: /* status */
			if (statusreg&EXTPWR)
			{
				FNX_PRINTF("[:power external]");
				return(CMD_success);
			}

			if (statusreg&BATTOK)
			{
				FNX_PRINTF("[:power batt_OK]");
				return(CMD_success);
			}
			FNX_PRINTF("[:power batt_low]");
			return(CMD_success);
			break;
		case 3: /* sleep */
...
int cm_cmd_mode(LPTTS_HANDLE_T phTTS)
{
	...
	{
		value = cm_util_string_match((const unsigned char **)mode_options,(unsigned char *)pCmd_t->pString[i]);
		if(value == NO_STRING_MATCH)
		{
			return(CMD_bad_string);
		}       
			...
			case 0:    /* First param */
				...
				{                               
					...
					case 8: /* email reading mode */
						...
						break;
					case 9: /* email reading mode */
						pipe_value[2] |= MODE_READING;
						break;
					default:
						return(CMD_bad_param);
				}
...
int cm_cmd_pronounce(LPTTS_HANDLE_T phTTS)
{
	...
	for(i=0;i<pCmd_t->param_index;i++)
	{
		value = cm_util_string_match((const unsigned char **)pronounce_options, (unsigned char *)pCmd_t->pString[i]);
		if(value == NO_STRING_MATCH)
			return(CMD_bad_string);
		switch(value)
		{
...
int cm_cmd_name(LPTTS_HANDLE_T phTTS)
{
	...
	if(CT[pCmd_t->cmd_index].esc_value == DCS_NAME)
	{
		...
		pipe_value[1] = pCmd_t->params[0];
#else
		pipe_value[1] = cm_util_string_match((const unsigned char **)voice_names,pCmd_t->pString[0]);
#endif
	}
	else
	{
...
int cm_cmd_mark(LPTTS_HANDLE_T phTTS)
{
	...
	cmd_type = cm_util_string_match((const unsigned char **)index_options,pCmd_t->pString[0]);
	if(cmd_type == NO_STRING_MATCH)
	{
...
#else
#ifdef ARM7
#ifdef EMB_INDEX_MARKS
	  if (phTTS->EmbCallbackRoutine((short *) pKsd_t->lastindex,3)==NULL) // index returned
	  {
		phTTS->pKernelShareData->halting=1;
	  }
#endif
...
//#ifdef _WIN32
...
#ifdef TOMBUCHLER
			phTTS->index_pre_delay=pCmd_t->params[1];
			return(CMD_success);
#else
		...
		case DCS_INDEX_VOLUME:
			...
			pipe_value[2] = pCmd_t->params[2];//N?
			break;
		case DCS_INDEX_REPLACE_START:
			pipe_value[0] = (2<<PSNEXTRA) | INDEX_REPLACE_START;
			pipe_value[2] = pCmd_t->params[2];//N?
			break;
		case DCS_INDEX_REPLACE_END:
			pipe_value[0] = (2<<PSNEXTRA) | INDEX_REPLACE_END;
			pipe_value[2] = pCmd_t->params[2];//N?
			break;
//#endif // _WIN32
	...
	if (pCmd_t->punct_mode==PUNCT_pass || pCmd_t->skip_mode==SKIP_all || 
		pKsd_t->lang_curr == LANG_japanese || pKsd_t->lang_curr == LANG_chinese 
...
#endif
#if defined ARM7 && defined EMB_INDEX_MARKS
	memcpy(pCmd_t->input_indexes[pCmd_t->input_counter].index,pipe_value,sizeof(index_data_t));
#endif
...
int cm_cmd_preamble(LPTTS_HANDLE_T phTTS)
{
	...
	if (list!=NULL)
	{
		...
		{
#ifdef NEW_MULTI_LANG_ARCH
			phTTS->lts_funcs[phTTS->cur_lts_lang]->ml_lts_loop(phTTS,(unsigned short *)&(list[i]));
#else
...
int cm_cmd_define(LPTTS_HANDLE_T phTTS)
{
	...
	if(pCmd_t->param_index == 0)
		return(CMD_success);
	pipe_value[1] = cm_util_string_match((const unsigned char **)define_options,pCmd_t->pString[0]);
...
int cm_cmd_plang(LPTTS_HANDLE_T phTTS)
{
	...
	switch (pKsd_t->lang_curr)
	{
		...
		case LANG_italian:					//chj 7/20/00
			phoneme_code_add=(PFIT<<PSFONT);
			break;
	}
...
int cm_cmd_tone(LPTTS_HANDLE_T phTTS)
{
	...
	pipe[3] = (TONE_AMPLITUDE *	int_volume_table[pKsd_t->vol_tone])>>15;
	pipe[4] = 1000;
	pipe[5] = 0;
	
	/* GL 04/21/1997  change this for OSF build */
	/* RDK 08/29/2003 Added ARM7 */
...
/* GL 04/21/1997  change this for OSF build */
	...
	else
	{
	  ...
	  pipe[3] = (DTMF_HIGH_TONE_AMPLITUDE  * int_volume_table[pKsd_t->vol_tone])>>15;
	  pipe[4] = tlitone1[iIndex];
	  pipe[5] = (DTMF_LOW_TONE_AMPLITUDE  * int_volume_table[pKsd_t->vol_tone])>>15;;

/* GL 04/21/1997  change this for OSF build */
#if !defined NO_TONE_SUPPORT
	  // MGS CHECKME
	  vtm_loop(phTTS,pipe);
#endif // defined (WIN32) || defined (ARM7) || defined _UNIX_LIKE_
...
int cm_cmd_gender(LPTTS_HANDLE_T phTTS)
{
	...
	cmd_type =  cm_util_string_match((const unsigned char **)gender_options,pCmd_t->pString[0]);
	if (cmd_type == NO_STRING_MATCH)
		return(CMD_bad_string);
