 ...
 * 080	MGS		11/18/2004	Fixed <word> + space + <period> + 0x0b so it would speak.
 */
...
//#ifdef WIN32
#if !defined _UNIX_LIKE_ || defined _APPLE_MAC_ || defined _IOS_
#ifdef INLINE_FUNC_IN_FILE_USED
...
/* GL 04/21/1997  add this for OSF build */
		...
		if ((pCmd_t->email_header == 0) &&
      	    (!FNX_STRNCMP(header_buff,header1,4)  ||
      	     !FNX_STRNCMP(header_buff,header2,12) ||
      	     !FNX_STRNCMP(header_buff,header3,15) ||
      	     !FNX_STRNCMP(header_buff,header4,11)))
      	{   
...
//			WINprintf("get_clause_parse at %ld.\n", ulStartTime);
		...
		*/ 
		...
		if ((pCmd_t->index_counter == 0) && (pCmd_t->input_counter < PAR_MIN_INPUT_SIZE) && (pCmd_t->punct_mode != PUNCT_all))
		{
			...
			FNX_STRCPY(pCmd_t->output_buf,pCmd_t->clausebuf);
		...
		else
		{
		  ...
		  if ((pCmd_t->skip_mode != SKIP_email) &&
...
#ifdef DEBUG_PARSER
		      FNX_PRINTF("The input to Email.%s.\n",pCmd_t->clausebuf);		
#endif
		      ...
		      if (DT_DBG(CMD_DBG,0x040))
			{
			  ...
			  FNX_PRINTF("\nInput to Email:(%d)(%x)",pCmd_t->input_counter,temp_mode);
#endif
			  for (k=0; k < FNX_STRLEN(pCmd_t->clausebuf); k++)
			    {
#ifndef ARM7_NOSWI
#ifndef MSDOS
					if (pKsd_t->dbglog)		/* mfg added for dbglog.txt support*/
				...
				FNX_PRINTF("\nEmail output:");
					...
					for (k=0; k < FNX_STRLEN(pCmd_t->output_buf); k++)
					{
#ifndef ARM7_NOSWI
#ifndef MSDOS
					if (pKsd_t->dbglog)		/* mfg added for dbglog.txt support*/
				...
				FNX_STRCPY(pCmd_t->clausebuf,pCmd_t->output_buf);
#ifdef NEW_INDEXING
			...
			if (pCmd_t->skip_mode != SKIP_punct)
			{
				...
				/* cm_text_preproc(pCmd_t); */             
#ifdef DEBUG_PARSER
				FNX_PRINTF("the input to punct.%s.\n",pCmd_t->clausebuf);		
#endif
	        	...
	        	pCmd_t->input_counter=FNX_STRLEN(pCmd_t->clausebuf);
				didit=0;
...
#ifdef DEBUG_PARSER
				FNX_PRINTF("the input to normal .%s.\n",pCmd_t->clausebuf);		
				FNX_PRINTF("the dict hit buf is  ");
				for (i=0;i<pCmd_t->input_counter;i++)
				{
					FNX_PUTC(pCmd_t->dict_hit_buf[i]+'0');
				}
				FNX_PRINTF("\n");
#endif
...
#ifdef NEW_MULTI_LANG_ARCH
				phTTS->lts_funcs[phTTS->cur_lts_lang]->ml_lts_loop(phTTS,pCmd_t->output_indexes[i].index);
#else
