...
{   
...
#ifdef DEBUGPARS
	FNX_PRINTF("CMD_PARS: cm_cmd_match_comm:\n");
#endif
	...
	if(c == ' ' || c == '\t' || c == '\r' || c == '\n')
	{
		if(pCmd_t->total_matches == 1)
		{
			...
			{   
//				FNX_PRINTF("not index command 1\n");
...
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_match_comm: Parse State Switch: STATE_COMMAND->STATE_PARAM\n");
#endif
		}
		else 
		{
...
#ifdef NEW_INDEXING
//				FNX_PRINTF("not index command 2\n");
			 	temp=pCmd_t->ParseChar; 
			 	pCmd_t->ParseChar=0x0fff;
/*	0010	MGS		11/19/1998		BATS #812 fixed say-letter mode extra spaces */
				pCmd_t->letter_mode_flag=1;
...
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_match_comm: Parse State Switch: STATE_COMMAND->STATE_TOSS\n");
#endif
			}
		}
		return;
	...
	if(c == ']')
	{
		...
		/* by sending a 0x0fff to it*/
#ifdef NEW_INDEXING
//		FNX_PRINTF("not index command 3\n");
	 	temp=pCmd_t->ParseChar;     
	 	pCmd_t->ParseChar=0x0fff;
/*	0010	MGS		11/19/1998		BATS #812 fixed say-letter mode extra spaces */
		pCmd_t->letter_mode_flag=1;
...
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_match_comm: Parse State Switch: STATE_COMMAND->STATE_NORMAL\n");
#endif
		return;
	}
...
#ifdef NEW_INDEXING
//			FNX_PRINTF("not index command 4\n");
		 	temp=pCmd_t->ParseChar; 
		 	pCmd_t->ParseChar=0x0fff;
/*	0010	MGS		11/19/1998		BATS #812 fixed say-letter mode extra spaces */
			pCmd_t->letter_mode_flag=1;
		...
		else
		{
			...
			if (memcmp(CT[pCmd_t->cmd_index].c_name,"index",5)!=0)
			{
//				FNX_PRINTF("not index command 5\n");
			 	temp=pCmd_t->ParseChar; 
			 	pCmd_t->ParseChar=0x0fff;
/*	0010	MGS		11/19/1998		BATS #812 fixed say-letter mode extra spaces */
				pCmd_t->letter_mode_flag=1;
...
#ifdef DEBUGPARS
	FNX_PRINTF("CMD_PARS: cm_cmd_do_command:\n");
#endif
	if(pCmd_t->total_matches == 1)
	{                                   
...
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_do_command: Parse State Switch: STATE_?->STATE_new?\n");
#endif
			return;
		}
		else 
...
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_do_command: Parse State Switch: STATE_?->STATE_NORMAL\n");
#endif
#ifdef ESCPAE_SEQ
...
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_do_command: Parse State Switch: STATE_?->STATE_TOSS\n");
#endif
				return;
			}
		}
...
void cm_cmd_build_param(LPTTS_HANDLE_T phTTS, unsigned int c)
{
...
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_build_param: Parse State Switch: STATE_PARAM->STATE_BRACKET\n");
#endif
		return;
	}
	...
	if(FORMAT_PARAM == '*')
	{
...
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_build_param: Parse State Switch: STATE_PARAM->STATE_COMMAND\n");
#endif
			return;
		}
		if(c == ' ' || c == '\t' || c == '\r' || c == '\n')
...
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_build_param: Parse State Switch: STATE_PARAM->STATE_KEEP\n");
#endif
			return;
		}
		if(c == ']')
		{
			cm_cmd_do_command(phTTS, STATE_NORMAL);
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_build_param: Parse State Switch: STATE_PARAM->STATE_NORMAL\n");
#endif
			return;
		}
	...
	switch (c)       
	{
		...
		case ',':
			if(FORMAT_PARAM == '\0' || FORMAT_PARAM == '*')
			{
				...
				cm_pars_new_state(pCmd_t, STATE_TOSS);
#ifdef DEBUGPARS
		FNX_PRINTF("CMD_PARS: cm_cmd_build_param: Parse State Switch: STATE_PARAM->STATE_TOSS\n");
#endif
			}
			break;
...
#ifdef DEBUGPARS
	FNX_PRINTF("CMD_PARS: cm_cmd_reset_comm:\n");
#endif
	if(state != STATE_KEEP)
	{
		for(i=0;i<NPARAM;i++)
