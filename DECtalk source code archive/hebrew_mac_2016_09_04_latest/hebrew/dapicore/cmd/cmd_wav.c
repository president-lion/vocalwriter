...
//#include <stdlib.h>
//#include <string.h>
//#elif defined ARM7 || defined _APPLE_MAC_
//#include <stdlib.h>
//#include <string.h>
//#else
//#include <malloc.h>
...
int cm_cmd_play(LPTTS_HANDLE_T phTTS)
{
  ...
  */
  ...
  /* fix [:play xxx] for ce to keep the output synchronous */
	...
	MultiByteToWideChar(CP_ACP,0,pCmd_t->pString[0],FNX_STRLEN(pCmd_t->pString[0]),&string,sizeof(string));
...
/* GL 04/21/1997  add this for OSF build */
	...
	lpData = (char *)FNX_MALLOC( 2 * PLAY_READ_BUFFER_SIZE * sizeof(char));
	...
	pOldWaveFormat = (LPWAVEFORMATEX)FNX_MALLOC(sizeof(WAVEFORMATEX));
	...
	/******************************************************************/
	if (FNX_STRRCHR(pCmd_t->pString[0],'.')!=NULL && 
		 FNX_STRICMP(FNX_STRRCHR(pCmd_t->pString[0],'.'),".au")==0)
	{
		is_au=1;
		/* open the file */
		if ((au_fd=open(pCmd_t->pString[0],O_RDONLY))==-1)
		{
	  		FNX_FREE( pOldWaveFormat );
			if (pWaveFormat)
	  			FNX_FREE( pWaveFormat );
		  	FNX_FREE( lpData );
			TextToSpeechErrorHandler( phTTS,
		...
		if (FNX_STRCMP(AuFileHdr.magic,".snd"))
		{
			TextToSpeechErrorHandler( phTTS,
				  ERROR_BAD_WAVE_FILE_FORMAT,
				  MMSYSERR_ERROR );
...
/* GL 04/21/1997  change this for OSF build */
  ...
  pWaveFormat = (LPWAVEFORMATEX)FNX_MALLOC((size_t)dwSize);
