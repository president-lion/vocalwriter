...
//#include "dbgwins.h" //moved to port.h for each platform - mlt
...
//#include <limits.h> //moved to port.h for each platform - mlt
...
int main(void)
{
	...
	{                                       			
#ifdef SET_INDEXES
		for (i=0,j=1;i<(int)FNX_STRLEN(input_array);i++)
		{
			if (input_array[i]==' ')
			{
				for (k=FNX_STRLEN(input_array);k>(i+1);k--)
				{
		...
		FNX_STRCPY(input_array,output_array);
		par_copy_index_list(input_indexes,0,output_indexes,0,FNX_STRLEN(output_array));
...
{
#ifdef DEBUG
		FNX_PRINTF("entering par_read_input\n");
#endif
	if ((FNX_FGETS(input_array,PAR_MAX_INPUT_ARRAY-1,stdin))==NULL)
	{
#ifdef DEBUG
		FNX_PRINTF("leaving par_read_input 0\n");
#endif
		return(0);
	}
	input_array[FNX_STRLEN(input_array)-1]='\0';
#ifdef DEBUG
		FNX_PRINTF("leaving par_read_input 1\n");
#endif
	return(1);
}
...
#ifdef SET_INDEXES
#ifdef PRINT_INDEXES
	for (i=0;i<(int)FNX_STRLEN(output_array);i++)
	{
		if (output_indexes[i].index[0]!='\0')
		{
			FNX_PRINTF("%c",output_indexes[i].index[0]);
		}
		else
		{
			FNX_PRINTF(".");
...
{
#ifdef DEBUG
		FNX_PRINTF("entering par_initialize_variables\n");
#endif
	memset(input_array,0,PAR_MAX_INPUT_ARRAY);
	memset(output_array,0,PAR_MAX_OUTPUT_ARRAY);
	memset(dict_hit_array,0,PAR_MAX_INPUT_ARRAY);
#ifdef DEBUG
		FNX_PRINTF("leaving par_initialize_variables\n");
#endif
}
#endif /* #ifdef PARSER_STANDALONE_DEBUG */
...
{                               
	int i;
#ifdef DEBUG
	FNX_PRINTF("entering par_initialize_arrays\n");
#endif
...
#ifdef DEBUG
		FNX_PRINTF("leaving par_initialize_arrays\n");
#endif
}
                                  ...
                                  preturn_value_t ret_value)
{
...
#ifdef DEBUG
    	FNX_PRINTF("entering par_process_input\n");
#endif
	...
	{
		FNX_PRINTF("par_process_input; no such rule section %d\n",rule);
		FNX_STRCPY(output_array,"Invalid rule section. ");
		return(ret_value);
	}
	...
	input_length=FNX_STRLEN(input_array);

	FNX_STRCPY(new_input,input_array);
    	   ...
    	   (((new_ret.input_pos+new_ret.input_offset-new_input_diff)<input_length) && (go_until==1) ) )
	{
		...
		{  	
			...
			if (current_rule->special_rule!=NORMAL_TAG_VALUE)
			{
				...
				FNX_PRINTF("special rule value is out of range\n");
				break;
			}
...
#ifdef DEBUG
				FNX_PRINTF("the language flag is 0x%04X\n",current_rule->lang_flag);
#endif
...
#ifdef DEBUG
				FNX_PRINTF("the mode flag is 0x%04X\n",current_rule->mode_flag);
#endif
			/*	044	MGS		09/24/1997	BATS#469 Fix for NWS parser problem */
			if (last_rule_was_hit==0)
			{
			...
			if (current_rule->dict_flag!=UNSET_PARAM)
			{
				...
				{
#ifdef DEBUG
						FNX_PRINTF("processing rule because of dictionary HIT on the word\n");
#endif				
				}
				else
				{
					...
					{
#ifdef DEBUG
							FNX_PRINTF("processing rule because of dictionary MISS on the word\n");
#endif
					}
					else
					{
...
#ifdef DEBUG_RULES
#ifdef PARSER_STANDALONE_DEBUG
				FNX_FPRINTF(stderr,"rule number is R%d\n",current_rule->rule_number);
				FNX_FPRINTF(stderr,"next hit rule is %d\n",current_rule->next_hit_rule);
				FNX_FPRINTF(stderr,"next_miss_rule is %d\n",current_rule->next_miss_rule);
#else
				FNX_PRINTF("rule number is R%d\n",current_rule->rule_number);
				FNX_PRINTF("next hit rule is %d\n",current_rule->next_hit_rule);
				FNX_PRINTF("next_miss_rule is %d\n",current_rule->next_miss_rule);
#endif
#endif // DEBUG_RULES
#ifdef DEBUG
			...
			{                               
				...
				else                      
				{
					...
					if (pKsd_t->dbglog)		/* mfg added for dbglog.txt logging support*/
						FNX_FPRINTF(pKsd_t->dbglog,"Rule hit:R%d\n",current_rule->rule_number);
#endif	
					FNX_PRINTF("Rule hit:R%d\n",current_rule->rule_number);
					}
#endif // PARSER_STANDALONE_DEBUG
#ifdef DISPLAY_RULES_HIT
#ifdef PARSER_STANDALONE_DEBUG
#ifdef OUTPUT_HITS_NORMAL
					FNX_FPRINTF(stderr,"H");
					FNX_PRINTF("the rule hit was R%d\n",current_rule->rule_number);
#else
					FNX_FPRINTF(stderr,"the rule hit was R%d\n",current_rule->rule_number);
					...
					if ((current_rule->next_hit_rule!= -1) || (current_rule->next_goret_hit!= -1))
					{
						...
						{
#ifdef DEBUG
							FNX_PRINTF("par_process_input;the output is larger than the input\n");
#endif
							...
							else
							{
								...
								for (i=FNX_STRLEN(new_input)+size_diff;i>j;i--)
								{
#ifdef DEBUG
										FNX_PRINTF("process_input;copying new_input[%d]%c(%0X)=new_input[%d]%c(%0X)\n",i,new_input[i],new_input[i],i-size_diff,new_input[i-size_diff],new_input[i-size_diff]);
#endif
						...
						else
						{
							...
							{
#ifdef DEBUG
								FNX_PRINTF("par_process_input;the output is the same size as the input\n");
#endif              
							...
							{
#ifdef DEBUG
								FNX_PRINTF("par_process_input;the output is smaller than the input\n");
#endif                                   
								/* 
...
#ifdef DEBUG
				FNX_PRINTF("process_input done = %d\n",done);
#endif
...
#ifdef DEBUG
		FNX_PRINTF("leaving par_process_input\n");
#endif
	return(ret_value);
}                       
						   ...
						   short current_rule_number)
{
	...
	{   
#ifdef DEBUG
		FNX_PRINTF("par_get_return_level;going to rule %d the level is %d\n",return_rule[(*return_level)-1],(*return_level)-1);
#endif
	...
	{
//#ifdef DEBUG
			FNX_PRINTF("par_get_return_level;no place to go to, going to next rule %d\n",current_rule_number);
//#endif
						  ...
						  short go_rule)
{
	...
	{
#ifdef DEBUG
		FNX_PRINTF("par_set_return_level;saving rule %d in level %d\n",go_rule,*return_level);
#endif
	...
	{
//#ifdef DEBUG
		FNX_PRINTF("par_set_return_level;too many levels of gorets throwing away %d\n",go_rule);
//#endif
	}
}
...
//	int end_marker=End_Is_Slash;			/* used to store the end of state marker for comparison */
...
#ifdef DEBUG
		FNX_PRINTF("leaving par_match_rule some input is NULL returning FAIL\n");
#endif
		return;
	}
        length_of_input=FNX_STRLEN(input_array);
	/* 
	...
	if (state==MACRO_STATE)
	{
		...
		new_ret.rule=rule_p;
		if ((new_ret.input_pos+new_ret.input_offset > (int)FNX_STRLEN(input_array))) 
		{
	...
	while ((current_rule[new_ret.rule]!=end_marker) && 
		   ...
		   /*(new_ret.input_pos+new_ret.input_offset < (int)FNX_STRLEN(input_array))*/ )	/* slow ?? */
	/* process all the data in this action state that is before the ending slash */
	{
		if ((new_char_type=par_get_char_type(current_rule[new_ret.rule]))!=NULL_TYPE)
		{
			...
			{
#ifdef DEBUG
					FNX_PRINTF("leaving par_match_rule; end of string reached in string\n");
#endif
				if (new_ret.optional==1)
				{
				    new_ret.optional= -1;
		...
		else
		{
			if ((new_action_state=par_get_state(current_rule[new_ret.rule]))!=NULL_STATE)
			{
				...
				{
#ifdef DEBUG
						FNX_PRINTF("leaving par_match_rule; end of string reached in rule\n");
#endif
					if (new_ret.optional==1)
					{
						new_ret.optional= -1;
...
#ifdef INDEX_DEBUG2
			FNX_PRINTF("cleared indexes from %d to %d in the output, end_of_string\n",new_ret.output_pos,new_ret.output_pos+new_ret.output_offset);
#endif
		memset(output_indexes+new_ret.output_pos,0,new_ret.output_offset*sizeof(index_data_t));
		ret_value->value=END_OF_STRING;
#ifdef DEBUG
			FNX_PRINTF("leaving par_match_rule; the end of the string was encountered\n");
#endif
		return;
	}
...
#ifdef DEBUG
		FNX_PRINTF("the current output is:%s\n",output_array);
		FNX_PRINTF("output_pos=%d output_offset=%d\n",new_ret.output_pos,new_ret.output_offset);
#endif
...
//FNX_PRINTF("B parser_flag is %d\n",new_ret.parser_flag);
...
#ifdef DEBUG
    	FNX_PRINTF("par_match_rule;the output after the action has been performed\n is %s\n",output_array);
#endif                       
    if (new_ret.value==FAIL)
    {
#ifdef INDEX_DEBUG2
		FNX_PRINTF("cleared indexes from %d to %d in the output, fail-fail\n",new_ret.output_pos,new_ret.output_pos+new_ret.output_offset);
#endif
		memset(output_indexes+new_ret.output_pos,0,new_ret.output_offset*sizeof(index_data_t));
    	if (new_ret.optional==1)
    	{
	...
	{
#ifdef DEBUG
		FNX_PRINTF("par_match_rule;optional set to OPT_FAIL\n");
#endif
#ifdef INDEX_DEBUG2
		FNX_PRINTF("cleared indexes from %d to %d in the output, opt_fail\n",new_ret.output_pos,new_ret.output_pos+new_ret.output_offset);
#endif
		memset(output_indexes+new_ret.output_pos,0,new_ret.output_offset*sizeof(index_data_t));
	...
	ret_value->rule=new_ret.rule;
#ifdef DEBUG
	FNX_PRINTF("leaving par_match_rule success\n");
#endif
}
...
{                                                                                            
...
#ifdef DEBUG
	FNX_PRINTF("entering perform action\n");
#endif
	if (ret_value==NULL)
	{
#ifdef DEBUG
		FNX_PRINTF("leaving perform action ret_value is NULL\n");
#endif
		return;
	}
	if ((current_rule==NULL) || (output_array==NULL))
	{
		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
		FNX_PRINTF("leaving perform action inputs are NULL\n");
#endif
		return;
	}
	switch(state)
	{
		...
		case STATUS_STATE:
			...
			ret_value->parser_flag = FNX_ATOI(buf);
//FNX_PRINTF("A parser_flag is %d\n",ret_value->parser_flag);
			ret_value->rule++;
		default:
			break;
	}            
#ifdef DEBUG
		FNX_PRINTF("leaving perform action;the action is done\n");
#endif
}
...
{
#ifdef DEBUG
		FNX_PRINTF("in par_get_char_type type=%d\n",c);
#endif
	switch(c)
	{
...
{
#ifdef DEBUG
		FNX_PRINTF("in par_get_state state=%d\n",c);
#endif
	switch(c)
	{
		case COPY_DELIMITER:
					   ...
					   preturn_value_t ret_value)
{
...
#ifdef DEBUG
 	FNX_PRINTF("entering par_delete_string\n");
#endif
	/*
 	...
 	{
#ifdef DEBUG
 		FNX_PRINTF("leaving par_delete_string ret_value is NULL\n");
#endif
 		return;
 	}
 	if ((output_array==NULL))
 	{
 		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
 		FNX_PRINTF("leaving par_delete_string input is NULL\n");
#endif
 		return;
 	}                     
	...
	for (j=i=ret_value->output_pos;i<save_offset;i++)
	{
		...
		{
#ifdef INDEX_DEBUG
			FNX_PRINTF("copying index in delete state from %d to %d\n",i,j);
#endif
...
#ifdef DEBUG
		FNX_PRINTF("leaving par_delete_string string deleted\n");
#endif
}
						...
						prange_value_t range_value)
{
	...
	index_data_t  temp_index[100];
 	...
 	FNX_PRINTF("entering par_replace_string\n");
#endif
	...
	memset(temp_index,0,100*sizeof(index_data_t));
 	...
 	{
#ifdef DEBUG
 		FNX_PRINTF("leaving par_replace_string ret_value is NULL\n");
#endif
 		return;
 	}
 	if ((current_rule==NULL) || (output_array==NULL) || (match_array==NULL))
 	{
 		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
 		FNX_PRINTF("leaving par_replace_string inputs are invalid\n");
#endif
 		return;
 	}
	/* 
	...
	if (ret_value->optional!= -1)
	{
		...
		for (j=0,i=ret_value->output_pos;i<ret_value->output_pos+ret_value->output_offset;i++)
		{
			if (par_is_index_set(output_indexes,i))
			{
				...
				{
#ifdef INDEX_DEBUG
					FNX_PRINTF("copying index in replace 1 from %d to buf %d\n",i,j);
#endif
				...
				else
				{
					...
					buf[j+1]='\0';     
#ifdef INDEX_DEBUG
					FNX_PRINTF("copying index in replace 2 from %d to buf %d\n",i,j);
#endif
					par_copy_index(temp_index,j,output_indexes,i);
					j++;
					length++;
		...
		FNX_STRCPY((output_array+(ret_value->output_pos)),buf);
		 ...
		 */
#ifdef DEBUG
		FNX_PRINTF("par_replace_string; length=%d\n",length);
#endif	
		ret_value->output_offset=length;
	}
	ret_value->rule++;
#ifdef DEBUG
	FNX_PRINTF("leaving replace_state replacement is successful\n");
#endif
}
					   ...
					   prange_value_t range_value)
{
	...
	int pos,new_int,off;
...
#ifdef DEBUG
 	FNX_PRINTF("entering par_insert_string\n");
#endif
 	/* 
 	 *	check the inputs for valid data 
 	 */
 	...
 	{
#ifdef DEBUG
 		FNX_PRINTF("leaving par_insert_string ret_value is NULL\n");
#endif
 		return;
 	}
 	if ((current_rule==NULL) || (output_array==NULL) || (match_array==NULL))
 	{
 		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
 		FNX_PRINTF("leaving par_insert_string; inputs are invalid\n");
#endif
 		return;
 	}
	/* 
	...
	ret_value->rule++;
#ifdef DEBUG
	FNX_PRINTF("leaving par_insert_string; insert is done\n");
#endif
	return;
}
							 ...
							 prange_value_t range_value)
{
...
#ifdef DEBUG
		FNX_PRINTF("entering par_insert_string_after\n");
#endif
 	/*
 	...
 	{
#ifdef DEBUG
 		FNX_PRINTF("leaving par_insert_string_after ret_value is NULL\n");
#endif
 		return;
 	}
 	if ((current_rule==NULL) || (output_array==NULL) || (match_array==NULL))
 	{
 		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
 		FNX_PRINTF("leaving par_insert_string_after inputs are invalid\n");
#endif
 		return;
 	}
    /*
    ...
    if (ret_value->optional!= -1)
    {
    	...
    	FNX_STRCPY((output_array+(ret_value->output_pos+ret_value->output_offset)),buf);
    	/* add the length of the inserted atring to the offset */
    	ret_value->output_offset+=length;
    }
	ret_value->rule++;
#ifdef DEBUG
 	FNX_PRINTF("leaving par_insert_string_after insert after is done\n");
#endif
    return;
}                                                                             
							  ...
							  prange_value_t range_value)
{
...
#ifdef DEBUG
	FNX_PRINTF("entering par_insert_string_before\n");
#endif
 	/*
 	 *	check the input values
 	 */
 	...
 	{
#ifdef DEBUG
 		FNX_PRINTF("leaving par_insert_string_before ret_value is NULL\n");
#endif
 		return;
 	}
 	if ((current_rule==NULL) || (output_array==NULL) || (match_array==NULL))
 	{
 		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
 		FNX_PRINTF("leaving par_insert_string_before; inputs are invalid\n");
#endif
 		return;
 	}
	...
	ret_value->rule++;
#ifdef DEBUG
		FNX_PRINTF("leaving par_insert_string_before; insert before is done\n");
#endif
    return;
}    
...
void par_save_string(unsigned char *output_array,
					 ...
					 preturn_value_t ret_value)
{
	/* this should be the last thing in the output array, but to be sure FNX_STRNCPY is used */
#ifdef DEBUG
	FNX_PRINTF("entering par_save_string\n");
#endif
	/* 
	 *	check the input values 
	 */
 	...
 	{
#ifdef DEBUG
 		FNX_PRINTF("leaving par_save_string ret_value is NULL\n");
#endif
 		return;
 	}
 	if ((output_array==NULL) || (match_array==NULL))
 	{
 		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
 		FNX_PRINTF("leaving par_save_string; inputs are invalid\n");
#endif
 		return;
 	}
 	if (num<0 || num >PAR_MAX_ARRAYS)
 	{
 		FNX_PRINTF("$%d is out of range\n",num);
 		ret_value->value=FATAL_FAIL;
 		return;
 	}
	if (ret_value->optional!= -1)
...
#ifdef DEBUG
	FNX_PRINTF("leaving par_save_string; string is saved in array %d\n",num);
#endif
	return;
}
						 ...
						 short dict_state_flag)
{
	...
	short	result=FAIL;
#ifdef DEBUG
	FNX_PRINTF("entering par_dom_dict_search\n");
#endif
	...
	if (dict_state_flag)
	{
		...
		{
#ifdef DEBUG
			FNX_PRINTF("leaving par_dom_dict_search 1 success\n");
#endif
			ret_value->value=SUCCESS;
			return;
		}
#ifdef DEBUG
		FNX_PRINTF("leaving par_dom_dict_search 1 failure\n");
#endif
		return;		
	}
	if (result==1)
	{
		if (memcmp(current_rule+ret_value->rule,"FAIL",4)==0)
		{
			...
			{
#ifdef DEBUG
				FNX_PRINTF("par_dom_dict_search hit set optinal to -1\n");
#endif
			...
			else
			{
				...
				FNX_PRINTF("leaving par_dom_dict_search because of a fail hit_action\n");
#endif
				return;
			}
		}
	...
	else    /* if (result==1) */
	{
		...
		if (memcmp(current_rule+ret_value->rule,"FAIL",4)==0)
		{
			...
			{
#ifdef DEBUG
				FNX_PRINTF("par_dom_dict_search miss set optinal to -1\n");
#endif
				ret_value->optional= -1;
				ret_value->rule+=5;
			}
#ifdef DEBUG
				FNX_PRINTF("leaving par_dom_dict_search because of a fail miss_action\n");
				par_print_rule_error("",current_rule,ret_value->rule);
#endif
			return;
		}
...
#ifdef DEBUG
	FNX_PRINTF("leaving par_dom_dict_search\n");
	par_print_rule_error("",current_rule,ret_value->rule);
#endif
}
...
{           
...
#ifdef DEBUG
	FNX_PRINTF("entering par_look_ahead_dictionary\n");
#endif
	...
	{
#ifdef DEBUG
		FNX_PRINTF("leaving par_look_ahead_dictionary success\n");
#endif
		return(1);
	}
#ifdef DEBUG
	FNX_PRINTF("leaving par_look_ahead_dictionary failure\n");
#endif                                                    
	return(0);
}
						  ...
						  short dict_state_flag)
{
	...
	short save_for;
#ifdef DEBUG
	FNX_PRINTF("entering par_search_for_word\n");
#endif                        
...
#ifdef DEBUG
			FNX_PRINTF("comparing %s to %d %s\n",input,pos,(unsigned char *)(dict_data_table+dict_index_table[pos]));
#endif
		if ((value=FNX_STRICMP(input,(unsigned char *)(dict_data_table+dict_index_table[pos])))==0)
		{
			break;
		}
		else
	...
	if (dict_state_flag)
	{
		...
		{
#ifdef DEBUG
			FNX_PRINTF("leaving par_search_for_word in dict and short search\n");
#endif
			return(1);
		}
	}                                    
	...
	{
#ifdef DEBUG
		FNX_PRINTF("leaving par_search_for_word not in dic\n");
#endif
		return(0);
	}
	/* do the case sensitive part of the search */
	rev_same=pos-1;
	while ((rev_same>=low) && (FNX_STRICMP(input,(unsigned char *)(dict_data_table+dict_index_table[rev_same]))==0))
	{
		rev_same--;
	}                  
	for_same=pos+1;
	while ((for_same>=low) && (FNX_STRICMP(input,(unsigned char *)(dict_data_table+dict_index_table[for_same]))==0))
	{
		for_same++;
	}                  
	rev_same++;
	...
	if (rev_same!=for_same)
	{
...
#ifdef DEBUG
				FNX_PRINTF("comapring case %s to %d %s\n",input,npos,(unsigned char *)(dict_data_table+dict_index_table[npos]));
#endif
			if ((value=FNX_STRCMP(input,(unsigned char *)(dict_data_table+dict_index_table[npos])))==0)
			{
				break;
			}
			else
		...
		else
		{
			...
			{
#ifdef DEBUG
				FNX_PRINTF("leaving par_dom_dict_search capitilization doesn't match\n");
#endif
				return(0);
			}
		}
	...
	if (value==0)
	{
		FNX_STRCPY(output,((unsigned char *)(dict_data_table+dict_index_table[save_for]))+(FNX_STRLEN((unsigned char *)(dict_data_table+dict_index_table[save_for]))+1));
#ifdef DEBUG
		FNX_PRINTF("leaving par_search_for_word success\n");
#endif
		return(1);
	}             
	return(0);
#ifdef DEBUG
	FNX_PRINTF("leaving par_search_for_word failure\n");
#endif
}
...
#ifdef DEBUG
 	FNX_PRINTF("entering par_check_word_string\n");
#endif
	/*
 	 *	check the inputs for valid data
 	 */
 	...
 	{
#ifdef DEBUG
 		FNX_PRINTF("leaving par_check_word_string ret_value is NULL\n");
#endif
 		return;
 	}
 	if ((output_array==NULL))
 	{
 		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
 		FNX_PRINTF("leaving par_check_word_string input is NULL\n");
#endif
 		return;
 	}                                  
	if (ret_value->optional!= -1)
	{
		...
		while (i<(ret_value->output_offset+ret_value->output_pos) && (has_cons==0 || has_vowel==0))
		{
			...
			if ((parser_char_types[output_array[i]] & TYPE_alpha) == 0)
			{
				ret_value->value=FAIL;
#ifdef DEBUG
				FNX_PRINTF("the output is not a word\n");
#endif
				return;
			}			
			i++;
		...
		{
#ifdef DEBUG
			FNX_PRINTF("the output may be a word\n");
#endif
		}         
		else
		{
...
#ifdef DEBUG
		FNX_PRINTF("leaving par_check_word_string\n");
#endif
	return;
}
										  ...
										  int *length)
{
...
#ifdef DEBUG
	FNX_PRINTF("entering par_build_string_from_rule\n");
#endif
	/*
	 *	check input values
	 */
	...
	{
#ifdef DEBUG
		FNX_PRINTF("leaving par_build_string_from_rule ret_value is NULL\n");
#endif
		return(NULL);
	}
	...
	{
#ifdef DEBUG
		FNX_PRINTF("leaving par_build_string_from_rule; inputs are invalid\n");
#endif
		ret_value->value=FATAL_FAIL;
		return(NULL);
	}
...
#ifdef DEBUG
		FNX_PRINTF("current_rule[%d]=%c\n",rule_p,current_rule[rule_p]);
		if (current_rule[rule_p]==EXACT_CHAR_DELIM)
			FNX_PRINTF("found an exact character delimiter\n");
#endif
			...
			switch (current_rule[rule_p])
			{
...
#ifdef DEBUG
						FNX_PRINTF("build_string %c ",current_rule[rule_p]);
#endif
						if (ret_value->optional!= -1)
						{
				...
				case SAVE_DELIMITER: /* a $# sequence was found. */
					...
					if (ret_value->optional!= -1)
					{
						...
						{
							FNX_PRINTF("build_string;shuffle index %d out of range",match_num);
							par_print_rule_error("",current_rule,rule_p);
							ret_value->value=FATAL_FAIL;
							return(NULL);
						}
						match_l=FNX_STRLEN(match_array->array[match_num]);	/* get the length of the strig in the array */
...
#endif
#ifdef 	DEBUG
		FNX_PRINTF("leaving par_build_string_from_rule buf=%s\n",buf);
#endif
	return(buf);
}
...
#ifdef DEBUG
	FNX_PRINTF("entering par_match_string\n");
#endif
	rule_p=ret_value->rule;
	...
	switch(char_type)
	{
			...
			while ((current_rule[rule_p]!=EXACT_CHAR_DELIM))
			{
				...
				if (ret_value->optional!= -1)
				{
					if (current_rule[rule_p]!=input_array[ipos+length])
					{
						ret_value->value=FAIL;
#ifdef DEBUG
						FNX_PRINTF("leaving par_match_string; exact characters dont match\n");
#endif
						if (ret_value->optional==1)
						{
#ifdef DEBUG
							FNX_PRINTF("par_match_string; exact optional set to -1\n");
#endif
							ret_value->optional= -1;
						}
			...
			while ((current_rule[rule_p]!=EXACT_CASE_DELIM))
			{
				...
				if (ret_value->optional!= -1)
				{
					if (par_lower[current_rule[rule_p]]!=par_lower[input_array[ipos+length]])
					{
						ret_value->value=FAIL;
#ifdef DEBUG
						FNX_PRINTF("leaving par_match_string; exact case characters dont match\n");
#endif
						if (ret_value->optional==1)
						{
#ifdef DEBUG
							FNX_PRINTF("par_match_string; exact case optional set to -1\n");
#endif
							ret_value->optional= -1;
						}
						length= -1; /* set to -1 because of the length++ later */
		...
		case HEXADECIMAL_TYPE:
			if (ret_value->optional!= -1)
			{
				...
				if (input_array[ipos]!=temp_hex)
				{
					...
					{
#ifdef DEBUG
						FNX_PRINTF("par_match_string; hex optional set to -1\n");
#endif
						ret_value->optional= -1;
					}
#ifdef DEBUG
						FNX_PRINTF("leaving par_match_string hexadecimal value %02X mismatch\n",temp_hex);
#endif
					return(0);
				}
				length=1;
		...
		case SAVE_CHAR_TYPE:						/* a $# sequence was found. */
			...
			if (ret_value->optional!= -1)
			{
				...
				{
					FNX_PRINTF("par_match_string;shuffle index %d out of range",temp_hex);
					par_print_rule_error("",current_rule,rule_p);
					ret_value->rule=rule_p+1;
					ret_value->value=FATAL_FAIL;
					return(0);
				}
				length=FNX_STRLEN(match_array->array[temp_hex]);	/* get the length of the strig in the array */
				if ((memcmp(input_array+ipos,match_array->array[temp_hex],length)!=0))
				{
					...
					{
#ifdef DEBUG
						FNX_PRINTF("par_match_string; save optional set to -1\n");
#endif
						ret_value->optional= -1;
					}
				}
...
#ifdef DEBUG
	FNX_PRINTF("leaving par_match_string length = %d ret_value->value=%d\n",length,ret_value->value);
#endif
	return(length);
}
...
#ifdef DEBUG
		FNX_PRINTF("entering par_find_conditional_number\n");
#endif
	if ((current_rule==NULL))
	{
#ifdef DEBUG
			FNX_PRINTF("leaving find_contitional_number a 0\n");
#endif
		return(rule_p);
	}
	while (cond_num>0)
...
#ifdef DEBUG
	FNX_PRINTF("leaving par_find_conditional_number 1\n");
#endif
	return(rule_p);
}
...
#ifdef DEBUG
	FNX_PRINTF("entering par_find_end_of_rule\n");
#endif
	if ((current_rule==NULL))
	{
#ifdef DEBUG
		FNX_PRINTF("leaving par_find_end_of_rule a 0\n");
#endif
		return(rule_p);
	}                                               
	while (current_rule[rule_p]!=STATE_PART_DELIM)
...
#ifdef DEBUG
		FNX_PRINTF("leaving par_find_end_of_rule 1\n");
#endif
	return(rule_p);					
}
						  ...
						  preturn_value_t ret_value)
{
	int i;
#ifdef DEBUG
	FNX_PRINTF("entering par_copy_string_data\n");
#endif
...
#ifdef DEBUG
	FNX_PRINTF("leaving par_copy_string_data\n");
#endif
}
...
#ifdef DEBUG
	FNX_PRINTF("entering par_convert_hex_number\n");
#endif
	if ((*string)!='0')
		return(-1);
	if (*(string+1)!='x')
...
#ifdef DEBUG
	FNX_PRINTF("leaving par_convert_hex_number 1\n");
#endif
	return(total);
}
...
short par_get_int_length(short i)
{
	int j;
#ifdef DEBUG
		FNX_PRINTF("entering par_get_int_length\n");
#endif
	if (i==0)
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_get_int_length length=1\n");
#endif
		return(1);
	}
	for (j=0;i;j++)
		i/=10;
#ifdef DEBUG
		FNX_PRINTF("leaving par_get_int_length length=%d\n",j);
#endif
	return(j);
}
					 ...
					 preturn_value_t ret_value)
{
...
#ifdef DEBUG
		FNX_PRINTF("entering par_look_ahead\n");
	...
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_look_ahead end of string reached \n");
#endif
		return(0);
	}
	if (*find_type== -2)
		...
		while (((temp_type=par_get_char_type(current_rule[rule_p]))==0) && (current_rule[rule_p]!='\0'))
		{
			...
			if ((state=par_get_state(current_rule[rule_p]))!=0)
			{
			  if (state==DICTIONARY_STATE)
			  {
			      ...
			      {
#ifdef DEBUG
			      		FNX_PRINTF("par_look_ahead;dictionary_state dict entry found\n");
#endif
			      	return(1);
			      }
#ifdef DEBUG
			      	FNX_PRINTF("par_look_ahead;dictionary_state dict entry not found\n");
#endif
			      return(0);			   
			  }
			  if (state==MACRO_STATE)
				{
#ifdef DEBUG
				  FNX_PRINTF("macro state found, no lookahead performed\n");
#endif				  
				  *find_type= -1;
				  return(0);
				}
...
#ifdef DEBUG
								FNX_PRINTF("par_look_ahead;end of string has been found\n");
#endif
							return(0);
					    case SAVE_STATE:
	...
	if (temp_type==NO_LOOKAHEAD)
	{
		*find_type= -1;
#ifdef DEBUG
			FNX_PRINTF("lookahead disabled by the type\n");
#endif
		return(0);
	}
	...
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_look_ahead; end of the rule was reached before a type was found\n");
#endif              
		*find_type= -1;
		return(0);
	}
...
#ifdef LOOK_DEBUG
		FNX_PRINTF("current_type=%d new_find=%d\n",cur_type,new_find);
#endif
	switch(temp_type)
	{
		case EXACT_CHAR_TYPE:
			...
			i=0;
#ifdef LOOK_DEBUG
				FNX_PRINTF("matching chracters are ");
#endif
			while (current_rule[rule_p]!=EXACT_CHAR_DELIM)
			{
				if (current_rule[rule_p]==ESCAPE_DELIM)
...
#ifdef LOOK_DEBUG
					FNX_PRINTF("%c",current_rule[rule_p]);
#endif
...
#endif
#ifdef DEBUG
						FNX_PRINTF("leaving par_look_ahead exact chars failed\n");
#endif
					return(0);
				}               
				i++;
...
#endif
#ifdef DEBUG
				FNX_PRINTF("leaving par_look_ahead exact chars matched\n");
#endif
			if (i>0)
			  return(1);
			else
...
#endif
#ifdef DEBUG
						FNX_PRINTF("leaving par_look_ahead exact case chars failed\n");
#endif
					return(0);
				}               
				i++;
...
#endif
#ifdef DEBUG
				FNX_PRINTF("leaving par_look_ahead exact case chars matched\n");
#endif
			if (i>0)
			  return(1);
			else
		...
		case HEXADECIMAL_TYPE:
			...
			{
#ifdef DEBUG
					FNX_PRINTF("leaving par_look_ahead; hex number success\n");
#endif
				return(1);
			}
#ifdef DEBUG
				FNX_PRINTF("leaving par_look_ahead; hex number failure\n");
#endif
			return(0);
		case SAVE_CHAR_TYPE:						/* a $# sequence was found. */
			rule_p++;		
			...
			if ((memcmp(input_array+ipos,match_array->array[i],FNX_STRLEN(match_array->array[i]))==0))
			{
#ifdef DEBUG
					FNX_PRINTF("leaving par_look_ahead; par_save_string success\n");
#endif
...
#ifdef DEBUG
				FNX_PRINTF("leaving par_look_ahead; par_save_string failure\n");
#endif
			return(0);
		case SET_CHAR_TYPE:
			/*
			 ...
			 */
#ifdef DEBUG
				FNX_PRINTF("leaving lookahead; next type is a set \n");
#endif
			return(0);
		case DIGIT_CHAR_TYPE:
			...
			{
#ifdef DEBUG
					FNX_PRINTF("leaving par_look_ahead; non ambigous combination\n");
#endif
				return(0);
			}
			if ((current_rule[rule_p+1]=='[') || (current_rule[rule_p+2]=='['))
			{
				...
				{
#ifdef DEBUG
						FNX_PRINTF("leaving par_look_ahead;digit range success\n");
#endif
					if (char_length>0)
					  return(1);
					else
...
#ifdef DEBUG
					FNX_PRINTF("leaving par_look_ahead;digit range fail\n");
#endif
				return(0);
			}				
		case UPPER_CHAR_TYPE:
        ...
        case SOME_PUNCT_TYPE:
			...
			{
#ifdef DEBUG
					FNX_PRINTF("leaving par_look_ahead;standard success\n");
#endif
				if (char_length>0)
				  return(1);
				else
...
#ifdef DEBUG
				FNX_PRINTF("leaving par_look_ahead;standard fail\n");
#endif
			return(0);
		default:
#ifdef DEBUG
				FNX_PRINTF("leaving par_look_ahead; non recognized type\n");
#endif
			return(0);
	}             
#ifdef DEBUG
		FNX_PRINTF("leaving par_look_ahead; how did it get here??\n");
#endif
	return(0);
...
#ifdef DEBUG
  	FNX_PRINTF("entering par_lookup_ambiguous\n");
#endif
  if (!from_reverse)
	{
	  bit_to_check<<=2;
...
#ifdef DEBUG
  	FNX_PRINTF("leaving par_lookup_ambiguous with %02X\n",ambiguous_char[cur_type][new_type] & bit_to_check);
#endif
							...
							preturn_value_t ret_value)
{
...
#ifdef DEBUG
		FNX_PRINTF("entering par_copy_word_to_output\n");
#endif
	if ((ret_value==NULL) || (input_array==NULL) || (output_array==NULL))
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_copy_word_to_output a 0\n");
#endif
		return(0);
	}                      
	ipos=ret_value->input_pos+ret_value->input_offset;
...
#ifdef DEBUG
			FNX_PRINTF("output_array[%d]=%c ",opos+i,output_array[opos+i]);
#endif
		i++;
	}
	...
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_copy_word_to_output b -1\n");
#endif
		return(-1);
	}
#ifdef DEBUG
		FNX_PRINTF("leaving par_copy_word_to_output b \n");
#endif
	return(0);
}
...
void par_skip_states(char *rule,preturn_value_t ret_value,short state,char end)
{
...
#ifdef DEBUG
	FNX_PRINTF("entering par_skip_states\n");
#endif
	if (state==NULL_STATE)
	{
		end='\0';
	...
	if (state==SAVE_STATE)
	{
		...
		if ((rule[(ret_value->rule)]<'0') || (rule[(ret_value->rule)]>'9'))
		{
			...
			ret_value->value=FATAL_FAIL;
#ifdef DEBUG
				FNX_PRINTF("leaving par_skip_states\n");
#endif
			return;
		}
		end=')';
	...
	else
	{
		while (rule[(ret_value->rule)]!=end)
		{
			...
			{
#ifdef DEBUG
					FNX_PRINTF("leaving par_skip_states syntax error in the rule\n");
#endif
				return;
			}
		}
...
#ifdef DEBUG
		FNX_PRINTF("leaving par_skip_states, action dome \n");
#endif
}
...
{
#ifdef DEBUG
	FNX_PRINTF("entering par_skip_action\n");
#endif
	switch(state)
	{
		case	COPY_STATE:
		...
		case DICTIONARY_STATE:
			...
			else
			{
				...
				if (rule[(ret_value->rule)]=='\0')
				{
					...
					ret_value->value=FATAL_FAIL;
#ifdef DEBUG
						FNX_PRINTF("leaving par_skip_action FAIL\n");
#endif
					return;
				}
			}
...
#ifdef DEBUG
		FNX_PRINTF("leaving par_skip_action normal\n");
#endif	
}            
...
{
#ifdef DEBUG
	FNX_PRINTF("entering par_skip_string\n");
#endif
	switch(char_type)
	{
		case	EXACT_CHAR_TYPE:
...
#ifdef DEBUG
	FNX_PRINTF("leaving par_skiip_string\n");
#endif
}
						 ...
						 preturn_value_t ret_value)
{
...
#ifdef DEBUG
		FNX_PRINTF("entering par_skip_white_space\n");
#endif
	if ((ret_value==NULL) || (input_array==NULL) || (output_array==NULL))
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_skip_white_space a 0\n");
#endif
		return(0);
	}
	ipos=ret_value->input_pos+ret_value->input_offset;
	...
	while (((parser_char_types[(int)(input_array[ipos+i])] & TYPE_white)!=0) && (input_array[ipos+i]!='\0'))
	{
		if (i==0 || par_is_index_set(input_indexes,ipos+i))
		{
			...
			{                                       
#ifdef INDEX_DEBUG
					FNX_PRINTF("copying index from %d to %d\n",ipos+i,opos+j);
#endif
			...
			else
			{
				...
				if (i!=0)
				{
					FNX_PRINTF("copied extra space because of index\n");
				}
				FNX_PRINTF("output_array[%d]=%c\n",opos+i,output_array[opos+i]);
#endif
		}
		i++;
	}
	...
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_skip_white_space b -1\n");
#endif
		return(-1);
	}
#ifdef DEBUG
		FNX_PRINTF("leaving par_skip_white_space b 0\n");
#endif
	return(0);
}
						  ...
						  int pos)
{
	int i;
	FNX_PRINTF("%s\n",message);
	FNX_PRINTF("error in rule at position %d\n",pos);
	FNX_PRINTF("rule, %s\n",current_rule);
	FNX_PRINTF("      ");
	for (i=0;i<pos;i++)
	{
					   ...
					   int break_on_min_match)
{
...
#ifdef DEBUG
		FNX_PRINTF("entering par_match_standard\n");
#endif
	if (ret_value==NULL)
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_match_standard a 0\n");
#endif
		return(0);
	}
	if ((current_rule==NULL) || (input_array==NULL))
	{
		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
			FNX_PRINTF("leaving par_match_standard b 0\n");
#endif
		return(0);
	}
	rule_p=ret_value->rule;
	...
	{
#ifdef DEBUG
			FNX_PRINTF("match_standard set no lookahead\n");
#endif
		lookahead=0;
		rule_p++;
	}
	...
	{
#ifdef DEBUG
			FNX_PRINTF("match_standard set to non matches\n");
#endif
		match_non_match=0;
		rule_p++;
	}
...
#ifdef DEBUG
			FNX_PRINTF("par_match_standard;rule_p=%d\n",rule_p);
#endif
		if (end_of_type!=2)
			rule_p++;	
	}	/* while loop inner */
	...
	if (satisfied_min_cond== -1)
   	{                                               
		...
		{
#ifdef DEBUG
				FNX_PRINTF("par_match_standard; optional set to -1\n");
#endif
			ret_value->optional= -1;
		}
	   	ret_value->value=FAIL;
	...
	ret_value->rule=rule_p;
#ifdef DEBUG
		FNX_PRINTF("leaving par_match_standard length=%d ret_value->value=%d\n",length,ret_value->value);
#endif
	return(length);
}
					 ...
					 int break_on_min_match)
{
...
#ifdef DEBUG
		FNX_PRINTF("entering par_match_digits\n");
#endif                      
	if (ret_value==NULL)
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_match_digits a 0\n");
#endif
		return(0);
	}
	if ((current_rule==NULL) || (input_array==NULL) ||
		(range_value==NULL))
	{
		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
			FNX_PRINTF("leaving par_match_digits b 0\n");
#endif
		return(0);
	}
	rule_p=ret_value->rule;
	...
	{
#ifdef DEBUG
			FNX_PRINTF("match_digits set no lookahead\n");
#endif
		lookahead=0;
		rule_p++;
	}
	...
	{      
		while ((current_rule[rule_p]!=']') && (match_is_over==0) && (par_look_ahead_success==0) && (end_of_type==0))
		{
			...
			if (ret_value->optional!= -1)                
			{
...
#ifdef DIGIT_DEBUG
					FNX_PRINTF("par_match_digits; before match length=%d match_is_over=%d satisfied_min_cond=%d\n",length,match_is_over,satisfied_min_cond);
					FNX_PRINTF("par_match_digits; before match min_range=%d max_range=%d\n",min_range,max_range);
#endif
				...
				{
#ifdef DIGIT_DEBUG
						FNX_PRINTF("looping in par_match_digits i=%d temp_num=%d\n",i,temp_num);
#endif
					if (temp_num>=range_value->min)
					{
#ifdef DIGIT_DEBUG
							FNX_PRINTF("matched %d\n",temp_num);
#endif
						length=i+1;
...
#ifdef DIGIT_DEBUG
			FNX_PRINTF("take 2 length=%d match_is_over=%d satisfied_min_cond=%d\n",length,match_is_over,satisfied_min_cond);
			FNX_PRINTF("take 2 min_range=%d max_range=%d\n",min_range,max_range);
#endif
#ifdef DEBUG
			FNX_PRINTF("par_match_digits;rule_p=%d\n",rule_p);
#endif
	...
	/* if satisfied_min_cond was not changed, there was no successful matching */
   	if (satisfied_min_cond== -1)
   	{
   		...
   		{
#ifdef DEBUG
				FNX_PRINTF("par_match_digits; optional set to -1\n");
#endif
			ret_value->optional= -1;
		}
   		ret_value->value=FAIL;
...
#endif
#ifdef DEBUG
		FNX_PRINTF("leaving par_match_digits length=%d\n",length);
#else
#ifdef DIGIT_DEBUG
			FNX_PRINTF("leaving par_match_digits length=%d ret_value->value=%d\n",length,ret_value->value);
#endif
#endif
   	return(length);
}
...
/* int find_type= -2; */ /* uncomment when look_ahead is implimented from sets */
...
#ifdef DEBUG
		FNX_PRINTF("entering par_match_sets_and_ranges\n");
#endif
	if (ret_value==NULL)
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_match_sets_and_ranges a 0\n");
#endif
		return(0);
	}
	if ((current_rule==NULL) || (input_array==NULL))
	{
		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
			FNX_PRINTF("leaving par_match_sets_and_ranges b 0\n");
#endif
		return(0);
	}
	rule_p=ret_value->rule;
					...
					if (length==0 && new_ret.value==SUCCESS)
					  {
						if (total_length==0)
						  {
#ifdef DEBUG
							FNX_PRINTF("set satisfied_min_cond to -2\n");
#endif
							satisfied_min_cond= -2;
							match_is_over=1;
							break;
...
#ifdef DEBUG
			FNX_PRINTF("par_match_sets_and_ranges;rule_p=%d\n",rule_p);
#endif
		if (end_of_type!=2)
			rule_p++;	
	}	/* while loop inner */
   	...
   	if (current_rule[rule_p]=='>')
	{
   	...
   	{                                               
   		...
   		{
#ifdef DEBUG
				FNX_PRINTF("par_match_sets_and_ranges; optional set to -1\n");
#endif
			ret_value->optional= -1;
		}
	   	ret_value->value=FAIL;
	...
	ret_value->rule=rule_p;
#ifdef DEBUG
	FNX_PRINTF("leaving par_match_sets_and_ranges length=%d total_length=%d ret_value->value=%d\n",length,total_length,ret_value->value);
#endif
   	return(total_length);
}
					...
					int lookahead)
{
...
#ifdef DEBUG
		FNX_PRINTF("entering par_match_set\n");
#endif
	if (ret_value==NULL)
	{
#ifdef DEBUG
			FNX_PRINTF("leaving par_match_set ret_value is NULL\n");
#endif
		return(0);
	}
	if ((current_rule==NULL) || (input_array==NULL) || 
		(match_array==NULL) || (range_value==NULL))
	{
		ret_value->value=FATAL_FAIL;
#ifdef DEBUG
			FNX_PRINTF("leaving par_match_set, inputs are bad\n");
#endif
		return(0);
	}
	new_ret.rule=save_p;
	...
	while ((current_rule[new_ret.rule]!='}') && (current_rule[new_ret.rule]!='\0') && (a_success==FAIL))
	{
		...
		while ((current_rule[new_ret.rule]!=',') && (current_rule[new_ret.rule]!='}') && (current_rule[new_ret.rule]!='\0') && (a_success==FAIL))
		{
				...
				if (num_chars_matched== -1)
				{
					...
					else
					{
						if (current_rule[new_ret.rule]=='}')
						{
							ret_value->value=END_OF_STRING;                         
#ifdef DEBUG
						  	FNX_PRINTF("leaving par_match_set; end of string reached in string\n");
#endif
							return(-1);
					  	}
					}
			...
			{
#ifdef DEBUG
					FNX_PRINTF("par_match_set;this section has failed, look for the next section\n");
#endif
				this_success=FAIL;
				while ((current_rule[new_ret.rule]!=',') && (current_rule[new_ret.rule]!='}') && (current_rule[new_ret.rule]!='\0'))
				{
			...
			{
#ifdef DEBUG
					FNX_PRINTF("this type has succeeded, matching next type\n");
#endif
			}
		}    
#ifdef DEBUG
		...
		if (this_success==SUCCESS)
		{
			a_success=SUCCESS;
#ifdef DEBUG
				FNX_PRINTF("par_match_set;this section has succeeded\n");
#endif
			if (range_value->range_set==0)
			{
		...
		else
		{
			...
			{
#ifdef DEBUG
					FNX_PRINTF("par_match_set;all sections failed\n");
#endif
			}
			else
			{
#ifdef DEBUG
					FNX_PRINTF("par_match_set;this section has failed, going to next section\n");
#endif
				rule_p=new_ret.rule;
				par_copy_return_value(&new_ret,&save_ret);
				num_match++;
	...
	if (a_success==FAIL)
	{
		ret_value->value=FAIL;
#ifdef DEBUG
			FNX_PRINTF("par_match_set; it failed\n");
#endif
	}    
#ifdef DEBUG
		FNX_PRINTF("leaving par_match_set length=%d ret_value->value=%d\n",length,ret_value->value);
#endif
	return(length);
}	
