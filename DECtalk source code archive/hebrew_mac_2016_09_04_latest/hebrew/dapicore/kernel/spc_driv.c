 ...
 */
	...
	spc_pkt = FNX_MALLOC(SPC_PACKET_POOL);
	...
	dgt_pkt = FNX_MALLOC(DGT_PACKET_POOL);
...
#ifdef DEBUG
	  	FNX_PRINTF("dma count=%d ",((*dgt_pkt).length));
		for(cnt=initcnt;((*dgt_pkt).length) >= cnt;cnt=cnt+18)
			{
			for(i=0;i<=10;i++)
				{
				FNX_PRINTF(" bd=%d ",(*dgt_pkt).data[i]);
				}
			if ((*dgt_pkt).data[cnt] !=  0 )
				{
				FNX_PRINTF(" n=%d pos=%d",(*dgt_pkt).data[cnt],cnt);
				}
			if(j>=10)
				{
				j=0;
...
_far spc_task()
{
	...
	if (KS.l_verbose)
		FNX_PRINTF("SPC Task starting. SS:%04x DS:%04x ES:%04x\n",
...
#ifdef DEBUG
	FNX_PRINTF("spc_task:DSP reset.\n");
#endif
	last_pkt = wait_queue(&KS.spc_pool);
	while(true)
		{
#ifdef DEBUG
		FNX_PRINTF("spc_task: wait_queue(%08x)\n",&KS.spc_active);
#endif
		spc_pkt = wait_queue(&KS.spc_active);
#ifdef DEBUG
		{
 ...
 */
		...
		if (read_gpio()&0x80)
			...
			FNX_PRINTF("spc_task: SPC error! (%04x) ",
				_inpw(0x2e0));
			_outp(0xd8,0); //clear ff
			sitmp = _inp(0xcc);
			sitmp |= (_inp(0xcc))<<8;
			FNX_PRINTF(" curadr = %04x ",sitmp);
			sitmp = _inp(0xce);
			sitmp |= (_inp(0xce))<<8;
			FNX_PRINTF(" cnt=%04x\n",sitmp);
			slflush();
...
#ifdef DEBUG
		FNX_PRINTF("spc_task: wait_semaphore(%08x) (spc_dma)\n",
			&KS.spc_dma);
#endif
		...
		if (foo != 0xFF)
			{
			FNX_PRINTF("spc_task: DMA not done?? count=0x%04x\n",
				foo);
			}
...
#ifdef DEBUG
		FNX_PRINTF("spc_task: packet at [%04x][%04x]\n",
		...
		/* and let it go. */
#ifdef DEBUG
		FNX_PRINTF("spc_task: DMA starting: %04x --> %04x\n",
			DMA_SINGLE_MASK, 3);
		slflush();
#endif
...
_far index_task()
{
	...
	if (KS.l_verbose)
		FNX_PRINTF("Index Task starting. SS:%04x DS:%04x ES:%04x\n",
		get__ss(),get__ds(),get__es());
...
_far dgt_task()
{
	...
	if (KS.l_verbose)
		FNX_PRINTF("Dgt Task starting. SS:%04x DS:%04x ES:%04x\n",
		get__ss(),get__ds(),get__es());
