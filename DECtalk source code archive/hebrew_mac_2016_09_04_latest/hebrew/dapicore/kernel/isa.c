...
void _far module_check()
{
	...
	while(true)
		...
		if ((KS.sleep_interval) && (KS.idleseconds >= KS.sleep_interval))
			...
			dsp_sleep();
#ifdef DEBUG
			FNX_PRINTF("[:rem sleeping]\n");
#endif
			}
		  else
			sysclk_on_off(1);
#ifdef DEBUG
		FNX_PRINTF("[idlesec:%d flag:%01X]",
			KS.idleseconds,KS.idleflag);
		if (KS.spc_sleeping) FNX_PRINTF("[sleeping]");
		if (KS.spc_waking) FNX_PRINTF("[waking]");
		{
			...
			_outp(0xEC,oldptr);
			kernel_enable(flags);
			FNX_PRINTF("[slpctl=%02X oldptr=%02x]",temp,oldptr);
		}
#endif
