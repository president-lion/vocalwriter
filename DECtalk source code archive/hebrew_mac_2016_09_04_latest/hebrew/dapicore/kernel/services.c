...
struct spc_packet *get_spc_packet(void)
{
	...
	if (gp->cur_packet_number==0)
	{
		return(&(gp->global_spc_pkt[0]));
		gp->cur_packet_number=1;
	}
	else
	{
		return NULL;
...
void free_spc_packet(struct spc_packet *spc_pkt)
{
	...
	if (spc_pkt==&(gp->global_spc_pkt[0]))
	{
...
//#endif
  ...
  if ( spc_pkt == NULL_SPC_PACKET )
  {
	...
	if (spc_pkt==NULL)
		return;
#else
	spc_pkt = (struct spc_packet *)FNX_MALLOC(sizeof(struct spc_packet));
#endif
...
//printf("save index sym %d value %d how %d\n",sym, value, how);
//#ifdef WIN32
  /* tek 6mar97 bats 278 this is protected by a critial section */
#ifndef ARM7
...
//#ifdef WIN32
	...
	case INDEX_REPLACE_START:
		buf[0] = SPC_type_index	| SPC_subtype_replace_start;
		break;
	case INDEX_REPLACE_END:
		buf[0] = SPC_type_index	| SPC_subtype_replace_end;
		break;
//#endif
	} // switch
...
//printf( "check index %d %d %d \n",buf[1], spc_pkt->data[4],spc_pkt->data[5]);
...
#endif
//#endif
//printf("which %d dir %d del %d\n",which, direction, del);
  if(( spc_pkt = pKsd_t->spc_pkt_save ) != NULL_SPC_PACKET )
  {
	while(spc_pkt != NULL_SPC_PACKET)
	{
	  if ( (int)(spc_pkt->data[5]) >= ((int)which)+(int)spc_pkt->data[6] )
	  {
//printf("adj %d nphone %d which %d total del %d dir %d del %d\n",spc_pkt->data[5], spc_pkt->data[4], which, (int)spc_pkt->data[6], direction, del);
	spc_pkt->data[4] = (unsigned int)((int)(spc_pkt->data[4]) + direction);
	spc_pkt->data[6] = (unsigned int)((int)(spc_pkt->data[6]) + del);
	  }
	  else
	  {
		//printf("nad %d nphone %d which %d total del %d dir %d del %d\n",spc_pkt->data[5], spc_pkt->data[4], which, (int)spc_pkt->data[6], direction, del);
	  }
	  spc_pkt = spc_pkt->link;
...
//#endif
  ...
  if((spc_pkt = pKsd_t->spc_pkt_save ) != NULL_SPC_PACKET )
  {
	while(spc_pkt != NULL_SPC_PACKET)
	{
	  ...
	  {
		  //printf("adjust index allo which %d pos %d direction %d 4=%d\n",which, spc_pkt->data[5], direction,spc_pkt->data[4]);
...
//#endif
  ...
  if (( spc_pkt = pKsd_t->spc_pkt_save ) != NULL_SPC_PACKET )
  {
	while( spc_pkt != NULL_SPC_PACKET )
	{
	  ...
	  {
		  //printf("st index allo nphone %d nallo %d\n",nphone, nallo);
...
#ifdef SERVDEBUG
FNX_SPRINTF(tmp, "serv: In default lang %d ready code: %d\n", lang_code, ready_code);
f_fprintf(tmp);
#endif
...
#ifdef SERVDEBUG
FNX_SPRINTF(tmp, "serv: pKsd_t->lang_ready %d \n", pKsd_t->lang_ready[lang_code]);
f_fprintf(tmp);
#endif
...
#if defined _UNIX_LIKE_ || defined ARM7
const static int dwVolumeTable[MAX_VOLUME+1] =
{
...
#ifdef WIN32
const static dwVolumeTable[MAX_VOLUME+1] =
{
