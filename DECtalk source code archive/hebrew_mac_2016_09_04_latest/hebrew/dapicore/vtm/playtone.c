...
#else
  pToneBuffer = (short *)FNX_MALLOC( MAX_TONE_BLOCK * sizeof(short));
#endif
  if ( pToneBuffer == NULL )
  {
    return( TRUE );
...
#else
  pRiseBuffer = (float *)FNX_MALLOC( iRiseSamples * sizeof(float));
#endif
...
#if !defined (ARM7) || defined(OS_SYMBIAN)
  FNX_FREE( pRiseBuffer );
  FNX_FREE( pToneBuffer );
#endif
