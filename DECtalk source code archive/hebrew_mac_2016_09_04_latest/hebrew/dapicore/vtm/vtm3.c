...
//#if defined __linux__ || defined _APPLE_MAC_
...
const int int_volume_table[161]=
{
0,
1071,
1110,
...
void setzeroabc(int f, int bw, int rnpg, int *sacoef,short *sbcoef,short *sccoef);//EAB 3/298 new coef for rnz
...
{	
	...
	int doublet[8]={ 000,000,-12000,-12000,12000,12000,000,00};
#endif
...
#define HLSYN_NEWPOLE
	...
	S16 BTPinHZ;
#endif
	S16 B1inHZ=90;  /*  First formant bw in Hz,           40 to 1000 */
	S16 B2inHZ=120;  /*  Second formant bw in Hz,          40 to 1000 */
	S16 B3inHZ=180;  /*  Third formant bw in Hz,           40 to 1000 */
...
#ifdef HLSYN_NEWPOLE
	FZ2_MINinHZ=  pVtm_t->glotal_zero;
	FZ2inHZ = FP2inHZ=  pVtm_t->glotal_pole;
#endif
	/********************************************************************/
	/********************************************************************/
...
//	F3inHZ = frac4mul( F3inHZ, pVtm_t->fnscal );
	...
	FZ2inHZ = FZ2inHZ  -((FZ2inHZ  -FZ2_MINinHZ)*tmp/100);
	BZ2inHZ = 300+(300*tmp/100);
	...
	B1inHZ = (variabpars[OUT_B1]+B1inHZ)>>1;
	B2inHZ = (variabpars[OUT_B2]+B2inHZ)>>1;
	B3inHZ = (variabpars[OUT_B3]+B3inHZ)>>1;
	...
	//FNX_PRINTF("F2inHZ %d B2inHZ %d   %d\n", F2inHZ, B2inHZ,pVtm_t->R2ca);
#ifndef NEW_TILT
	TILTDB = variabpars[OUT_TLT]-12;
//	printf(" tilt %d \n", TILTDB);
...
#endif // NEW_TILT
	if ( pKsd_t->uiSampleRate == 8000 || pVtm_t->R5pb==0) 
	// if no 5th formant then we use the funny a6
	{
		if(A5inDB > 40)
		{
			A6inDB = A5inDB;
	...
	abreth = variabpars[OUT_BR];
	abreth = amptable[abreth];
...
//	FNX_PRINTF("APlin %d \n",APlin);
	...
	r6pa = amptable[A6inDB];  /*  Convert dB to linear */
	...
	Diplo =  variabpars[OUT_DP];

	APlin = frac4mul( APlin, pVtm_t->APgain );  

	/*  Scale asp by spdef GV  */

//	FNX_PRINTF("aplin %d %d\n",APlin,pVtm_t->APgain);
	if (variabpars[OUT_GF] <= 0)
	{
		...
		pVtm_t->AFcgain = variabpars[OUT_GF];
	  ...
	  noisef = pVtm_t->randomx >> 2;
		...
		for ( nsr4 = 0; nsr4 < 4; nsr4++ )
		{
			...
			/****************************************************************/
				...
				pVtm_t->avlind = pVtm_t->avlin; 
#ifdef STEVE_HAWKINGSout
	//		if ( pVtm_t->nper > ( pVtm_t->T0 - 8 ))//if ( pVtm_t->nper < 3)
	//		{
	//		pVtm_t->voice0 = doublet[pVtm_t->T0 - pVtm_t->nper];
			if ( pVtm_t->nper > ( pVtm_t->T0 - pVtm_t->nopen ))
			{
				
				if (pVtm_t->nper < (( pVtm_t->T0 - pVtm_t->nopen )+pVtm_t->nfirsthalf) )
				{
                pVtm_t->voice0 += pVtm_t->slopet1;
               if (pVtm_t->vwave > pVtm_t->maxt1)
				   pVtm_t->voice0 = pVtm_t->maxt1;
            }
            else {
                pVtm_t->voice0 -= pVtm_t->slopet2;
                if (pVtm_t->vwave > pVtm_t->maxt2)
				   pVtm_t->voice0 = pVtm_t->maxt2;
            }
		//	printf(" %d \n ", pVtm_t->voice0);
#else
			if ( pVtm_t->nper > ( pVtm_t->T0 - pVtm_t->nopen ))

			{
...
#endif				
#ifdef NEW_GLOTTIS_reiner
				float E0=7000.0;
				float Tc=(float)pVtm_t->nopen;
				float Te=Tc-10.0;
				float Ta=5.0;
				float Tp=Te-8.0;

				float t=(float)(pVtm_t->nper-pVtm_t->topen);
				float ga1=E0/(Te*Te*(Te-Tp));
				float gb1=E0/Ta;
				float gb2=(E0*Ta-2*E0*Tc)/(Ta*Tc*Tc);
				float gb3=(E0*Tc-2*E0*Ta)/(Ta*Tc*Tc*Tc);
				float temp_g;
				if (t < Te)
				{
					temp_g=ga1*(Tp-t)*t*t;
				...
				{
					temp_g=-E0+gb1*(t-Te)+gb2*(t-Te)*(t-Te)+gb3*(t-Te)*(t-Te)*(t-Te);
				}


				pVtm_t->voice0=(int)(temp_g);
#endif
			...
			/****************************************************************/
			///This was changed to >= to accomaindate hlsyn in the past but it can create
			//abberations on statup. 
//			if(variabpars[OUT_PH] != GEN_SIL)
//			FNX_PRINTF("NPER %d t0 %d avindb %d  %d\n", pVtm_t->nper, pVtm_t->T0,AVinDB,variabpars[OUT_PH]);
			if((pVtm_t->lastavindb == 0 && AVinDB > 0)
			|| ( pVtm_t->nper == pVtm_t->T0 ))
			{
				/**************************************************************/
				/*  Reset period when 'nper' reaches T0, glottis about to     */
				/*  open.                                                     */
				/**************************************************************/
				pVtm_t->lastavindb = 1; //to not stay in loop				
				pVtm_t->nper = 0;
	    		...
	    		else
				{
					...
					{
		    			temp = pVtm_t->T0 - pVtm_t->nopen ;  /* Dur of closed phase in samples*4 */
		    			if (temp < 1)    temp = 1;
				...
				if ( pVtm_t->nopen < 10 )
					...
					if ( pVtm_t->nopen > (263>>1) )
						pVtm_t->nopen = (263>>1);     /*  Max is 263 */
				...
				pVtm_t->as = 90;
				pVtm_t->assym = (pVtm_t->nopen*(pVtm_t->as-50))/100;  /* as=50 is symmetrical  CHECK */
                pVtm_t->nfirsthalf = (pVtm_t->nopen>>1) + pVtm_t->assym;
                if (pVtm_t->nfirsthalf >= pVtm_t->nopen) 
					pVtm_t->nfirsthalf = pVtm_t->nopen -1;
                if (pVtm_t->nfirsthalf <= 0)   
					pVtm_t->nfirsthalf = 1;
                pVtm_t->nsecondhalf = pVtm_t->nopen - pVtm_t->nfirsthalf;
                pVtm_t->Afinal = -12000.;
                pVtm_t->maxt2 = pVtm_t->Afinal;// * 0.25;
                pVtm_t->slopet2 = pVtm_t->Afinal / pVtm_t->nsecondhalf;
              //  pVtm_t->vwave = -(pVtm_t->Afinal * pVtm_t->nsecondhalf) / pVtm_t->nfirsthalf;   /* CHECK */
                pVtm_t->maxt1 = pVtm_t->vwave; // * 0.25;
                pVtm_t->slopet1 = /*- pVtm_t->vwave*/pVtm_t->Afinal / pVtm_t->nfirsthalf;
				...
				/**************************************************************/
				  ...
				  pVtm_t->rnpa = d2pole_cf123( pVtm_t,pKsd_t, &pVtm_t->rnpb, &pVtm_t->rnpc,FNPinHZ , BNPinHZ ,18596 );
#endif

#if defined HLSYN_NEWPOLE && defined HLSYN
				 pVtm_t->rnp2a = d2pole_cf123( pVtm_t,pKsd_t, &pVtm_t->rnp2b, &pVtm_t->rnp2c,FP2inHZ ,BP2inHZ ,5606 );
	//			 pVtm_t->rnp2a = d2pole_cf123( pVtm_t,pKsd_t, &pVtm_t->rnp2b, &pVtm_t->rnp2c,400 ,80 ,1606 );
#endif
				...
				if ( pVtm_t->R1ca > 16383 )
					...
					if(pVtm_t->R2ca < 0)
						pVtm_t->R2ca++;
					//FNX_PRINTF("next F2inHZ %d B2inHZ %d   %d\n", F2inHZ, B2inHZ,pVtm_t->R2ca);
					   //d2pole_cf45( pVtm_t,&pVtm_t->R4cb, &pVtm_t->R4cc, F4c, B4c, 0);
...
//      }
...
#else
				if(variabpars[OUT_SEX] ==0)
				setzeroabc( FZinHZ,BZinHZ,700, &pVtm_t->rnza, &pVtm_t->rnzb, &pVtm_t->rnzc);
				else
				setzeroabc( FZinHZ,BZinHZ,300, &pVtm_t->rnza, &pVtm_t->rnzb, &pVtm_t->rnzc);
#endif
				 pVtm_t->lastFZinHZ= FZinHZ;
...
#endif
#if defined HLSYN_NEWPOLE && defined HLSYN				
		 	setzeroabc( FZ2inHZ,BZ2inHZ, 3000, &pVtm_t->rnz2a, &pVtm_t->rnz2b, &pVtm_t->rnz2c);
			//setzeroabc( 430,BZ2inHZ, 1000, &pVtm_t->rnz2a, &pVtm_t->rnz2b, &pVtm_t->rnz2c);
#endif
	...
	//	if(voice != 0)
	//	printf(" %d \n ", voice);
		if ( (pVtm_t->avlind + pVtm_t->avlin > 30) ||  APlin  > 30)
			pVtm_t->cas_count = 80;
		else
			if(pVtm_t->cas_count)
				pVtm_t->cas_count--;
	...
	//	pVtm_t->iwave[ns] = voice;
		...
		/******************************************************************/
		//code change to recognize that very breathy voices interchange aspir and breathiness
		//though the  mechanism for aspiration may or may not be the same
		//if(pVtm_t->aturb1 > 0)
		//{
	//		temp = APlin-pVtm_t->aturb1;
//			if(temp <0)
//				temp = APlin;
//		}
//		else
//			temp = APlin;
		if(pVtm_t->avlind >0)
		{
			temp1 = frac4mul( pVtm_t->avlind, pVtm_t->aturb1); 
		voice += frac4mul( (temp1>>2), noiseb );
...
skip_cascade:
		...
		two_pole_filter( noisef+impulse, pVtm_t->r6pd1, pVtm_t->r6pd2, r6pa,pVtm_t->R6pb, pVtm_t->r6pc );
...
skip_parallel:
		...
		*/
		...
		if(pVtm_t->bDoTuning)
		{
			...
			getmax(pVtm_t->r6pd1, &pVtm_t->vtdTuneResults.r6pd1);
			getmax(tempout, &pVtm_t->vtdTuneResults.iwave);
			...
			if (checkmax(pVtm_t->voicemark, MAX_FRIC_TRIGGER) ||
				checkmax(pVtm_t->rnzd1, MAX_TRIGGER) ||
				checkmax(pVtm_t->rnpd1, MAX_TRIGGER) ||
				checkmax(pVtm_t->r6pd1, MAX_FRIC_TRIGGER) ||
				checkmax(pVtm_t->r5pd1, MAX_FRIC_TRIGGER) ||
				checkmax(pVtm_t->r4pd1, MAX_FRIC_TRIGGER) ||
				checkmax(pVtm_t->r3pd1, MAX_FRIC_TRIGGER) ||
				checkmax(pVtm_t->r2pd1, MAX_FRIC_TRIGGER) ||
				...
				checkmax(tempout, 32000))
			{
				...
				pVtm_t->vtdTuneResults.r5pd1=pVtm_t->r5pd1;
				pVtm_t->vtdTuneResults.r6pd1=pVtm_t->r6pd1;
				pVtm_t->vtdTuneResults.iwave=tempout;
				...
				if (phTTS->hTuneStopEvent!=NULL)
				{
					...
					OP_Sleep(0);
					OP_WaitForEvent(phTTS->hTuneContinueEvent,OP_INFINITE);
				}
			}
		} // if(pVtm_t->bDoTuning)		
#endif
	} // for ( nsr4 = 0; nsr4 < 4; nsr4++ )
	pVtm_t->lastavindb = AVinDB;
	return;
}
...
void read_speaker_definition(LPTTS_HANDLE_T phTTS) 
{
	...
	*/
	...
	//eab found that mvp had hard coded these which is wrong
	//eab 9/18/04
 if ( pVtm_t->uiSampleRateChange == SAMPLE_RATE_INCREASE )
		f6p = 5100;
	else  if ( pVtm_t->uiSampleRateChange == SAMPLE_RATE_DECREASE )
		f6p = 4300;
	else
		f6p = 4400;

	d2pole_pf( pVtm_t,&pVtm_t->R6pb, &pVtm_t->r6pc, f6p, 500, 0 );
...
void InitializeVTM(LPTTS_HANDLE_T phTTS)
{
	...
	pVtm_t->lastf1 = 500;
	pVtm_t->lastfnp =500;
	pVtm_t->T0 = 100; //force firat update.
 ...
 *	Comments:
 * *****************************************************************/
void setzeroabc(int f,int bw,int rnzg, int *sacoef,short *sbcoef,short *sccoef)
{
	...
	*sccoef = -((ccoef*rnzg)/acoef);
	...
	/* Debugging printout *
       FNX_PRINTF("fz=%3d bw=%3d acoef=%8.5f bcoef=%8.5f ccoef=%8.5f\n",
