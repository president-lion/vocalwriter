                 ...
                 S16 gain )
{
  ...
  {
    frequency = (short)frac1mul( pVtm_t->inv_rate_scale, frequency ) << 1;
    bandwidth = (short)frac1mul( pVtm_t->inv_rate_scale, bandwidth ) << 1;
  }
  else
  {
    if ( pVtm_t->uiSampleRateChange == SAMPLE_RATE_INCREASE )
    {
      frequency = (short)frac1mul( pVtm_t->inv_rate_scale, frequency );
      bandwidth = (short)frac1mul( pVtm_t->inv_rate_scale, bandwidth );
    }
  }
...

  {
    ...
    *bcoef = (short)frac4mul( radius, cosine_table[ frequency >> 3 ] );

    /******************************************************************/
    /*  Let ccoef = - r^2                                             */
    /******************************************************************/

    *ccoef = - (short)frac4mul( radius, radius );
  }
