...

{
  ...
  *pFNZ = fn * (float) DTsqrt((1.f + MmOverMn) 
  ...
  if(!( (fn > *pFNZ && fm < *pFNZ) 
    ...
    FNX_PRINTF("FNZ must be between fn and fm in NasalZero.");
    exit(1);
  }

  if(fn >= fm){
    FNX_PRINTF("fn should be less than fm.");
    exit(1);
  }
#endif
  /*****
...

{
  ...
  *****/
  ...
  if(Column2Set==NO){
#ifdef DEBUG
    FNX_PRINTF(" InterpolateTable finished without a value.");
    exit(1);
#endif
  }
...
#ifdef DEBUG
  if( (x2-x1) == 0.0f ){
    FNX_PRINTF(" Cannot linear interpolate equal x values.");
    exit(1);
  }
#endif
...
#ifdef DEBUG
    FNX_PRINTF("Zero division in NasalFirstFormant.\n");
    exit(1);
#endif
    }
  else
...

{
  ...
  *****/
  ...
  if(FNPvars.fp==FNPvars.fn){
    FNX_PRINTF("fp must not equal fn in NasalPole.");
    exit(1);
  }
#endif
...
#ifdef DEBUG
      FNX_PRINTF("Unable to bracket nasal pole");
      exit(1);
#endif
    }
  }
...

{
  ...
  if(FNPvars->fn>=FNPvars->fp){
    FNX_PRINTF("fn must be less than fp in FiniteBracketFNP");
    exit(1);
  }
#endif
