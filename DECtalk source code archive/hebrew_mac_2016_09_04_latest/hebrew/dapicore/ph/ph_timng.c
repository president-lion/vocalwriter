...
void init_timing (LPTTS_HANDLE_T phTTS);
...
void prphdurs (PDPH_T pDph_t)
{
	...
	for (n = 0; n < pDph_t->nallotot; n++)
	{
		...
		/* if (pDph_t->f0mode == HAT_F0_SIZES_SPECIFIED) prf0 = 0; FNX_PRINTF("\n\t%s\  4d  %4d ", */
		FNX_PRINTF ("allo=%d allodur %d\n", pDph_t->allophons[n], (((pDph_t->allodurs[n] * NSAMP_FRAME) + 5) / 10));
...
//extern const short fr_mindur[];
	...
	if(tmp == PFUSA<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFUSA]->mindur[phone & PVALUE]);
	}
	else if(tmp == PFUK<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFUK]->mindur[phone & PVALUE]);
	}
	else if(tmp == PFGR<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFGR]->mindur[phone & PVALUE]);
	}
	else if(tmp == PFLA<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFLA]->mindur[phone & PVALUE]);
	}
	else if(tmp == PFSP<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFSP]->mindur[phone & PVALUE]);
	}
	else if(tmp == PFFR<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFFR]->mindur[phone & PVALUE]);
	}
	else if(tmp == PFIT<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFIT]->mindur[phone & PVALUE]);
	}
	else if(tmp == PFJP<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFJP]->mindur[phone & PVALUE]);
	}
	else if(tmp == PFCH<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFCH]->mindur[phone & PVALUE]);
	}
	else if(tmp == PFKR<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFKR]->mindur[phone & PVALUE]);
	}
	else if(tmp == PFHE<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFHE]->mindur[phone & PVALUE]);
	}
	else 
	{
#ifdef PH_DEBUG
	FNX_PRINTF("OH MY gosh my golly it's no time for mistletoe and holly. \n");
#endif
	return(pDph_t->cur_speaker[PFUSA]->mindur[phone & PVALUE]);
	}    
}
...
//extern const short fr_inhdr[];
	...
	{
		//tmp=phone & PVALUE;
		return(pDph_t->cur_speaker[PFUSA]->inhdr[phone & PVALUE]);
	}
	else if(tmp == PFUK<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFUK]->inhdr[phone & PVALUE]);
	}
	else if(tmp == PFGR<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFGR]->inhdr[phone & PVALUE]);
	}
	else if(tmp == PFLA<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFLA]->inhdr[phone & PVALUE]);
	}
	else if(tmp == PFSP<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFSP]->inhdr[phone & PVALUE]);
	}
	else if(tmp == PFFR<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFFR]->inhdr[phone & PVALUE]);
		//when we have french change to fr_
	}
	else if(tmp == PFIT<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFIT]->inhdr[phone & PVALUE]);
	}
	else if(tmp == PFJP<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFJP]->inhdr[phone & PVALUE]);
	}
	else if(tmp == PFCH<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFCH]->inhdr[phone & PVALUE]);
	}
	else if(tmp == PFKR<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFKR]->inhdr[phone & PVALUE]);
	}
	else if(tmp == PFHE<<PSFONT)
	{
		return(pDph_t->cur_speaker[PFHE]->inhdr[phone & PVALUE]);
	}
	else 
	{
#ifdef PH__DEBUG
	FNX_PRINTF("OH MY GOD! THEY'VE KILLED KENNY\n");
#endif
	return(pDph_t->cur_speaker[PFUSA]->inhdr[phone & PVALUE]);
	}    
}
