...
short us_gettar (LPTTS_HANDLE_T phTTS, int nphone_temp)
{
  ...
  short begtypnex;
	...
	const short *us_place=pDph_t->cur_speaker[PFUSA]->place;
...
//			FNX_PRINTF(" %d is av AT %d PHONE_TEMP\n",tartemp,phone_temp);
			...
			{
				if ((phone_feature( pDph_t,pDph_t->allophons[nphone_temp-2]) & FOBST) IS_MINUS)
				tartemp -= 10;
				else
					tartemp = 0;
			}
	...
	else if (pDphsettar->par_type IS_PARALLEL_FORM_AMP)
	{
		...
		if (pDphsettar->np == &PTILT)
		{
			...
			{
				tartemp = 20;		   /* Tilt down nasal murmurs */
				}			        
...
#endif					
					if (((pDphsettar->phcur == USP_N) || (pDphsettar->phcur == USP_EN))
 ...
 *
 */
void us_special_rules (LPTTS_HANDLE_T phTTS, short fealas,
...
//Set it once per loop when PAB
		 ...
		 && ((feacur & FSONOR) IS_PLUS ))) 
	{
		...
		{
			PAP.pspesh = 53;		   /* Stronger asp before +back */
		}
		...
		else if (  pDph_t->allophons[pDph_t->nphone -2]== USP_S)          
		{
