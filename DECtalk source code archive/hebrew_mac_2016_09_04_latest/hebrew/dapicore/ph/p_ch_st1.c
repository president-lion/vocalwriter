 ...
 *  016 CHJ		07/20/2000	Removed & PVALUE when comparing to CHP_*. The language bits need to be set.
...
short ch_gettar (LPTTS_HANDLE_T phTTS, int nphone_temp)
{
  ...
  short begtypnex;
	...
	const short *ch_place=pDph_t->cur_speaker[PFCH]->place;
...
// if we're at beggining silence then sil value is the value of the next 
	...
	if (pDphsettar->np < &PFZ)                      
		pphotr = (npar) * CH_TOT_ALLOPHONES;
	else
		pphotr = (npar - 1) * CH_TOT_ALLOPHONES;
	...
	if ((pDphsettar->par_type = partyp[npar]) IS_FORM_FREQ_OR_BW)
	{
		...
		if ((npar == F2 - 1))
			if (phone_feature( pDph_t,phlas_temp ) & FVELARR) 
		{
			tartemp -= 100;
		}	
		...
		if ((((phone_temp) == CHP_N) || ((phone_temp) == CHP_NX))
			&& (npar == B3 - 1))
		{
			if (((ch_place[phnex_temp & PVALUE] & F2BACKI) IS_PLUS)
				|| ((ch_place[(phlas_temp & PVALUE)] & F2BACKF) IS_PLUS))
			{
				tartemp = 1600;
			}
		}
...
//			FNX_PRINTF(" %d is av AT %d PHONE_TEMP\n",tartemp,phone_temp);
		...
		//	if(pDph_t->allophons[pDph_t->nphone ]== CHP_T1)
		//		tartemp = 42;
		//	else
				tartemp = 40;
 ...
 *
 */
short ch_special_coartic (PDPH_T pDph_t, short nfon, short diphpos)
{
...
//	extern const short ch_place[];
	...
	{
		if (((phone_feature( pDph_t,foncur) & FVOWEL) IS_PLUS))
		{
			if (((fonlas == CHP_W) )
				|| ((fonnex == CHP_W) ))
			{
	...
	if (pDphsettar->np == &PF2)
	{
		...
		if ((fonlas == CHP_W) || (fonlas == CHP_L) )
		{
			if (((foncur >= CHP_IY) && (foncur <= CHP_UE)) || (foncur == CHP_EY))
			{
		...
		if (foncur == CHP_UW)
		{
			if ((ch_place[fonlas & PVALUE] & FALVEL) IS_PLUS)
			{
				temp = 200;
			}
		}
		if ((foncur == CHP_UW) && (diphpos > 0))
		{
			if ((ch_place[fonnex  & PVALUE] & FALVEL) IS_PLUS)
			{
				temp += 200;
			}
		}
 ...
 *
 */
void ch_forw_smooth_rules (LPTTS_HANDLE_T phTTS, short shrif, short pholas,
				   			   ...
				   			   short struccur, int feanex)
{
	...
	if (pDphsettar->par_type IS_FORM_FREQ)
	{
		...
		else
		{
			...
			if ((feacur & FNASAL) IS_PLUS)
			{
				...
				else if (((pDphsettar->phcur == CHP_N) )
						 && (endtyp(pDph_t,pholas) == 1))
				{
					if (pDphsettar->np == &PF2)
					{
						pDphsettar->bouval -= 100;
						if ((ch_place[pholas  & PVALUE] & F2BACKF) IS_PLUS)
						{
							pDphsettar->bouval -= 100;
						}
					}
				...
				else if ((pDphsettar->np == &PF2) && (pDphsettar->phcur == CHP_M)
						 && ((ch_place[pholas  & PVALUE] & F2BACKF) IS_PLUS))
				{
	...
	else if (pDphsettar->par_type IS_FORM_BW)
	{
		...
		if ((fealas & FNASAL) IS_PLUS)
		{
			...
			if ((pDphsettar->np == &PB2) && ((pholas == CHP_N) )
			 ...
			 ||*/ (pDphsettar->par_type IS_AV_OR_AH))
	{
		...
		{
			if ((pDphsettar->phcur == GEN_SIL) || (pDphsettar->phcur == CHP_F) 
				|| (pDphsettar->phcur == CHP_S) )
			{
				if (((fealas & FVOICD) IS_PLUS)
 ...
 *
 */
void ch_back_smooth_rules (LPTTS_HANDLE_T phTTS, short shrib, int feacur,
				   			   int feanex, short strucnex)
{
		...
		else
		{
			...
			if ((feacur & FNASAL) IS_PLUS)
			{
						 ...
						 && (begtyp(pDph_t,pDphsettar->phonex) == 1))
				{
					...
					{
						pDphsettar->bouval -= 100;
						if ((ch_place[pDphsettar->phonex  & PVALUE] & F2BACKI) IS_PLUS)
						{
							pDphsettar->bouval -= 100;
						}
					}
				...
				/* Lower F2 of [m] murmur near [i,y,yu,ir] */
						 ...
						 && ((ch_place[pDphsettar->phonex & PVALUE] & F2BACKI) IS_PLUS))
				{
					pDphsettar->bouval -= 150;
				}
			}
	...
	else if (pDphsettar->par_type IS_FORM_BW)
	{
		...
		if ((feanex & FNASAL) IS_PLUS)
		{
			...
			if ((pDphsettar->np == &PB2) && ((pDphsettar->phonex == CHP_N) )
 ...
 *
 */
void ch_special_rules (LPTTS_HANDLE_T phTTS, int fealas,
...
//Set it once per loop whne PAB
			...
			{
			if(ch_place[pDphsettar->phcur & PVALUE] & FLABIAL)
			{
				PAREAL.tspesh=closure_dur;
				PAREAB.tspesh =0;
				PTONGUEBODY.tspesh=0;
			}
			else if(ch_place[pDphsettar->phcur & PVALUE] & BLADEAFFECTED )
			{
				PAREAL.tspesh=0;
				PAREAB.tspesh=closure_dur;
				PTONGUEBODY.tspesh=0;
			}
				else if(ch_place[pDphsettar->phcur & PVALUE] & FVELAR )
			{
				PAREAL.tspesh=0;
				PAREAB.tspesh=0;
	...
	if ((((fealas & FPLOSV) IS_PLUS)
		 ...
		 //|| pDph_t->allophons[pDph_t->nphone -1]== CHP_ZH
		||  pDph_t->allophons[pDph_t->nphone -1]== CHP_T1) 
	{
		PAP.pspesh = 58;			   /* Amplitude of aspiration in dB     */
	
		if (begtyp(pDph_t,pDphsettar->phcur) != 1)
		{
			PAP.pspesh = 63;		   /* Stronger asp before +back */
		}
		PAV.pspesh = 0;				   /* Amplitude of voicing during aspir */

		vot = NF60MS; 

		 if (  pDph_t->allophons[pDph_t->nphone -2]== CHP_S)          
		{
