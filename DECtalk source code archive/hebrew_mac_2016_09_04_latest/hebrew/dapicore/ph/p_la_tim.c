...
/* MAIN LOOP, for each output phoneme */
	...
	/* 2/9/88 EAB, 
		...
		*/
		...
		//erik wants stress vowel lengthen this is the esiest way to do it overlll
		//first try lewnthening all parts of syl
		if (strucstresscur == FSTRESS_1 )
		{
			arg1 = prcnt;
			arg2 = N150PRCNT;
	...
	/*	else*/ if(pDphsettar->sprat0 >= 325)
		...
		if( phone_feature( pDph_t , pDph_t->allophons[nphon ]) & FSYLL
			&&  ((strucstresscur & FSTRESS_1) IS_MINUS)
	//		&&	((pDph_t->allofeats[nphon+1] & FSTRESS_1) IS_MINUS)
			&&( phone_feature( pDph_t , pDph_t->allophons[nphon + 1]) & FSYLL))
			
		
		{
				pDph_t->allodurs[nphon +1] += (pDph_t->allodurs[nphon]>>1)+(pDph_t->allodurs[nphon]>>2) ;
				pDph_t->allodurs[nphon ] -= (pDph_t->allodurs[nphon ]>>1)+(pDph_t->allodurs[nphon]>>2);
