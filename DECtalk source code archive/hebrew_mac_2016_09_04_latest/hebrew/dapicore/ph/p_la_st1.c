...
short la_gettar (LPTTS_HANDLE_T phTTS, int nphone_temp)
{
	...
	short npar;
	short                   phone_feat;
	const short *la_place=pDph_t->cur_speaker[PFLA]->place;
	...
	else if (pDphsettar->par_type IS_AV_OR_AH)
	{
		...
		else
		{
			...
			if(phone_temp == LAP_LL)
				tartemp=10;
			if(phone_temp == LAP_J)
				tartemp=40;/* make stronger for castillion*/
		}
	}
			 ...
			 || (pDphsettar->par_type IS_AV_OR_AH))
	{
		...
		{					   /* 7-May-86    */
			...
			if ((feacur & FOBST) IS_MINUS)
			{
				...
				if (fealas ==
					(FVOICD + FOBST + FPLOSV + FCONSON + FBURST + FSTOP))
					pDphsettar->durtran = 0;
			}
			/* Voicing is special */
...
void special_rules (LPTTS_HANDLE_T phTTS, short fealas, int feacur, int feanex, 
 ...
 *
 * *****************************************************************/
void la_special_rules (LPTTS_HANDLE_T phTTS, int fealas, int feacur, int feanex,
							  short struclm2, short struccur, short pholas, short struclas)
{
	short                   bdur, vot; /* MVP made as a local variable */
...
*/
...
#endif
		if(pDph_t,pDph_t->allophons[pDph_t->nphone+1]== GEN_SIL)
			bdur = 0;
		if (bdur > 1)
		{
			if (((feacur & FPLOSV) IS_PLUS)
		...
		)
	{
		...
		PB2.pspesh = 1000;
		PB3.pspesh = 1500;
		PF1.pspesh = 220; //kkep f1 low during closure
	}
}
