...
#ifdef OS_PALM
const unsigned char * fr_common_affixes[] =
#else
const unsigned char *const fr_common_affixes[] =
#endif
{
	"sElvz",
	"kwIst",
...
const char fr_syl_vowels[] = "a@AeEiIoOuU^WRc|xLN";

#ifdef OS_PALM
const char * fr_syl_cons[] =
#else
const char *const fr_syl_cons[] =
#endif
{
...
void fr_saysyllable (LPTTS_HANDLE_T phTTS)
{
	...
	while (last <= old_tot)
	{
		...
		while (--k >= 0)
		{
			 ...
			 */
			if (fr_ascky_check[temp_symbols[last]])
				pDph_t->nphone++;
