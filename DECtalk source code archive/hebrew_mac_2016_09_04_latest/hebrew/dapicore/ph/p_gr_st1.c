...
short gr_special_coartic (PDPH_T pDph_t, short nfon, short diphpos)
{
	...
	PDPHSETTAR_ST           pDphsettar = pDph_t->pSTphsettar;
	const short *gr_place=pDph_t->cur_speaker[PFGR]->place;
...
//	PKSD_T                  pKsd_t = phTTS->pKernelShareData;
	PDPHSETTAR_ST           pDphsettar = pDph_t->pSTphsettar;
	const short *gr_begtyp=pDph_t->cur_speaker[PFGR]->begtyp;
...
//	PKSD_T                  pKsd_t = phTTS->pKernelShareData;
		...
		if ((feacur & FSONOR) IS_PLUS)
		{
			...
			if ((feacur & FSONCON) IS_MINUS)
			{
				if ((feanex & FSONCON) IS_PLUS)
				{
					...
					else if(pDph_t->allophons[pDph_t->nphone+1] == GRP_R)
					
					pDphsettar->durtran = pDph_t->allodurs[pDph_t->nphone]>>1;
	...
	else if (pDphsettar->par_type IS_FORM_BW)
	{
		...
		if ((feanex & FNASAL) IS_PLUS)
		{
			...
			if ((pDphsettar->np == &PB2) && (endtyp(pDph_t,pDphsettar->phcur) != 1))
			{
				pDphsettar->bouval += 60;
				pDphsettar->durtran = NF60MS;
			}
...
//Set it once per loop whne PAB
		 ...
		 && ((feacur & FSONOR) IS_PLUS)))
	{
		...
		PAP.pspesh = 45;			   /* Amplitude of aspiration in dB     */
		if (gr_begtyp[pDphsettar->phcur] != 1)
		{
			PAP.pspesh = 48;		   /* Stronger asp before +back */
		}
