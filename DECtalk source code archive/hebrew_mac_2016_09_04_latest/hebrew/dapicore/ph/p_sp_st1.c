...
short sp_gettar (LPTTS_HANDLE_T phTTS, int nphone_temp)
{
  ...
  short begtypnex;
#endif
	const short *sp_place=pDph_t->cur_speaker[PFSP]->place;
 ...
 *
 * *****************************************************************/
void sp_special_rules (LPTTS_HANDLE_T phTTS, int fealas, int feacur, int feanex,
...
*/
		 ...
		 || sp_place[pholas & PVALUE] == FVELAR) 
	{
		...
		if(sp_place[pholas & PVALUE] & FLABIAL)
			vot = 1; //for p
		else if(sp_place[pholas & PVALUE] & FVELAR)
			vot = 2;
