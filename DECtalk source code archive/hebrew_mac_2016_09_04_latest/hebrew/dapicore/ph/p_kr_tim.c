 ...
 *
 * *****************************************************************/
void kr_phtiming(LPTTS_HANDLE_T phTTS)
{
...
//#define MSDBG5
	...
	for (nphon = 0; nphon < pDph_t->nallotot; nphon++)
	{
		...
		if ((feacur & FSYLL) IS_PLUS) 
		{
			...
			else if ((feanex & FBURST) IS_PLUS) {
				//since we're past fstop this must
				//be a fircative or an affricate
				arg1 = N35PRCNT;
			}
			else if ((feanex & FSONCON) IS_PLUS) {
				//since we're past fstop this must
				//be a fircative or an affricate
				arg1 = N74PRCNT;
			}
		...
		if ((feacur & FSYLL) IS_PLUS) {
			...
			if (((fealas & FVOICD) IS_PLUS) && (pholas != GEN_SIL)) 
			{
				arg1 = N30PRCNT;
			}
			arg2 = prcnt;
...
/*	  Reset number of accent rises sofar in a phrase if new phrase */
    if ((struccur & FBOUNDARY ) > FPPNEXT) 
		...
		*/
		...
		// Need list of strongly aspirated consonants
