...
short uk_gettar(LPTTS_HANDLE_T phTTS, int nphone_temp)
{
  ...
  short begtypnex;
	...
	const short *uk_place=pDph_t->cur_speaker[PFUK]->place;
 ...
 *
 * *****************************************************************/
short UKP_special_coartic(PDPH_T pDph_t, short nfon, short diphpos)
{
 ...
 *
 * *****************************************************************/
void uk_forw_smooth_rules(LPTTS_HANDLE_T phTTS, short shrif, short pholas, int fealas,
								 int feacur, short struclas, short struccur, int feanex)
{
 ...
 *
 * *****************************************************************/
void uk_special_rules (LPTTS_HANDLE_T phTTS, int fealas, int feacur, int feanex,
...
*/
		...
		if ((pDphsettar->phcur == UKP_TH) || (pDphsettar->phcur == UKP_DH)
			|| pDphsettar->phcur == UKP_DZ) 
		{
		...
		&& (pDphsettar->phcur != UKP_TX))
	{
		...
		PAV.pspesh = 23;			   /* Large since low-pass TILT attenuates it */
