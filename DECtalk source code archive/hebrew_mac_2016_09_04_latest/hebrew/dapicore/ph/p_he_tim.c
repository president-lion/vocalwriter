 ...
 *
 * *****************************************************************/
void he_phtiming(LPTTS_HANDLE_T phTTS)
{
...
//				printf(" at %d \n", nphon);
			if(pDph_t->allophons[nphon] == HEP_NG
				&& pDph_t->allophons[nphon-1] == HEP_JH)
			{
					pDph_t->allodurs[nphon] = 3;
					pDphsettar->durxx = 3;
					goto skipall;
			}
//			if(pDph_t->allophons[nphon] == HEP_CZ)
//			{
//					pDph_t->allodurs[nphon] = 20;
//					pDphsettar->durxx = 3;
//					goto skipall;
//			}
...
//			{
//				if(pDph_t->allophons[nphon+1] != HEP_AX)
//				{
...
//				}
//			}
//			else if((struccur & FBOUNDARY) >= FWBNEXT)
//			{
//					pDph_t->allodurs[nphon] = 6;
//					pDphsettar->durxx =6;
//					goto skipall;
...
if((pKsd_t->modeflag & MODE_READING)==0 /* && isEnd */ )
{
				isEnd = 0;
				dpause =0;
}
		...
		*/
		...
		else
		{
			if ((feacur & FCONSON) IS_PLUS)
			{
				...
				if (bindb && ((fealas & FCONSON) IS_PLUS)
		...
		{
			if (bindb &((fealas & FVOICD) IS_MINUS)
		...
		if (bindb &((feacur & FPLOSV) IS_PLUS) &&
			((fealas & FPLOSV) IS_PLUS) &&
			((pDphsettar->feanex & FOBST) IS_PLUS) &&
			(strucboucur > FMBNEXT))
		{
...
skiptim:		pDphsettar->durxx = (prcnt * (durinh - durmin)) DIV_BY128;
#ifdef MSDBG5
...
break3:
		...
		if (pDphsettar->durxx < 0 ) /*eab oct 93 found dur could get set =0 compromise*/
			...
			if (pDph_t->allophons[nphon] == HEP_JH)
				if( (pDph_t->allofeats[nphon]& FSBOUND) == 0)
			/*	|| pDph_t->allophons[nphon-1] == HEP_T*/
				cons_dur +=pDphsettar->durxx ; 
#endif
...
nomodify:		pDph_t->allodurs[nphon] = pDphsettar->durxx;	   /* Save in array for phonetic comp */
...
#if (!(defined TOMBUCHLER || defined ODED))
...
//			FNX_PRINTF("ph=%d ",pDph_t->allophons[endcnt]); 
		...
		//		goto skipall;
			...
			if(cons_dur != 0 && nphon>1)        
			{
				...
				{
					pDph_t->adjusted_phon = nphon+1;
					cons_dur += 0;
				//	FNX_PRINTF("add in stio dur as a cons first \n");
				}
			//	FNX_PRINTF("consdur=%d \n",(cons_dur*64)/10);
			if ((phone_feature( pDph_t ,pDph_t->allophons[nphon+1]) & FSYLL)
			...
			//	if(pDph_t->allodurs[endcnt] > (cons_dur>>1))
			//	{
			//	pDph_t->allodurs[endcnt] -= (cons_dur>>1);
			//		cons_dur = (cons_dur >>1);
			//	}
