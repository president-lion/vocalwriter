...
void gr_phtiming (LPTTS_HANDLE_T phTTS)
{
	...
	int                    fealas = phone_feature( pDph_t,GEN_SIL); /* Made local and initialized */
	...
	for (nphon = 0; nphon < pDph_t->nallotot; nphon++)
	{
		...
		if (feasyllabiccur IS_PLUS)
		{
				...
				&& ((struccur & FTYPESYL) == FMONOSYL) )
			{
				...
				arg1 = N80PRCNT;
			if(phocur == GRP_EX || phocur == GRP_AH || phocur == GRP_EH
				|| phocur == GRP_IH || phocur == GRP_OH || phocur == GRP_UH)
				{
				arg1 = N30PRCNT;
				}
				...
				&& (pholas != GRP_S))
			{
					...
					&& (strucboucur < FVPNEXT))
				{
					...
					if (((feacur & FNASAL) IS_PLUS)
						&& ((pDphsettar->feanex & FNASAL) IS_MINUS)
...
#ifdef MSDBG5
							FNX_PRINTF ("durxx = NF20MS durxx=%d\n", pDphsettar->durxx);
#endif
							goto break3;
						}
			...
			if( ((struccur & FSTRESS) IS_PLUS))
				...
				/* printf("strucc=%o of phon %d at %d",struccur,pDph_t->allophons[nphon],nphon); */
					...
					for (endcnt = nphon-1; stcnt - (endcnt); endcnt--)
					{
						...
						// WINprintf("nphon %d ",endcnt); FNX_PRINTF("p=%d ",pDph_t->allophons[endcnt]);
