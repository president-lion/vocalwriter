...
/* GL 04/21/1997  change this for the OSF build */
	...
	for (m = msym + 1; m < pDph_t->nsymbtot; m++)
	{
...
#ifdef MSDBG8
				FNX_PRINTF ("add_feature(FSTRESS_1,CURRPHONE)\n");
#endif
			}
			if (sy == S2)
			{
...
#ifdef MSDBG8
				FNX_PRINTF ("add_feature(FSTRESS_2, CURRPHONE)\n");
#endif
			}
			if (sy == SEMPH)
			{
...
#ifdef MSDBG8
				FNX_PRINTF ("add_feature(FEMPHASIS, CURRPHONE)\n");
#endif
			}
			return;
		}
		...
		if (sy<100 && (phone_feature(pDph_t, sy) & FSYLL) IS_PLUS)
		{
