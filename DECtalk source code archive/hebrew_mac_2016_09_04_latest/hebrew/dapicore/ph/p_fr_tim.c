...
// RDK Comment out unused extern reference of this function
...
#ifdef DEBG
    if (dbgdur) FNX_PRINTF ("p_fr_tim.c F1_NbSyll()  NbS=%d\n", NbS);
#endif
...
#ifdef DEBG
    if (dbgdur) FNX_PRINTF ("p_fr_tim.c  regle() %2s %s\n",
        phprint (phocur), mes);
#endif
}
...
short Allonge_Cons_Finale (PDPH_T pDph_t, short Seg_Prec, short Cons, short pcnt) {
    //FNX_PRINTF ("Allonge_Cons_Finale cons %d %x\n", Cons, Cons);

#ifdef DEBG
    if (dbgdur) FNX_PRINTF ("p_fr_tim.c  Allonge_Cons_Finale entree %2x [%2x] pcnt = %d\n",
...
#ifdef DEBG
  if (dbgdur) {
    FNX_PRINTF ("p_fr_tim.c  Allonge_Cons_Finale()  sortie pcnt= %d\n", pcnt);
  }
#endif
...
void fr_phtiming (LPTTS_HANDLE_T phTTS) {
  ...
  short pcnt;
  //  RDK This variable is defined as 'const' so cannot be referenced like this
  //  It is, however not used in this file.
...
#define Fact_Elongation       PCent6
...
#ifdef DEBG
    if (dbgdur) FNX_PRINTF ("p_fr_tim.c  phtiming  entree pDph_t->NbSyllabes = %d\n",
                        pDph_t->NbSyllabes);
#endif
  ...
  Syll_Synt_Restantes   = pDph_t->Syll_Synt;
  Syll_Phrase_Restantes = pDph_t->NbSyllabes - Syll_Synt_Restantes;

#ifdef DEBG
  if (dbgdur) {
    FNX_PRINTF ("p_fr_tim.c  phtiming  PCent_Elongation %d\n", PCent_Elongation);
    FNX_PRINTF ("p_fr_tim.c  phtiming  pDph_t->nallotot %3d\n", pDph_t->nallotot);
  }
#endif
...
#ifdef DEBG
        if (dbgdur) {
          FNX_PRINTF ("p_fr_tim.c  phtiming  word or clause boundary Raise %d, pDph_t->allofeats [nphon]=  %4x\n",
...
#ifdef DEBG
        if (dbgdur) FNX_PRINTF ("******* Clause boundary  Syll_Synt_Restantes %d\n", 
                             Syll_Synt_Restantes);
#endif
      } else {  // Word boundary
        Fin_Groupe   = (pDph_t->allofeats[nphon] & FGROU) >> 4;
#ifdef DEBG
        if (dbgdur) FNX_PRINTF ("Fin Groupe %d\n", Fin_Groupe);
#endif
...
#ifdef DEBG
    if (dbgdur) FNX_PRINTF ("  Syll_Mot_Restantes %d, Syll_Synt_Restantes %d\n",
...
#ifdef DEBG
        if (dbgdur) FNX_PRINTF ("p_fr_tim.c  phtiming()  D_Base %3d\n", D_Base);
#endif
...
#ifdef DEBG
          if (dbgdur) {
            FNX_PRINTF ("p_fr_tim.c  phtiming()  D_Base %d\n", D_Base);
            FNX_PRINTF ("p_fr_tim.c  pcnt %d, Voy_Ouverte %d\n",
                     pcnt, Voy_Ouverte);
          }
#endif
...
#ifdef DEBG
          if (dbgdur) FNX_PRINTF ("p_fr_tim.c  section vowel  sortie pcnt %d\n", pcnt);
#endif
...
#ifdef DEBG
        if (dbgdur) FNX_PRINTF ("p_fr_tim.c End of a vowel (stressed or not) : pcnt : %d\n", pcnt);
#endif
      break; // vowels
...
#ifdef DEBG
        if (dbgdur) FNX_PRINTF ("End of a consonant section pcnt : %d\n", pcnt);
#endif
...
#ifdef DEBG
        if (dbgdur) {
          FNX_PRINTF ("p_fr_tim.c  phtiming()  pcnt %d, pDphsettar->durxx %d\n",
                   pcnt, pDphsettar->durxx);
        }
#endif

        // duration depending on speech rate (in ms)
        //FNX_PRINTF ("pDphsettar->durxx 1 %d\n", pDphsettar->durxx);
        pDphsettar->durxx = mlsh1 (pDphsettar->durxx, pDphsettar->sprat1);
        //FNX_PRINTF ("pDphsettar->durxx 2 %d\n", pDphsettar->durxx);
...
#ifdef DEBG
          if (dbgdur) {
            FNX_PRINTF ("p_fr_tim.c  phtiming() Phrase_Courte pDphsettar->durxx %d\n", 
                     pDphsettar->durxx);
          }
#endif
        }
...
#ifdef DEBG
      if (dbgdur) {
        FNX_PRINTF ("p_fr_tim.c  %2s ", phprint (phocur));
        FNX_PRINTF ("p_fr_tim.c  pDphsettar->durxx (frames) = %4d\n", pDphsettar->durxx);
        //FNX_PRINTF ("%2s : %3d\n", 
