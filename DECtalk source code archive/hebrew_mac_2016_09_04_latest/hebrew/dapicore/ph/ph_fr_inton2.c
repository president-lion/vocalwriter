...
void make_f0_command (LPTTS_HANDLE_T phTTS,short type, short rulenumber, 
...
short F_Nb_Syll (PDPH_T pDph_t, short nphon) {
  ...
  //FNX_PRINTF ("ph_fr_inton2.c F_Nb_Syll()  nphon=%d, phoneme:%s, NbS=%d\n",
...
static void regle (short phocur, char* mes) {
  //FNX_PRINTF ("p_fr_inton2.c  regle() %s %s\n", phprint (phocur), mes);
}
...
#define NB_PH_POS 6  // Number of values in f0_mphrase_position
  ...
  int feacur    = 0;   // current phone features (FSYLL, FVOICD, etc.)
  ...
  // STORING NUMBER SYLS IN FEATURE RUNS OUT OF FEATURE BITS FOR THE ALL LANGUAGE MODEL
  ...
  pDphsettar->nrises_sofar        = 0;
	...
	//to avoid conflict with other langauges that use this
  //FNX_PRINTF ("phinton.c phinton before main loop   pDph_t->Syll_Synt %d\n", pDph_t->Syll_Synt);
  ...
  for (nphon = 0; nphon < pDph_t->nallotot; nphon++) {
    ...
    //if (struccur & FMOT) FNX_PRINTF ("\n");  // New word
    //FNX_PRINTF ("ph_fr_inton2.c phinton()  nphon : %3d, %s\n",
    ...
    if (struccur & FMOT) {  // word or clause boundary
      ...
      if (struccur & FSYNT) {  // new clause
        ...
        Syll_Synt_Restantes = pDph_t->Syll_Synt;
        // FNX_PRINTF ("ph_fr_inton2.c phinton()  FSYNT pDph_t->Syll_Synt %d\n", pDph_t->Syll_Synt);
        ...
        //FNX_PRINTF ("ph_fr_inton2.c phinton()  Dernier_Mot_Acc %d, Mot_Accentue %d, Fin_Groupe %d\n",
        //           Dernier_Mot_Acc, Mot_Accentue, Fin_Groupe);
        //FNX_PRINTF("ph_fr_inton2.c phinton() Syll_Mot : %d\n", Syll_Mot);
      ...
      if ( (feacur & FSYLL) && Mot_Accentue) {
        ...
        if (pDph_t->hatpos == AT_BOTTOM_OF_HAT) { // first vowel of a stressed
          ...
          if (pDph_t->f0mode == NORMAL) {
            //FNX_PRINTF ("ph_fr_inton2.c phinton() Rule 1 %d\n", 
        ...
        // Calculate targf0 : the value of the impulse and delayf0 : time of impulse
          ...
          //FNX_PRINTF ("Ph_fr_inton2.c  pDphsettar->nrises_sofar %d, f0_mphrase_position[] %d\n",
        ...
        if ((Syll_Synt_Restantes == 1) && (pDph_t->hatpos == AT_TOP_OF_HAT)) {
          if (pDph_t->f0mode == NORMAL) {
            ...
            // FNX_PRINTF ("ph_fr_inton2.c phinton() Rule 3\n");
            // f0fall = 212; // F0_FINAL_FALL;
            // FNX_PRINTF ("pDph_t->allofeats [pDph_t->nallotot-1] %4x\n", 
            //    pDph_t->allofeats [pDph_t->nallotot-1]);

            if ((pDph_t->allofeats [pDph_t->nallotot-1]) & FPROP) {
              f0fall = 212; // F0_FINAL_FALL;
              //FNX_PRINTF ("f0fall FPROP %d\n", f0fall);
            } else {
              f0fall = 112; // NOT F0_FINAL_FALL;
              f0fall = 50;
              //FNX_PRINTF ("f0fall NOT FPROP %d\n", f0fall);
        }
...
//#define DEBUGFRENCH
  ...
  { // block for local variables
	...
	for (n=0; n<pDph_t->nf0tot; n++) {
	  ...
	  if (pDph_t->f0tar [n] == 0) {
	    FNX_PRINTF ("Baseline Reset and Hat Reset");
	  } else if (pDph_t->f0tar [n] >= 2000) {
	    if (pDph_t->f0tar [n] <= 2037) {
	      FNX_PRINTF ("Sung note %d = %d Hz*10",
		      pDph_t->f0tar [n]-2000, notetab [pDph_t->f0tar[n]-2001]);
	    } else {
	      FNX_PRINTF ("F0 target of %d Hz at phone end", pDph_t->f0tar [n]-2000);
	    }
	    //} else if (pDph_t->f0tar [n] & 01) {
	    } else if ( pDph_t->f0type [n] == IMPULSE) {
          FNX_PRINTF ("Impulsive Command %d", pDph_t->f0tar [n]);
	    } else {
          FNX_PRINTF ("Step Command %d", pDph_t->f0tar [n]);
	    }
	}
...
static void make_f0_command (LPTTS_HANDLE_T phTTS, short type, short rulenumber, 
  ...
  if (1) {
    if ((nphon%10) == 0) {
      FNX_PRINTF ("make_f0_command (type rulenumber tar delay length *psCumdur nphon phon");
      FNX_PRINTF ("     (nf0tot f0tim f0tar f0type)\n");
    }

    FNX_PRINTF ("make_f0_command  %4s %2d %2d %4d %4d %2d %4d %2d\n",
       phprint (pDph_t->allophons [nphon]),
       type, rulenumber, tar, delay, length, *psCumdur, nphon);

    FNX_PRINTF ("     %2d %4d, %4d, %4d, %4d\n", pDph_t->nf0tot, 
