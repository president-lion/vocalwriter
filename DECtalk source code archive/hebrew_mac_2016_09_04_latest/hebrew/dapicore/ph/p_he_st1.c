 ...
 *  016 CHJ		07/20/2000	Removed & PVALUE when comparing to HEP_*. The language bits need to be set.
...
short he_gettar (LPTTS_HANDLE_T phTTS, int nphone_temp)
{
  ...
  short begtypnex;
	...
	const short *he_place=pDph_t->cur_speaker[PFHE]->place;
...
// if we're at beggining silence then sil value is the value of the next 
	...
	if (pDphsettar->np < &PFZ)                      
		pphotr = (npar) * HE_TOT_ALLOPHONES;
	else
		pphotr = (npar - 1) * HE_TOT_ALLOPHONES;
	...
	if ((pDphsettar->par_type = partyp[npar]) IS_FORM_FREQ_OR_BW)
	{
		...
		if ((phone_temp) == HEP_N 
			&& (npar == B3 - 1))
		{
			if (((he_place[phnex_temp & PVALUE] & F2BACKI) IS_PLUS)
				|| ((he_place[(phlas_temp & PVALUE)] & F2BACKF) IS_PLUS))
			{
				tartemp = 1600;
			}
		}
...
//			FNX_PRINTF(" %d is av AT %d PHONE_TEMP\n",tartemp,phone_temp);
				...
				&& ((phone_feature( pDph_t,phlas_temp) & FVOICD) IS_MINUS))
			{
				...
				/* Voiced fricative is devoiced if next segment unvocied */
			if (((phone_feature( pDph_t,phone_temp) & FPLOSV) IS_MINUS))
				if ((phone_feature( pDph_t,phone_temp) & FOBST) IS_PLUS)
				if ((phone_feature( pDph_t,phnex_temp) & FVOICD) IS_MINUS)
			{
				tartemp = 0;
			}
 ...
 *
 */
short he_special_coartic (PDPH_T pDph_t, short nfon, short diphpos)
{
	...
	PDPHSETTAR_ST           pDphsettar = pDph_t->pSTphsettar;
//	extern const short us_place[];
	const short *he_place=pDph_t->cur_speaker[PFUSA]->place;
	...
	{
		if (((phone_feature( pDph_t,foncur) & FVOWEL) IS_PLUS) && (foncur != HEP_RR))
		{
			if ((fonlas == HEP_W) || (fonlas == HEP_R) 
				|| ((fonnex == HEP_W) || (fonnex == HEP_R) ))
			{
				temp = -150;
			}
		}
 ...
 *
 */
void he_forw_smooth_rules (LPTTS_HANDLE_T phTTS, short shrif, short pholas,
				   			   ...
				   			   short struccur, int feanex)
{
	...
	if (pDphsettar->par_type IS_FORM_FREQ)
	{
		...
		if ((feacur & FSONOR) IS_PLUS)
		{
			if ((feacur & FSONCON) IS_MINUS)
			{
				...
				if ((fealas & FSONCON) IS_PLUS)
				{
					...
					if ((pholas == HEP_LL) && (pDphsettar->np == &PF1))
					{
						pDphsettar->bouval += 80;
					}
					/* Make F3 & F2 transitions slower out of /r/ */
					if ((pholas == HEP_R) && (pDphsettar->np != &PF1))
					{
		...
		else
		{
			...
			if ((feacur & FNASAL) IS_PLUS)
			{
				...
				else if (((pDphsettar->phcur == HEP_N) )
						 && (endtyp(pDph_t,pholas) == 1))
				{
					if (pDphsettar->np == &PF2)
					{
						pDphsettar->bouval -= 100;
						if ((he_place[pholas  & PVALUE] & F2BACKF) IS_PLUS)
						{
							pDphsettar->bouval -= 100;
						}
					}
				...
				else if ((pDphsettar->np == &PF2) && (pDphsettar->phcur == HEP_M)
						 && ((he_place[pholas  & PVALUE] & F2BACKF) IS_PLUS))
				{
	...
	else if (pDphsettar->par_type IS_FORM_BW)
	{
		...
		if ((fealas & FNASAL) IS_PLUS)
		{
			...
			if ((pDphsettar->np == &PB2) && ((pholas == HEP_N) )
			 ...
			 || (pDphsettar->par_type IS_AV_OR_AH))
	{
		...
		{
			if ((pDphsettar->phcur == HEP_CH) || (pDphsettar->phcur == HEP_JH))
			{
		...
		{
			if ((pDphsettar->phcur == GEN_SIL) || (pDphsettar->phcur == HEP_F) || (pDphsettar->phcur == HEP_TH)
				|| (pDphsettar->phcur == HEP_S) || (pDphsettar->phcur == HEP_SH))
			{
				if (((fealas & FVOICD) IS_PLUS)
 ...
 *
 */
void he_back_smooth_rules (LPTTS_HANDLE_T phTTS, short shrib, int feacur,
				   			   int feanex, short strucnex)
{
		...
		if ((feacur & FSONOR) IS_PLUS)
		{
			...
			if ((feacur & FSONCON) IS_MINUS)
			{
				if ((feanex & FSONCON) IS_PLUS)
				{
					...
					if ((pDphsettar->phonex == HEP_LL) && (pDphsettar->np == &PF1))
					{
						pDphsettar->bouval += 80; 
					}
				}
		...
		else
		{
			...
			if ((feacur & FNASAL) IS_PLUS)
			{
						 ...
						 && (begtyp(pDph_t,pDphsettar->phonex) == 1))
				{
					...
					{
						pDphsettar->bouval -= 100;
						if ((he_place[pDphsettar->phonex  & PVALUE] & F2BACKI) IS_PLUS)
						{
							pDphsettar->bouval -= 100;
						}
					}
				...
				/* Lower F2 of [m] murmur near [i,y,yu,ir] */
						 ...
						 && ((he_place[pDphsettar->phonex & PVALUE] & F2BACKI) IS_PLUS))
				{
					pDphsettar->bouval -= 150;
				}
			}
	...
	else if (pDphsettar->par_type IS_FORM_BW)
	{
		...
		if ((feanex & FNASAL) IS_PLUS)
		{
			...
			if ((pDphsettar->np == &PB2) && ((pDphsettar->phonex == HEP_N) )
			 ...
			 || (pDphsettar->par_type IS_AV_OR_AH))
	{
		...
		if ((pDphsettar->np == &PAV) && (pDphsettar->bouval < pDphsettar->np->tarnex)
			&& (pDphsettar->phcur != HEP_V) 
			&& (pDphsettar->phcur != HEP_JH)  && (pDphsettar->phcur != HEP_Z))
		{
			pDphsettar->durtran = 0;
			//debugbackward (" Make next onset abrupt", pDphsettar->np);
			/* Voicebar dies out in a voiced plosive */
			if (((feacur & FPLOSV) IS_PLUS) || (pDphsettar->phcur == HEP_CH))
			{
				if ((feacur & FVOICD) IS_PLUS)
				{
		...
		if (pDphsettar->phcur >= HEP_P)
		{
			pDphsettar->durtran = NF15MS;
			if (pDphsettar->phcur < HEP_CH)
			{
		...
		if (pDphsettar->np == &PAP)
		{
			...
			/* OUT                    (pDphsettar->phcur == GEN_SIL) ||                         END OUT */
				   (pDphsettar->phcur == HEP_F) || (pDphsettar->phcur == HEP_TH)
				   || (pDphsettar->phcur == HEP_S) || (pDphsettar->phcur == HEP_SH))
			{
				if (((feanex & FVOICD) IS_PLUS)
					&& ((feanex & FOBST) IS_MINUS))
 ...
 *
 */
void he_special_rules (LPTTS_HANDLE_T phTTS, short fealas,
...
//Set it once per loop when PAB
			...
			{
			if(he_place[pDphsettar->phcur & PVALUE] & FLABIAL)
			{
				PAREAL.tspesh=closure_dur;
				PAREAB.tspesh =0;
				PTONGUEBODY.tspesh=0;
			}
			else if(he_place[pDphsettar->phcur & PVALUE] & BLADEAFFECTED )
			{
				PAREAL.tspesh=0;
				PAREAB.tspesh=closure_dur;
				PTONGUEBODY.tspesh=0;
			}
				else if(he_place[pDphsettar->phcur & PVALUE] & FVELAR )
			{
				PAREAL.tspesh=0;
				PAREAB.tspesh=0;
		...
		if ((pDphsettar->phcur == HEP_TH) ) 
		{
			for (pDphsettar->np = &PA2; pDphsettar->np <= &PA6; pDphsettar->np++) 
			{		/* All fric gains to */
				pDphsettar->np->tspesh = pDph_t->durfon - 3;		/* zero during closur*/
				pDphsettar->np->pspesh = 0;
			}
        }
	...
	{
		PAP.pspesh = 47;			   /* Amplitude of aspiration in dB     */
	

		if (begtyp(pDph_t,pDphsettar->phcur) != 1)
		{
			PAP.pspesh = 50;		   /* Stronger asp before +back */
		}
		...
		if (((feacur & FSONCON) IS_PLUS) || (pDphsettar->phcur == HEP_RR))
		{
			PAP.pspesh += 5;		   /* Aspiration stronger in sonor cons */
			vot += 5;
		}

		else if (  pDph_t->allophons[pDph_t->nphone -2]== HEP_S)          
		{
		...
		{
			vot += 1;			   /* Vot longer in a sonorant consonant */
		}
