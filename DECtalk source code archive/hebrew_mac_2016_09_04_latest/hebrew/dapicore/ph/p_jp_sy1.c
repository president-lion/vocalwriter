...
const unsigned char *CONSTCONST us_common_affixes[] =
{
	"sElvz",
	"kwIst",
	"flEks",
...
const char *CONSTCONST us_syl_cons[] =
{
...
void us_saysyllable (LPTTS_HANDLE_T phTTS)
{
	...
	while (last <= old_tot)
	{
		...
		while (--k >= 0)
		{
			 ...
			 */
			if (us_ascky_check[temp_symbols[last]])
				pDph_t->nphone++;
