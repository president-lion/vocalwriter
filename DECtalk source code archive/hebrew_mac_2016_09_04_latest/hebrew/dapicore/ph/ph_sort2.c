...
void init_med_final(LPTTS_HANDLE_T phTTS, short msym)
{
...
#ifdef MSDBG8
				FNX_PRINTF ("add_feature(sylltype,CURRPHONE) sylltype=%d\n", sylltype);
#endif
			}
			return;
		}
 ...
 *
 */
//modified to undestand that the stress must be in the last word
// needed to hanlde secondary stress gracefully
//also perhaps we should start at the begiining of the word
//and go forward but I think thee cases always occur in monosyllables.
void find_syll_to_stress (LPTTS_HANDLE_T phTTS, short *locend, short nstartphrase)
{
...
if(pKsd_t->lang_curr != LANG_german)
  ...
  /* eab 4/21/98 This code is wierd in that in German we don't want to do this
	...
	way after this code was writeen by Dennis*/
		//if already stressed nothing to do 
		...
		//for new code to stres final word which could be already stressed
		if(pDph_t->symbols[m] == WBOUND
			|| pDph_t->symbols[m] == S1)
			break;
	}
	...
	//EAB DEC 7 - This code doesn't proerly hanlde a word with secondary stress you must
	//first look for a secondary to promte then abort to stressing the first vowel it can find
	//presently in a poly syl word it stresses the first syl if the second syl has a primary
	for (m = locbeg; m < *locend; m++)
	{
		//make sure its not another control phone
		if ((pDph_t->symbols[m]& PVALUE) <= 100)
		if ((phone_feature(pDph_t,pDph_t->symbols[m]) & FSYLL) IS_PLUS)
		{
			if( pDph_t->symbols[m-1] == S2)
			{
				pDph_t->symbols[m-1] = S1;
				return;
			}
			
		}
	}
			//EAB DEC 7 - Didin't find a secondary to promote so promote first
	//vowel you find
	for (m = locbeg; m < *locend; m++)
	{
		//make sure its not another control phone
...
#ifdef MSDBG8
			FNX_PRINTF ("add_feature(bounftab[symbols[m] - SBOUND], CURRPHONE) bounftab[%d]=%d\n", pDph_t->symbols[m] - SBOUND, bounftab[pDph_t->symbols[m] - SBOUND]);
#endif
			return;
		}
		else if (((phone_feature(pDph_t,pDph_t->symbols[m])) & FSYLL) IS_PLUS)
