...
//#include <unistd.h> //moved to port.h for each platform - mlt
...
//#include <errno.h> //moved to port.h for each platform - mlt
//#include <sys/ioctl.h> //moved to port.h for each platform - mlt
...
//#include <sys/stat.h>  //moved to port.h for each platform - mlt
#endif
//#ifdef VXWORKS //moved to port.h for each platform - mlt
//#include <time.h> //moved to port.h for each platform - mlt
//#else //moved to port.h for each platform - mlt
//#include <sys/time.h> //moved to port.h for each platform - mlt
//#endif //moved to port.h for each platform - mlt
#include <sys/mman.h>
#ifdef __arm__
#ifndef __ipaq__
...
#define IOCTL(a,b,c)		((-1==ioctl(a,b,&c))&&(FNX_PERROR("ioctl:"#b":"#c),0))
...
char *GetAudioDev(char *name)
{
	...
	infile=FNX_FOPEN("/etc/DECtalk.conf","r");
	if (infile==NULL)
	{
#ifdef TESTING
		infile=FNX_FOPEN("DECtalk.conf","r");
		if (infile==NULL)
#endif
		{
			FNX_STRCPY(name,SOUND_DEV);
			return(name);
		}
	}
	name[0]='\0';
	while (FNX_FGETS(line,499,infile)!=NULL)
	{
		if (FNX_STRNCMP(line,AUDIO_DEV_TAG,9)==0)
		{
			...
			if (line[9]=='$')
			{
				...
				FNX_STRCPY(name,env_dev);
				break;
			}
			else
			{
				FNX_STRCPY(name,line+9);
				break;
			}
		}
	}
...
LONG OSS_WaveInit(void)
{
  ...
  FNX_STRCPY(WOutDev[0].caps.szPname, "SB16 Wave Out");
#else
  WOutDev[0].caps.wMid = 0x00FF; 	/* Manufac ID */
  WOutDev[0].caps.wPid = 0x0001; 	/* Product ID */
  /*    FNX_STRCPY(WOutDev[0].caps.szPname, "OpenSoundSystem WAVOUT Driver");*/
  FNX_STRCPY(WOutDev[0].caps.szPname, "CS4236/37/38");
#endif
...
UINT16 waveOutGetNumDevs()
{
  UINT16	count = 0;
#ifdef _DEBUG
  FNX_FPRINTF(stderr,"Linux ONLY: waveOutGetNumDevs(VOID)\n");
#endif
  count += OSS_wodMessage( MMSYSTEM_FirstDevID(), WODM_GETNUMDEVS, 0L, 0L, 0L);
#ifdef _DEBUG
  FNX_FPRINTF(stderr, "waveOutGetNumDevs return %u \n", count);
#endif
  return count;
}
...
{
#ifdef _DEBUG
  FNX_FPRINTF(stderr,"Linux ONLY: waveOutGetDevCaps\n");
  //FNX_FPRINTF(stderr,"uDeviceID = %i\n",uDeviceID);
#endif  
...
static UINT16 waveGetErrorText(UINT16 uError, LPSTR lpText, UINT16 uSize)
{
  ...
  FNX_STRNCPY(lpText, msgptr, uSize);
  return TRUE;
}
		   ...
		   DWORD dwInstance, DWORD dwFlags)
{
...
#ifdef _DEBUG
  FNX_FPRINTF(stderr,"Linux ONLY: waveOutOpen\n");
  if (dwFlags & WAVE_FORMAT_QUERY)
    FNX_FPRINTF(stderr, "WAVE_FORMAT_QUERY requested !\n");
#endif
  if (uDeviceID == (UINT16)WAVE_MAPPER) {
#ifdef _DEBUG		
    FNX_FPRINTF(stderr, "WAVE_MAPPER mode requested !\n");
#endif		
    bMapperFlg = TRUE;
    uDeviceID = 0;
  }
  ...
  hWaveOut = (int) FNX_MALLOC(sizeof(WAVEOPENDESC));
  ...
  while(uDeviceID < MAXWAVEDRIVERS) {
    ...
    uDeviceID++;
#ifdef _DEBUG
    FNX_FPRINTF(stderr, "WAVE_MAPPER mode ! try next driver...\n");
#endif		
  }
  lpDesc->uMappedDeviceID = uDeviceID;  /* save physical Device ID */
  if (dwFlags & WAVE_FORMAT_QUERY) {
#ifdef _DEBUG
    FNX_FPRINTF(stderr, "End of WAVE_FORMAT_QUERY !\n");
#endif	
    FNX_FREE((void *)hWaveOut);
  }
  return dwRet;
}
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr,"Linux ONLY: waveOutClose");
	FNX_FPRINTF(stderr, "(%04X)\n", hWaveOut);
#endif
	lpDesc = (LPWAVEOPENDESC) hWaveOut;
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr, "Linux ONLY: waveOutPrepareHeader");
	FNX_FPRINTF(stderr, "(%04X, %p, %u);\n",
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr, "Linux ONLY: waveOutUnprepareHeader");
	FNX_FPRINTF(stderr, "(%04X, %p, %u);\n", 
		hWaveOut, lpWaveOutHdr, uSize);
#endif
	lpDesc = (LPWAVEOPENDESC) USER_HEAP_LIN_ADDR(hWaveOut);
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr, "Linux ONLY: waveOutWrite");
	FNX_FPRINTF(stderr, "(%04X, %p, %u);\n", hWaveOut, lpWaveOutHdr, uSize);
#endif	
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr, "Linux ONLY: waveOutPause");
	FNX_FPRINTF(stderr, "(%04X)\n", hWaveOut);
#endif	
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr, "Linux ONLY: waveOutRestart");
	FNX_FPRINTF(stderr, "(%04X)\n", hWaveOut);
#endif	
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr, "Linux ONLY: waveOutReset");
	FNX_FPRINTF(stderr, "(%04X)\n", hWaveOut);
#endif
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr, "Linux ONLY: waveOutGetVolume");
	FNX_FPRINTF(stderr, "(%04X)\n", hWaveOut);
#endif
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr, "Linux ONLY: waveOutSetVolume");
	FNX_FPRINTF(stderr, "(%04X)\n", hWaveOut);
#endif
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr, "Linux ONLY: waveOutGetPosition");
...
#ifdef _DEBUG
	FNX_FPRINTF(stderr, "Linux ONLY: waveOutGetID");
	FNX_FPRINTF(stderr, "(%04X, %p);\n", hWaveOut, lpuDeviceID);
#endif
...
#endif
  {
    FNX_PERROR("ioctl SNDCTL_DSP_RESET");
    wwo->hThread = 0;
...
/* First size and stereo then samplerate */
        ...
        FNX_PERROR("samplesize error");
        if (temp!=samplesize)
          FNX_FPRINTF(stderr,"new samplesize=%d\n",samplesize);
        temp=dsp_stereo;
        if (ioctl(audio, SNDCTL_DSP_CHANNELS, &dsp_stereo)==-1)
        FNX_PERROR("channels error");
        if (temp!=dsp_stereo)
          FNX_FPRINTF(stderr,"new dsp_stereo=%d\n",dsp_stereo);
        temp=smplrate;
        if (ioctl(audio, SNDCTL_DSP_SPEED, &smplrate)==-1)
        FNX_PERROR("smplrate error");
        if (temp!=smplrate)
          FNX_FPRINTF(stderr,"new smplrate=%d\n",smplrate);
#else
