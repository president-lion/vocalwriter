...
//#include <process.h> //moved to port.h for each platform - mlt
...
static int InitializeAudioParams(LPAUDIO_PARAMS_T pAudioParams)
{
	...
	pAudioParams->pcsAudioDeviceState =
		(LPCRITICAL_SECTION)FNX_MALLOC( sizeof( CRITICAL_SECTION ));
	...
	pAudioParams->pAudioQueue = (LPQUEUE_T)FNX_MALLOC(sizeof(QUEUE_T));
	...
	FNX_MALLOC(( AUDIO_BUFFER_SIZE) * sizeof(AUDIO_T));
								 ...
								 LPAUDIO_PARAMS_T *ppAudioParams)
{
  ...
  /*******************************************************/
	...
	*ppAudioParams = (LPAUDIO_PARAMS_T)FNX_MALLOC(sizeof(AUDIO_PARAMS_T));
...
//	EnterCriticalSection( pAudioParams->pcsQueueCount );
   ...
   /********************************************************************/
	   ...
	   )
	{
		...
		{
			char szTemp[256]="";
			FNX_SPRINTF(szTemp,"PA_Queue, detected RanDry; qcount=%d at %lu\n",
		...
		&& !pAudioParams->bInAudioReset)
	{
		...
		{
			char szTemp[256]="";
			FNX_SPRINTF(szTemp,"PA_Queue, kicking audio at %lu\n",
				timeGetTime());
			OutputDebugString(szTemp);
		}
#endif //AUD_DEBUG
...
MMRESULT PA_DestroyPlayHandle( LPAUDIO_PARAMS_T pAudioParams)
{
  ...
  {
	  char szTemp[256];
	  FNX_SPRINTF(szTemp,"PA_DestroyPlayHandle requesting hModeWnd shutdown at %llu\n",
		  timeGetTime());
	  OutputDebugString(szTemp);
  }
#endif //AUD_DEBUG
...
static void DestroyAudioObjects(LPAUDIO_PARAMS_T pAudioParams)
{
	...
	if ( pAudioQueue != NULL )
	{
	  ...
	  if ( pAudioQueue->pQueueStart != NULL )
		FNX_FREE( pAudioQueue->pQueueStart );

	  /****************************************************************/
	  /*  Free Memory for the Audio Queue Structure.                  */
	  /****************************************************************/

	  FNX_FREE( pAudioQueue );
	}
	...
	if ( pAudioParams->pcsQueueAudioBusy != NULL )
	{
	  ...
	  FNX_FREE( pAudioParams->pcsQueueAudioBusy );
	}
	...
	if ( pAudioParams->pcsQueueCount != NULL )
	{
	  ...
	  FNX_FREE( pAudioParams->pcsQueueCount );
	}
	...
	if ( pAudioParams->pcsAudioDeviceState != NULL )
	{
	  ...
	  FNX_FREE( pAudioParams->pcsAudioDeviceState );
	}
  ...
  while( ! pAudioParams->bTimerExit )
  {
	   ...
	   )
	{
...

		{
			...
			{
				char szTemp[256]="";
				FNX_SPRINTF(szTemp,"request unclaim check at %lu\n",
					timeGetTime());
				OutputDebugString(szTemp);
			}
#endif //AUD_DEBUG
						   ...
						   QWORD *pqwPosition)
{
  ...
  {
	  char szTemp[256]="";
	  FNX_SPRINTF(szTemp,"PA_GetQWPosition pos=%lu at %lu\n",
							   ...
							   LPDWORD pdwPosition )
{
  ...
  pMmt = (LPMMTIME)FNX_MALLOC(sizeof(MMTIME));
											  ...
											  LPWAVEFORMATEX pWaveFormat )
{
  ...
  else
  {
	...
	else
	{
	  ...
	  pWaveFormatNew =
		(LPWAVEFORMATEX)FNX_REALLOC( pAudioParams->pWaveFormat,
