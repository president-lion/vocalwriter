...
#if !defined _IOSSIMULATOR_ && !defined NO_SESSION 
...
err = AudioSessionSetProperty (
 ...
 if (err !=0)
  ...
  iOUnitDescription.componentFlagsMask     = 0;
    ...
    err = AUGraphAddNode(pPlayAudio->outGraph, &iOUnitDescription, &ioNode);

    // now do the wiring. The graph needs to be open before you call AUGraphNodeInfo
    err = AUGraphOpen(pPlayAudio->outGraph);
    
    err = AUGraphNodeInfo(pPlayAudio->outGraph, ioNode, 0, &ioUnit);
...
#if defined _IOSSIMULATOR_ //|| defined _MAC_OSX_
...
#if 0
      err=AUGraphConnectNodeInput(pPlayAudio->outGraph,
...
static void StartAudioPlaying( HPLAY_AUDIO_T pPlayAudio )
{
  ...
  else
  {
    ...
    if (pPlayAudio->outGraph) 
    {
        err = AUGraphIsRunning(pPlayAudio->outGraph, &isRunning);
        ...
        if(!isRunning)
	  {
		...
		if (err != noErr)
			FNX_PRINTF("error starting output graph\n");
	  }
    }
...
//  int vol_att;
...
#if defined USE_CORE_AUDIO && !defined _IOSSIMULATOR_ //&& !defined _MAC_OSX_
  for (i=0;i<(uiLength>>1);i++)
  {
    ...
    float f_value = ((float)s_value)/16384.0;
      ...
      (pPlayAudio->core_audio_float_output_buffer)[i*2]= f_value;
      (pPlayAudio->core_audio_float_output_buffer)[i*2+1]= f_value;
  }
  mmStatus=0;
#elif defined USE_PORTAUDIO || (defined USE_CORE_AUDIO && (defined _IOSSIMULATOR_ /*|| defined _MAC_OSX_*/))
  for (i=0;i<(uiLength>>1);i++)
  {
...
//	Sleep(100);
  ...
  // Set up a callback function to generate output to the output unit
    if (pPlayAudio->outGraph)
    {
      ...
      if(!outIsInitialized)
	{
        err = AUGraphInitialize(pPlayAudio->outGraph);
	}
    }
