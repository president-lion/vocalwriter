...
#define OutputDebugString(x) {\
        if (!(fpODS_File)) \
                fpODS_File = FNX_FOPEN("odslog.log","w"); \
...
#endif
#if !defined USE_PORTAUDIO && !defined _USE_CORE_AUDIO
...
#ifdef USE_PORTAUDIO
#define PORT_AUDIO_BUF_SIZE 71 //this is the number of samples
...
{
  volatile LPATYPE_T pStart;
  volatile LPATYPE_T pInput;
  volatile LPATYPE_T pOutput;
  volatile unsigned int uiInputPosition;
  volatile unsigned int uiOutputPosition;
  volatile unsigned int uiLength;
  volatile unsigned int uiCount;
  HEVENT_T hevNotFull;
...
ATYPE_T Process_MM_WOM_DONE_Message( HPLAY_AUDIO_T, ATYPE_T );
...
unsigned int WriteBufferToWaveDevice( HPLAY_AUDIO_T,
                                             LPWAVEHDR,
                                             unsigned int );
...
void interruptionListenerCallback (void    *inUserData, UInt32  interruptionState ) 
{
  ...
  HPLAY_AUDIO_T *pPlayAudio = inUserData;
    ...
    ProcessResumeMessage(pPlayAudio);
...
#ifdef IOS_AVSESSION
    (AVAudioSession *)AVSesson;
#elif defined USE_CORE_AUDIO
...
UInt32 audioRouteOverride = kAudioSessionOverrideAudioRoute_Speaker;
  ...
  *ppPlayAudio = NULL;
...
#if defined _UNIX_LIKE_ && !defined __osf__ && !defined USE_PORTAUDIO && !defined USE_CORE_AUDIO
  OSS_WaveInit();
#endif
  ...
  pPlayAudio = (HPLAY_AUDIO_T)FNX_MALLOC(sizeof(PLAY_AUDIO_T));
...
#endif
#ifdef IOS_AVSESSION
    AVSesson.category = AVAudioSessionCategoryPlayback;
    AVSesson.mode = AVAudioSessionModeDefault;
    AVSesson.preferredSampleRate = 11025.0;
    AVSession.
...
err = AudioSessionInitialize (
 ...
 if (err !=0)
   FNX_PRINTF("session not initialized\n");
...
err = AudioSessionSetProperty (
 ...
 if (err !=0)
   FNX_PRINTF("hw sample rate not set\n");
    //#endif
 
//#endif


  NewAUGraph(&pPlayAudio->outGraph);
  ...
  iOUnitDescription.componentFlagsMask     = 0;
    ...
    AUGraphAddNode(pPlayAudio->outGraph, &iOUnitDescription, &ioNode);

    // now do the wiring. The graph needs to be open before you call AUGraphNodeInfo
    AUGraphOpen(pPlayAudio->outGraph);
    
    AUGraphNodeInfo(pPlayAudio->outGraph, ioNode, 0, &ioUnit);
    

  input.inputProc = CoreAudioCallbackFunction;
  input.inputProcRefCon = pPlayAudio;
 ...
 //   | kAudioFormatFlagIsNonInterleaved;
  ...
  streamFormat.mSampleRate = 11025.0;		//	the sample rate of the audio stream
  streamFormat.mFormatID = kAudioFormatLinearPCM;	//	the specific encoding type of audio stream
  streamFormat.mFormatFlags = kLinearPCMFormatFlagIsFloat; //SignedInteger ;
 //   | kAudioFormatFlagsNativeEndian
    //| kLinearPCMFormatFlagIsPacked;
 //   | kAudioFormatFlagIsNonInterleaved;
  streamFormat.mBytesPerPacket = sizeof(float) * 256 * 2;
  streamFormat.mFramesPerPacket = 256;
...
//      AUGraphConnectNodeInput(pPlayAudio->outGraph,
//                          0, 0, // srcnode, inSourceOutputNumber
...
AudioUnitSetProperty (
  ...
  pPlayAudio->port_audio_output_buffer=NULL;
  pPlayAudio->core_audio_float_output_buffer=NULL;
  pPlayAudio->core_audio_float_output_buffer2=NULL;
  pPlayAudio->port_audio_samples_per_buffer=0;
  pPlayAudio->ClosePortAudioStream=0;
  pPlayAudio->FirstBuffer=0;
#endif
  ...
  {
    pShm_t->ppGlobalHandleList = (HPLAY_AUDIO_T *)FNX_MALLOC( sizeof(HPLAY_AUDIO_T ));
  ...
  if ( ! bHandleSaved )
  {
    ppGlobalHandleListTemp = ( HPLAY_AUDIO_T * )
      FNX_REALLOC( pShm_t->ppGlobalHandleList,
  ...
  pPlayAudio->ppWaveHdr =
    (LPWAVEHDR *)FNX_MALLOC( MAXIMUM_BUFFERS_QUEUED * sizeof(LPWAVEHDR));
  ...
  pPlayAudioRing = (LPPLAY_RING_T)FNX_MALLOC(sizeof(PLAY_RING_T));
...
/* TQL 05/21/1997  change this for OSF build */
  ...
  else
  {
    ...
    {
      char  szTemp[256]="";
      FNX_SPRINTF(szTemp,"PA_: device is not WAVE_MAPPER, it is: %i\n"
	      ,uiWaveOutDeviceID);
  ...
  if ( pPlayAudio != NULL )
      ...
      if ( pPlayAudio->bHandleValid )
	{
	  ...
	  if ( pPlayAudio->dwDeviceOptions & PA_OWN_DEVICE )
	    {
#ifdef USE_PORTAUDIO
		  pa_error = Pa_AbortStream(pPlayAudio->hWaveOut);
		  pa_error = Pa_CloseStream(pPlayAudio->hWaveOut);
		  if (pa_error == 0)
		    Process_MM_WOM_CLOSE_Message(pPlayAudio);
#elif defined USE_CORE_AUDIO
        AUGraphIsRunning(pPlayAudio->outGraph, &isRunning);

        if(isRunning)
	  {
          err = AUGraphStop(pPlayAudio->outGraph);
	  }
          //  err = AudioOutputUnitStop (pPlayAudio->hWaveOut);

      AUGraphIsInitialized(pPlayAudio->outGraph, &outIsInitialized);
      
      if(outIsInitialized)
          err = AUGraphUninitialize(pPlayAudio->outGraph);
          //  err =  AudioUnitUninitialize (pPlayAudio->hWaveOut);
	  //if (err == 0)
      DTAudioSessionSetActive(false);

      err= DisposeAUGraph(pPlayAudio->outGraph);
      ...
      if ( pShm_t->uiGlobalPlayAudioInstance == 0 )
	{
	  ...
	  if ( pShm_t->ppGlobalHandleList != NULL )
	    {
	      FNX_FREE( pShm_t->ppGlobalHandleList );
...
#endif
#ifdef USE_CORE_AUDIO
//	AudioComponentClose (pPlayAudio->hWaveOut);
#endif
    FNX_FREE( pPlayAudio );
                   ...
                   DWORD dwLength )
{
  ...
  {
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"PA_Queue: size=%lu at %lu\n",
...
MMRESULT PA_Reset( HPLAY_AUDIO_T pPlayAudio )
{
  ...
  {
    char szTemp[256]="";
    FNX_SPRINTF(szTemp,"PA_Reset: at %ld\n",timeGetTime());
    OutputDebugString(szTemp);
  }
#endif //AUD_DEBUG
  ...
  /********************************************************************/
#if !defined USE_PORTAUDIO && !defined USE_CORE_AUDIO
  ...
  //#ifdef USE_CORE_AUDIO
  //  ProcessResetMessage(pPlayAudio);
  //#else
  ...
  {  
	  ...
	  {
		  char szTemp[256];
		  FNX_SPRINTF(szTemp,"PA_Reset resending ID_Reset_Audio at %ld\n",
			  timeGetTime());
		  OutputDebugString(szTemp);
	  }
#endif //AUD_DEBUG
                       ...
                       LPWAVEFORMATEX pWaveFormat )
{
  ...
  streamFormat.mFormatFlags = kLinearPCMFormatFlagIsSignedInteger 
    ...
    | kLinearPCMFormatFlagIsPacked
    | kAudioFormatFlagIsNonInterleaved;
  streamFormat.mBytesPerPacket = 142;	
  streamFormat.mFramesPerPacket = 71;	
  streamFormat.mBytesPerFrame = 2;		
  streamFormat.mChannelsPerFrame = 1;	
  streamFormat.mBitsPerChannel = 16;	
  
  err = AudioUnitSetProperty (pPlayAudio->hWaveOut,
			      kAudioUnitProperty_StreamFormat,
			      kAudioUnitScope_Input,
			      0,
			      &streamFormat,
...
//#include <stdio.h> //moved to port.h for each platform - mlt
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"PA_SetFormat: SendPlayAudioMessage status=%lu (%08lx)\n",
			  mmStatus, mmStatus);
		  OutputDebugString(szTemp);
  }
#endif //AUD_DEBUG
                         ...
                         unsigned int bFast )
{
  ...
  else
  {
    ...
    pdwPos = (LPDWORD)FNX_MALLOC(sizeof(DWORD));
                                            ...
                                            ATYPE_T aParam )
{
  ...
  {
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"ProcWOMOPEN: outstanding now %d at %08lu\n",
			  pPlayAudio->iOutstandingBuffers,timeGetTime());
		  OutputDebugString(szTemp);
		  if ( pPlayAudio->dwDeviceOptions & PA_OWN_DEVICE )
		  {
			  FNX_SPRINTF(szTemp,"ProcWOMOPEN: PA_OWN_DEVICE is SET\n");
		  }
		  else
		  {
			  FNX_SPRINTF(szTemp,"ProcWOMOPEN: PA_OWN_DEVICE is NOT set\n");
		  }
		  OutputDebugString(szTemp);
  }
#endif //AUD_DEBUG
...
static void StartAudioPlaying( HPLAY_AUDIO_T pPlayAudio )
{
  ...
  {
    char  szTemp[256]="";
    FNX_SPRINTF(szTemp,"SAP: state=%ld QCount=%d new=%d outstanding=%d at %lu\n",
    ...
    if (pPlayAudio->outGraph) 
    {
        ...
        if(!isRunning)
	  {
	    DTAudioSessionSetActive (true);
            ...
            AUGraphStart(pPlayAudio->outGraph);
	  }
    }
...
//    OP_LockMutex( pPlayAudio->hmxQueueCount );
      ...
      && ( pPlayAudio->bPaused ))
    {
	  ...
	  {
          err = AUGraphStop(pPlayAudio->outGraph);
	  err = DTAudioSessionSetActive (false);
	  }
 //		AudioOutputUnitStop (pPlayAudio->hWaveOut);
#else
...
//		ClosePlayDeviceRequest( pPlayAudio );
		...
		{
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"SAP: iQueueCount=0 abort at %lu\n",
			  timeGetTime()
			  );
		  OutputDebugString(szTemp);
		}
  ...
  {
    char  szTemp[256]="";
    FNX_SPRINTF(szTemp,"SAP: %d samples to write at %lu\n",
	    uiQueueCount,
	    timeGetTime()
	    );
    OutputDebugString(szTemp);
	...
	uiWriteLength&=~0x03;
	  ...
	  if (pPlayAudio->lpDataArray[uiI].used == 0)
	  {

		  pPlayAudio->lpDataArray[uiI].used=1;
	  ...
	  else
	  {
		  ...
		  {
		    char  szTemp[256]="";
		    FNX_SPRINTF(szTemp,"SAP: header %d (%08lx) is busy at %lu\n",
			    uiI,
			    pPlayAudio->ppWaveHdr[uiI],
			    timeGetTime()
			    );
  ...
  {
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"SAP: exit at %lu\n",
			  timeGetTime()
			  );
		  OutputDebugString(szTemp);
  }
...
ATYPE_T Process_MM_WOM_DONE_Message( HPLAY_AUDIO_T pPlayAudio,
                                            ATYPE_T aParam )
{
  int dwRemaining=0;
  int olddwRemaining=0;
  ...
  /********************************************************************/
	...
	{
		uiOldWriteLength =  pPlayAudio->port_audio_samples_per_buffer<<1;
	}
	else
	{
		uiOldWriteLength = pPlayAudio->port_audio_samples_per_buffer<<1;
  ...
  for (uiI=0;uiI<MAXIMUM_BUFFERS_QUEUED;uiI++)
  {
	  if (pPlayAudio->lpDataArray[uiI].lpDataArray==pWaveHdr->lpData)
	  {
		  break;
	  }
  }
  if (uiI<MAXIMUM_BUFFERS_QUEUED)
  _ASSERTE(pPlayAudio->lpDataArray[uiI].used!=0); // tek 22jan99 make sure this isn't a bogus
									  // and/or idle header
...
//#endif
  ...
  {
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"ProcWOMDONE: %08lx; outst buffs now=%d; samp waiting=%dat %08lu\n",
			  pWaveHdr,
			  pPlayAudio->iOutstandingBuffers,
			  pPlayAudio->iSamplesWaitingForCompletion,
			  timeGetTime()
  ...
  {
    char  szTemp[256]="";
    FNX_SPRINTF(szTemp,"ProcWOMDONE: iQueueCount=%ld\n",
	    pPlayAudioRing->iQueueCount);
    OutputDebugString(szTemp);
    
    if (pPlayAudioRing->iQueueCount < 0)
      {
	FNX_SPRINTF(szTemp,"ProcWOMDONE: iQueueCount is negative? %d at %08lu\n",
		pPlayAudioRing->iQueueCount,
		timeGetTime()
		);
	OutputDebugString(szTemp);
  ...
  {
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"ProcWOMDONE: Closing. iQueueCount=%ld\n",
					pPlayAudioRing->iQueueCount);
		  OutputDebugString(szTemp);
  }
#endif //AUD_DEBUG
  ...
  for (uiI=0;uiI<MAXIMUM_BUFFERS_QUEUED;uiI++)
  {
	  if (pPlayAudio->lpDataArray[uiI].lpDataArray==pWaveHdr->lpData)
	  {
		  pPlayAudio->lpDataArray[uiI].used=0;
		  break;
	  }
  }
...
#if defined USE_PORTAUDIO || defined USE_CORE_AUDIO
  OP_UnlockMutex( pPlayAudio->hmxQueueCount );
		return(2); // SS TODO this may be a problem
#else
	if (((LPTTS_HANDLE_T)(pPlayAudio->aInstance))->uiQueuedCharacterCount>0 ||
		pipe_count( ((LPTTS_HANDLE_T)(pPlayAudio->aInstance))->pKernelShareData->cmd_pipe )>0)
     ClosePlayDeviceRequest( pPlayAudio );
     // ClosePlayDevice( pPlayAudio );
	else
      ClosePlayDevice( pPlayAudio );
  ...
  {
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"ProcWOMDONE: dwRemaining=%ld\n",
					dwRemaining);
		  OutputDebugString(szTemp);
  }
#endif //AUD_DEBUG
      ...
      if ( dwRemaining > 0 )
      {
		...
		else if (uiWriteSize<(PORT_AUDIO_BUF_SIZE<<1))
		{
			printf("drift of %d samples\n",((PORT_AUDIO_BUF_SIZE<<1)-uiWriteSize)>>1);
		}
#elif defined USE_CORE_AUDIO

		if (uiWriteSize>(pPlayAudio->port_audio_samples_per_buffer<<1))
		{
			uiWriteSize=(pPlayAudio->port_audio_samples_per_buffer<<1);
			pPlayAudio->core_audio_samples_not_written = 0;		
		}
		else if (uiWriteSize<(pPlayAudio->port_audio_samples_per_buffer<<1))
		{
		  //printf("drift of %d samples\n",((pPlayAudio->port_audio_samples_per_buffer<<1)-uiWriteSize)>>1);
		  pPlayAudio->core_audio_samples_not_written = 	((pPlayAudio->port_audio_samples_per_buffer<<1)-uiWriteSize)>>1;
		}
  ...
  for (uiI=0;uiI<MAXIMUM_BUFFERS_QUEUED;uiI++)
  {
	...
	olddwRemaining = dwRemaining;
			  ...
			  uiI++ )
		{
		  ...
		  if (pPlayAudio->lpDataArray[uiI].used == 0)
		  {
		    ...
		    {
		      char  szTemp[256]="";
		      FNX_SPRINTF(szTemp,"ProcWOMDONE: dwRemaining=%ld, uiWriteLength=%d, using header %d (%08lx) at %lu\n",
			      dwRemaining,
			      uiWriteSize,
			      uiI,
			      pPlayAudio->ppWaveHdr[uiI],
	...
	uiWriteSize&=0x03;
		  ...
		  else
		  {
			  ...
			  {
					  char  szTemp[256]="";
					  FNX_SPRINTF(szTemp,"ProcWOMDONE: header %d (%08lx) is busy at %lu\n",
						  uiI,
						  pPlayAudio->ppWaveHdr[uiI],
						  timeGetTime()
						  );
...
ATYPE_T Process_MM_WOM_CLOSE_Message( HPLAY_AUDIO_T pPlayAudio )
{
#ifdef AUD_DEBUG	 // tek 13may97
  {
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"ProcWOMCLOSE: outstanding now %d at %08lu\n",
			  pPlayAudio->iOutstandingBuffers,
			  timeGetTime()
			  );
		  OutputDebugString(szTemp);
...
unsigned int WriteBufferToWaveDevice( HPLAY_AUDIO_T pPlayAudio,
                                             LPWAVEHDR pWaveHdr,
...
//  int vol_att;
...
#ifdef AUD_DEBUG  //tek 13may97
  {
		  char  szTemp[256]="";
		  pPlayAudioRing = pPlayAudio->pPlayAudioRing;
		  FNX_SPRINTF(szTemp,"WBTWOD size=%d at %08lx; out=%d; waiting=%d qcount=%d at %08lu\n",
			  uiLength, 
			  pWaveHdr,
			  pPlayAudio->iOutstandingBuffers, 
			  pPlayAudio->iSamplesWaitingForCompletion,
                                    ...
                                    > pPlayAudioRing->iQueueLength )
  {
	...
	memmove( pPlayAudioRing->pQueueStart - iNumberToMoveToQueueTop,
...
#endif
#if !defined USE_PORTAUDIO && !defined USE_CORE_AUDIO
	//pWaveHdr->lpData=FNX_MALLOC(uiWriteLength);
	memcpy(pWaveHdr->lpData,(char *)pPlayAudioRing->pQueueOutput,uiWriteLength);
...
#if defined USE_CORE_AUDIO && !defined _IOSSIMULATOR_
  for (i=0;i<(uiLength>>1);i++)
  {
    short s_value = (((short *)(pPlayAudioRing->pQueueOutput))[i]);
#ifdef _IOSSIMULATOR_
    float f_value = ((float)s_value)/16384.0;
#else
    float f_value = ((float)s_value)/16384.0;
#endif
      (pPlayAudio->core_audio_float_output_buffer)[i]= f_value; 
      if (pPlayAudio->core_audio_float_output_buffer2!=NULL)
	(pPlayAudio->core_audio_float_output_buffer2)[i]= f_value;
  }
  mmStatus=0;
#elif defined USE_PORTAUDIO || (defined USE_CORE_AUDIO && defined _IOSSIMULATOR_)
  for (i=0;i<(uiLength>>1);i++)
  {
  ((short *)(pPlayAudio->port_audio_output_buffer))[i*2]=
	((short *)(pPlayAudioRing->pQueueOutput))[i];
  ((short *)(pPlayAudio->port_audio_output_buffer))[i*2+1]=
  	((short *)(pPlayAudioRing->pQueueOutput))[i];
  }
  ...
  if (uiLength<(PORT_AUDIO_BUF_SIZE<<1))
  {
	...
	//	  (PORT_AUDIO_BUF_SIZE<<1)-uiLength);
	  memset(pPlayAudio->port_audio_output_buffer+uiLength*2,0,
		  (PORT_AUDIO_BUF_SIZE<<2)-uiLength*2);
  }
#endif
  mmStatus=0;
#else
...
//	Sleep(100);
  ...
  // Set up a callback function to generate output to the output unit
    if (pPlayAudio->outGraph)
    {
      ...
      if(!outIsInitialized)
        AUGraphInitialize(pPlayAudio->outGraph);
    }
...
//#include <stdio.h> //moved to port.h for each platform - mlt
		  char szTemp[256]="";
		  FNX_SPRINTF(szTemp,"OpenWaveOutputDevice: waveOutOpen failed: status=%lu (%08lx)\n",
			  mmStatus, mmStatus);
		  OutputDebugString(szTemp);
	  }
#endif //AUD_DEBUG
...
static ATYPE_T ProcessResetMessage( HPLAY_AUDIO_T pPlayAudio )
{
  ...
  if (( pPlayAudio->dwAudioDeviceState == AUDIO_DEVICE_ACTIVE )
	...
	OP_Sleep(25);
//          err = AudioOutputUnitStop (pPlayAudio->hWaveOut);
	// close the devie to get it to stop playing
	ClosePlayDevice( pPlayAudio );
	pPlayAudio->dwAudioPlayState = AUDIO_STATE_PLAY;
...
//          err = AudioOutputUnitStop (pPlayAudio->hWaveOut);
	OP_Sleep(25);
	// close the device to get it to stop playing
	ClosePlayDevice( pPlayAudio );
	pPlayAudio->dwAudioPlayState = AUDIO_STATE_PLAY;
      ...
      {
		  char szTemp[256]="";
		  FNX_SPRINTF(szTemp,"ProcResetMsg: calling waveOutReset at %ld\n",
			  timeGetTime()
			  );
		  OutputDebugString(szTemp);
	  }
...
//tek 22may97
  ...
  {
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"ResetAudioQueue: at %08lu\n",
			  timeGetTime()
			  );
		  OutputDebugString(szTemp);
  }
  ...
  for ( uiI = 0; uiI < MAXIMUM_BUFFERS_QUEUED; uiI++ )
  {
      pPlayAudio->ppWaveHdr[uiI]->lpData = pPlayAudio->lpDataArray[uiI].lpDataArray;
 	  pPlayAudio->lpDataArray[uiI].used=0;
  }
  ...
  {
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"ResetAudioQueue: exit at %08lu\n",
			  timeGetTime()
			  );
		  OutputDebugString(szTemp);
  }
...
static ATYPE_T ProcessResumeMessage( HPLAY_AUDIO_T pPlayAudio )
{
  ...
  if ( pPlayAudio->dwAudioDeviceState == AUDIO_DEVICE_ACTIVE )
  {
	  ...
	  Pa_StartStream(pPlayAudio->hWaveOut);
#elif defined USE_CORE_AUDIO
	  err = DTAudioSessionSetActive (true);
          err = AUGraphStart(pPlayAudio->outGraph);
          //	err = AudioOutputUnitStart (pPlayAudio->hWaveOut);
#else
                                                  ...
                                                  ATYPE_T aTag )
{
  ...
  pSyncMark = (LPSYNC_MARK_T)FNX_MALLOC( sizeof(SYNC_MARK_T));
...
static LPMESSAGE_QUEUE_T CreateMessageQueue( unsigned int uiQueueLength )
{
  ...
  pMessageQueue = (LPMESSAGE_QUEUE_T)FNX_MALLOC( sizeof( MESSAGE_QUEUE_T ));
  ...
  pMessageQueue->pStart =
    (LPATYPE_T)FNX_MALLOC( uiBufferLength * sizeof(ATYPE_T));
  ...
  pMessageQueue->hevNotEmpty = OP_CreateEvent( FALSE, FALSE );
  ...
  pMessageQueue->hevNotFull = OP_CreateEvent( FALSE, TRUE );
...
static void DestroyMessageQueue( LPMESSAGE_QUEUE_T pMessageQueue )
{
  if ( pMessageQueue != NULL )
  {
    ...
    if ( pMessageQueue->pStart != NULL )
    {
      FNX_FREE( pMessageQueue->pStart );
    }

    FNX_FREE( pMessageQueue );
  }
...
{
	int ret=0;
	HPLAY_AUDIO_T pPlayAudio=((HPLAY_AUDIO_T)inRefCon);
	...
	if (pPlayAudio->dwAudioPlayState == AUDIO_STATE_RESET)
	{
        *ioActionFlags = kAudioUnitRenderAction_OutputIsSilence;
        // set reset flag in AudioUnitRenderActionFlags
	//ioData->mBuffers[0].mDataByteSize = 0;
	return(0);
	//  return(1);
	}

	pPlayAudio->core_audio_float_output_buffer=(float *)ioData->mBuffers[0].mData;
	if (ioData->mNumberBuffers > 1)
		pPlayAudio->core_audio_float_output_buffer2=(float *)ioData->mBuffers[1].mData;
	else
		pPlayAudio->core_audio_float_output_buffer2=NULL;
	pPlayAudio->port_audio_output_buffer=(char *)ioData->mBuffers[0].mData;
	pPlayAudio->port_audio_samples_per_buffer=inNumberFrames;
	
	ret=Process_MM_WOM_DONE_Message(pPlayAudio,0);
	//ioData->mBuffers[0].mDataByteSize = ioData->mBuffers[0].mDataByteSize - (pPlayAudio->core_audio_samples_not_written * 2);
	if (ret)
	{
		ClosePlayDeviceEx(pPlayAudio,1);
	}
...
void PA_DoSync(HPLAY_AUDIO_T pPlayAudio)
{
...
#ifdef AUD_DEBUG
  {
	  // tek 12nov97 corrected as part of the BATS508 edits
		  char  szTemp[256]="";
		  FNX_SPRINTF(szTemp,"DoSync at %08lu\n",
			  timeGetTime()
			  );	  
		  OutputDebugString(szTemp);
  }
