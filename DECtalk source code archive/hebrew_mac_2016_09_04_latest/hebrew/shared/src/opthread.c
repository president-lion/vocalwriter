                           ...
                           void * pThreadData )
{
	...
	pThread = (HTHREAD_T)FNX_MALLOC( sizeof(pthread_t));
	...
	{
		FNX_PRINTF("invalid detach state\n");
		return(NULL);
	}
#endif
	...
	FNX_PRINTF("getscheduler returned%d sched_fifo=%d sched_rr=%d\n",policy,SCHED_FIFO,SCHED_RR);
...
HMUTEX_T OP_CreateMutex()
{
  ...
  pMutex = (HMUTEX_T)FNX_MALLOC( sizeof(MUTEX_T));
                         ...
                         unsigned int bInitialState )
{
  ...
  pEvent = (HEVENT_T)FNX_MALLOC( sizeof(EVENT_T));
  ...
  if ( pEvent != NULL )
  {
    ...
    pEvent->pSignalMutex =
      (pthread_mutex_t *)FNX_MALLOC( sizeof(pthread_mutex_t));
    ...
    pEvent->pSignalCondition =
      (pthread_cond_t *)FNX_MALLOC( sizeof(pthread_cond_t));
...
void OP_DestroyEvent( HEVENT_T pEvent )
{
  ...
  if ( pEvent != NULL )
  {
    ...
    if ( pEvent->pSignalCondition != NULL )
    {
      ...
      FNX_FREE( pEvent->pSignalCondition );
    }
...
//moved to port.h for each platform - mlt
//#include <stdio.h> // for FNX_SPRINTF()
...
BOOL ThreadLock(TLOCK *ptlLock,DWORD dwTimeout)
{
	...
	while (0 != InterlockedExchange((long *)(&ptlLock->dwLock),1))
	{
		...
		{
			char szTemp[256];
			FNX_SPRINTF(szTemp,"TLock: thread %08lx FAILED to lock %08lx, owned by %08lx at %lu\n",
	...
	if (bGotLock) // if we own it, by hook or by crook..
	{
		...
		{
			char szTemp[256];
			FNX_SPRINTF(szTemp,"TLock: thread %08lx locked %08lx, count %08lx at %lu\n",
...
void ThreadUnlock(TLOCK *ptlLock)	
{
...
#ifdef VERIFY_TLOCKS
	// verify that the lock is locked.
	FNX_ASSERT(ptlLock->dwLock == 1);
#endif //VERIFY_TLOCKS
	// ALWAYS verify that we're the owner..
	FNX_ASSERT(ptlLock->dwThread == dwThreadId);
#endif
	...
	ptlLock->dwCount--;
		...
		{
			char szTemp[256];
			FNX_SPRINTF(szTemp,"TLock: thread %08lx unlocking %08lx, count %08lx at %lu\n",
				dwThreadId, (DWORD)ptlLock, ptlLock->dwCount, timeGetTime());
			OutputDebugString(szTemp);
		}
#endif //_DEBUG_TLOCK
	...
	if (ptlLock->dwCount == 0)
	{
		...
		{
			char szTemp[256];
			FNX_SPRINTF(szTemp,"TLock: thread %08lx unlocked %08lx at %lu\n",
...
VOID tekInitializeCriticalSection(LPTEKCRITICAL_SECTION lpCriticalSection, char *szIdent)
{
	...
	FNX_SPRINTF(szTemp, "CS 0x%08x (%s) initialized by thread %08lx at %ld\n",
	...
	FNX_STRNCPY(lpCriticalSection->szIdent,szIdent,64);
...
{
	char szTemp[256];
	FNX_SPRINTF(szTemp, "CS 0x%08x deleted by thread %08lx at %ld\n",
...
VOID tekEnterCriticalSection( LPTEKCRITICAL_SECTION lpCriticalSection)
{
	...
	FNX_SPRINTF(szTemp, "-->CS 0x%08x entering by thread %08lx at %ld\n",
		lpCriticalSection, GetCurrentThread(),timeGetTime());
	OutputDebugString(szTemp);
	...
	{
		// we'll block.
		FNX_SPRINTF(szTemp,"CS 0x%08lx (%s) will block thread %08lx; owned by %08lx at %ld\n",
	...
	{
		// made it in..
		FNX_SPRINTF(szTemp,"Blocked CS 0x%08lx entered by %08lx at %ld\n",
	...
	if (lpCriticalSection->dwThreadid == 0)
	{
		...
		{
			FNX_SPRINTF(szTemp,"***??? CS %08lx (%s) lock count not zero?\n",
				lpCriticalSection, lpCriticalSection->szIdent);
			OutputDebugString(szTemp);
		}
...
#ifdef _LOG_CRITICAL_SECTIONS
	FNX_SPRINTF(szTemp, "---->CS 0x%08x entered by thread %08lx at %ld\n",
		lpCriticalSection, dwThreadId,timeGetTime());
	OutputDebugString(szTemp);
#endif //LOG_CRITICAL_SECTIONS
...
VOID tekLeaveCriticalSection( LPTEKCRITICAL_SECTION lpCriticalSection )
{
	char szTemp[256];
#ifdef _LOG_CRITICAL_SECTIONS
	FNX_SPRINTF(szTemp, "<--CS 0x%08x leaving by thread %08lx at %ld\n",
		lpCriticalSection, GetCurrentThread(),timeGetTime());
	OutputDebugString(szTemp);
#endif //LOG_CRITICAL_SECTIONS
