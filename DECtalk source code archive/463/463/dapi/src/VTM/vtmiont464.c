...
const char *const replace_filelist[]={
	"",
	"ble.pcm",
	"res.pcm",
	"takeexit.pcm",
	  ...
	  if (( ! pKsd_t->halting ) && ( ! phTTS->bMemoryReset ))
	    {
		  ...
		  //printf("phon = %d AG %f ",pVtm_t->parambuff[OUT_PH+1], frame.ag);
		...
		pVtm_t->llframe.NF1 = pVtm_t->parambuff[OUT_F1+1];
		pVtm_t->llframe.NF2 = pVtm_t->parambuff[OUT_F2+1];
		pVtm_t->llframe.NF3 = pVtm_t->parambuff[OUT_F3+1];
		  
  //write these variable because normally hlsyn fills them in
	...
	if(DT_DBG(PH_DBG,0x040))
	{
		  ...
		  printf(" F0 %d ", pVtm_t->parambuff[OUT_T0+1]);
		  printf(" F1  %d", pVtm_t->parambuff[OUT_F1+1]);
		  printf(" F2  %d ", pVtm_t->parambuff[OUT_F2+1]);
		  printf(" F3 %d ", pVtm_t->parambuff[OUT_F3+1]);
#ifdef HLSYN
		 ...
		 // printf("b1 = %d ",(pVtm_t->parambuff[OUT_B1+1]));
		  ...
		  printf(" TPS %d ", pVtm_t->parambuff[OUT_GPS+1]);
		 ...
		 printf(" av  %d ",pVtm_t->parambuff[OUT_AV+1]);
	...
	if(1)//pKsd_t->lang_curr == LANG_italian)
	{



		pVtm_t->f2_ref = pVtm_t->llframe.NF2 + pVtm_t->f2_ref_scale;
		pVtm_t->f3_ref = pVtm_t->llframe.NF3 + pVtm_t->f3_ref_scale;
...
*/
	...
	else if(pVtm_t->parambuff[OUT_A2+1] == 3100)
	{
		...
		if(pVtm_t->parambuff[OUT_PH+1] == USP_HX)
			pVtm_t->llframe.NA2F = 30;
		if(pVtm_t->parambuff[OUT_PH+1] == UKP_HX
			|| pVtm_t->parambuff[OUT_PH+1] == GRP_H)
...
//		{
//			pVtm_t->llframe.NB1 +=(9500/(llframe.NF2-pVtm_t->llframe.NF1))*(5500/(pVtm_t->llframe.NF2-pVtm_t->llframe.NF1));
...
//		}
  ...
  //llframe.NATV=0;
	...
	if(DT_DBG(PH_DBG,0x040))
	{
			...
			printf(" ab  %d ",pVtm_t->parambuff[OUT_AB+1]);
			printf(" al  %d ",pVtm_t->parambuff[OUT_AL+1]);
			printf(" ATB  %d \n",pVtm_t->parambuff[OUT_ATB+1]);
...
//	pVtm_t->parambuff[OUT_FNP+1] = pVtm_t->llframe.NFNP;
	...
	if((pVtm_t->llframe.NFNP > pVtm_t->llframe.NF1))
	{
	...
	{
	pVtm_t->parambuff[OUT_FNP+1] = pVtm_t->llframe.NF1;
	pVtm_t->parambuff[OUT_BNP+1] = pVtm_t->llframe.NB1;
	pVtm_t->parambuff[OUT_F1+1] =pVtm_t->llframe.NFNP;
	pVtm_t->parambuff[OUT_B1+1] = pVtm_t->llframe.NBNP;
	}
	//pVtm_t->parambuff[OUT_B1+1] = pVtm_t->llframe.NB1;
...
//	
	...
	if(DT_DBG(PH_DBG,0x100))
	{
		...
		printf ("%s \n", phprint(pVtm_t->parambuff[OUT_PH+1]));
//		printf(" OQ %d ", pVtm_t->parambuff[OUT_OQ+1]);
		printf(" A2 %d ", pVtm_t->parambuff[OUT_A2+1]);
		printf("  %d ", pVtm_t->parambuff[OUT_A3+1]);
		printf("  %d ", pVtm_t->parambuff[OUT_A4+1]);
		printf("  %d ", pVtm_t->parambuff[OUT_A5+1]);
		printf(" %d ",  pVtm_t->parambuff[OUT_A6+1]);
		printf(" %d ", pVtm_t->parambuff[OUT_AB+1]);
		printf(" AH %d \n", pVtm_t->parambuff[OUT_AP+1]);
		printf(" AF %d ", pVtm_t->parambuff[OUT_GF+1]);
//		printf(" BR %d ", pVtm_t->parambuff[OUT_BR+1]);
		printf(" TLT %d ", pVtm_t->parambuff[OUT_TLT+1]);
		printf(" F0 %d ", (40000/pVtm_t->parambuff[OUT_T0+1]));
		printf(" AV %d ", pVtm_t->parambuff[OUT_AV+1]);
		printf(" F1 %d ", pVtm_t->parambuff[OUT_F1+1]);
		printf(" B1 %d ", pVtm_t->parambuff[OUT_B1+1]);
		printf(" B2 %d ", pVtm_t->parambuff[OUT_B2+1]);
		printf(" B3 %d ", pVtm_t->parambuff[OUT_B3+1]);
		printf(" F2 %d ", pVtm_t->parambuff[OUT_F2+1]);
		printf(" F3 %d ", pVtm_t->parambuff[OUT_F3+1]);
		printf(" F4 %d ", pVtm_t->parambuff[OUT_F4+1]);

		//printf(" F5 %d ", pVtm_t->llframe.NF5);
		printf(" nz %d np  %d ", pVtm_t->parambuff[OUT_FZ+1],pVtm_t->parambuff[OUT_FNP+1]);
		printf("\n ");
...
else // from if italian
{
  ...
  if (( phTTS->dwOutputState == STATE_OUTPUT_MEMORY )
	...
	if (pVtm_t->control==(SPC_type_index|SPC_subtype_replace_start))
	{
		...
		inwav=fopen(replace_filelist[pVtm_t->dwSyncParams[1]],"rb");
		if (inwav!=NULL)
		{
			number=1;
			while(number && !feof(inwav))
			{
				number=fread(samples,2,512,inwav);
				if (number)
				{
		 ...
		 DWORD NextPhone)
{
  ...
  case STATE_OUTPUT_AUDIO:
	...
	/* now it just does mono to stereo */
       newbuffer=malloc(sizeof(short)*(uiSamplesToOutput*2+1));
