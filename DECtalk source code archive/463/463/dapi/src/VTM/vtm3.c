...
if (1)
{
	...
	r2pg = amptable[A2inDB];  /*  Convert dB to linear */
	r3pg = amptable[A3inDB];  /*  Convert dB to linear */
	r4pa = amptable[A4inDB];  /*  Convert dB to linear */
	r5pa = amptable[A5inDB];  /*  Convert dB to linear */
	r6pa = amptable[A6inDB];  /*  Convert dB to linear */
	ABlin= amptable[ABinDB];  /*  Convert dB to linear */
	}
	else
	{
	if(APinDB > 10)
	APlin = amptable[APinDB+14];    /*  Convert dB to linear */
	else
	APlin = 0; //this saves computes
	r2pg = amptable[A2inDB ];  /*  Convert dB to linear */
	r3pg = amptable[A3inDB ];  /*  Convert dB to linear */
	r4pa = amptable[A4inDB+4];  /*  Convert dB to linear */
...
if(1)//pKsd_t->lang_curr == LANG_italian)
{
	if(pVtm_t->AFcgain)
	{
		pVtm_t->AFcgain = (pVtm_t->AFcgain) + (pVtm_t->AFgain - 55);
		if(pVtm_t->AFcgain < 0)
			pVtm_t->AFcgain=0;
	}
...
#ifdef NEW_VTM
#ifdef HLSYN
				//temp=(18596>>1)+(amptable[pVtm_t->vrnpg]<<4);
			pVtm_t->rnpa = d2pole_cf123( pVtm_t,pKsd_t, &pVtm_t->rnpb, &pVtm_t->rnpc,FNPinHZ , 200 ,18596 );
#else
			 d2pole_cf123( pVtm_t,pKsd_t, &pVtm_t->rnpb, &pVtm_t->rnpc,FNPinHZ , 80 ,18596 );
#endif
#endif
	...
	//			}
		...
		*/
		...
		voice += frac1mul(pVtm_t->aturb1 , noiseb );
...
skip_cascade:
		...
		/******************************************************************/
#ifdef MULTIPLE_LANGUAGES_LOADED	//trying not to do the check on any platform that doesn't have to
		...
		two_pole_filter( noise+impulse, pVtm_t->r6pd1, pVtm_t->r6pd2, r6pa,pVtm_t->R6pb, pVtm_t->r6pc );
		...
		{
			two_pole_filter( noise+impulse, pVtm_t->r5pd1, pVtm_t->r5pd2, r5pa, pVtm_t->R5pb, pVtm_t->r5pc );
		}
		else
		{
			pVtm_t->r5pd1 = 0;
		...
		{
			two_pole_filter( noise+impulse, pVtm_t->r4pd1, pVtm_t->r4pd2, r4pa, pVtm_t->R4pb, pVtm_t->r4pc );
		}
		else
		{
			pVtm_t->r4pd1=0;
		...
		two_pole_filter( noise+impulse, pVtm_t->r3pd1, pVtm_t->r3pd2, r3pa, r3pb, r3pc );
		...
		two_pole_filter( noise+impulse, pVtm_t->r2pd1, pVtm_t->r2pd2, r2pa, r2pb, r2pc );
...
void read_speaker_definition(LPTTS_HANDLE_T phTTS) 
{
	...
	S16 f6p;       /*  Frequency of fixed parallel 6th high broad     */
 ...
 if ( pVtm_t->uiSampleRateChange == SAMPLE_RATE_INCREASE )
	...
	d2pole_pf( pVtm_t,&pVtm_t->R6pb, &pVtm_t->r6pc, f6p, 1200, 0 );
	...
	pVtm_t->AFgain = ((SPD_CHIP *)spdeftochip)->afgain+8;     /*  19 */
