...

{
  ...
  if(frame->an < 3.0f && state->f1c < 300.0f)
...

  {
    llframe->NAH = (short)(
      ...
      + speaker->Ka 
		...
		if(llframe->NAH > speaker->Ka )
		{
			if( state->agf > 12 )
			{
				llframe->NAH = llframe->NAH + 9;
			}
			else if( state->agf > 9 )
			{
				llframe->NAH = llframe->NAH +(short)((state->agf - 9)*3.0f);
			}
		}
...
//    llframe->NB2 = 0;/*(short)speaker->B2m;*/
