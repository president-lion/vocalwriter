 ...
 * 0026 EAB		10/19/1997		use pDph_t->Cite_It variable and add syllable count support.
 ...
 * 0048 EAB		12/12/2000 		Fixed pDph_t->Cite_It bug in slowtalk with "to"
...
//		printf ("input phonenme to alloph is %d", curr_inph);
		...
		if(	pDph_t->phonemes[n] == pDph_t->phonemes[n + 1] 
			&& (phone_feature( pDph_t ,pDph_t->phonemes[n]) & FOBST))
		{
			dodelete = TRUE;
		}
