...
const short durdic[] = 
{
	...
	//12,WBOUND,USP_DH,USP_AX,WBOUND,EOS,0,40,60,0,0,-1,
...
if(pKsd_t->lang_curr == LANG_british)
{
		...
		if ((pDph_t->symbols[n] ) == LINKRWORD)
			//last argument means were not at the end 
		{
...
void add_feature (PDPH_T pDph_t, long feaname, short location)
{
	...
	/* Do it */
	//unless mode reading off and then for now don't interpret vpstarts
...
{
	if(phone>>8 == 8)
		return(all_featb[(phone)>>8][(phone)&0x00FF]);
	else
