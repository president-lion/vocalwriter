 ...
 */
	...
	const short dcval[11] = { 0, 30,40,80,90, 95,95, 100, 100,105,105};

	const  short ueval[11] = { 0,40,60,80,90,100,120,120,125,125,130};
	...
	if (pKsd_t->lang_curr == LANG_spanish || pKsd_t->lang_curr == LANG_latin_american)
		...
		{			
			...
			&& phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1]) & FVOICD)
					...
					pDph_t->target_ap = 0;
					//eab new_rule 8/13/01 
					// 


	//if((phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FPLOSV) &&
	//	!(pDph_t->allofeats[pDph_t->nphone+1] & FSTRESS)
//   && ((phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1])) & FSYLL)
//   && !(pDph_t->allofeats[pDph_t->nphone +1 ] & FDUMMY_VOWEL)) 
   // This reudces the glottal area to reduce the burst of a plosive in front
   // of an unstressed syllable
 if(pKsd_t->lang_curr != LANG_german)
	if(!(pDph_t->allofeats[pDph_t->nphone+1] & FSTRESS))
	{
		if(pDph_t->target_ag > 1400)
		pDph_t->target_ag = 1400;
		pDph_t->agspeed =1;
#ifdef PH_DEBUG
			if(DT_DBG(PH_DBG,0x080))
			{
				printf("reduce glotal area for unstressed %d \n",pDph_t->target_ag);
				printf("and increase speed\n");
			}
#endif	
#ifdef PH_DEBUG
					if(DT_DBG(PH_DBG,0x080))
					{
						printf("Unvoiced obstruent so close chink and glottis %d \n",pVtm_t->NOM_Open_Glottis);
						//DELAY OPENING SLIGHLTY ??
					}
#endif	
	...
	if(!(pDph_t->allofeats[pDph_t->nphone + 1] & FDUMMY_VOWEL)
		...
		if(DT_DBG(PH_DBG,0x080))
		{
			printf("SPread glottis for ?? sound \n");
		}
#endif
...
//chnage to a 20 ms closure
		...
		if(pDph_t->allophons[pDph_t->nphone] == USP_DH
			...
			{
				if (pDph_t->phonestep <  (pDph_t->allodurs[pDph_t->nphone]-6))
				{
#ifdef PH_DEBUG
					if(DT_DBG(PH_DBG,0x080))
					{
...
#ifdef TEST
	//test for female voice (and male)
	if(pDph_t->allophons[pDph_t->nphone] == USP_AX 
		|| pDph_t->allophons[pDph_t->nphone] == USP_IH
		|| pDph_t->allophons[pDph_t->nphone] == USP_UW
		|| pDph_t->allophons[pDph_t->nphone] == USP_AW
		|| pDph_t->allophons[pDph_t->nphone] == USP_OW
		&& pDph_t->phonestep == 1 
		)
	{
#ifdef PH_DEBUG
		if(DT_DBG(PH_DBG,0x080))
		{
			printf(" Spreading glotis at certatin vowels delta value %d \n",pDph_t->delta_a_forap;
	...
	{
		if( pDphsettar->nframb >  (pDph_t->tcumdur-25) 
			&& pDph_t->nphone < pDph_t->nallotot-1)
		{
			if(pDph_t->delta_area_g < 800)
			{
				...
				if(DT_DBG(PH_DBG,0x080))
				{
					printf(" Spreading glotis at final sonorant delta value %d and pressure_droping also %d \n",pDph_t->delta_area_g,pDph_t->pressure_drop);
					
				}
#endif
		//		pDph_t->pressure_drop += 30;
				if(pDph_t->number_words < 3)
					pDph_t->delta_area_g += 5;
				else
...
//#define OLD_PRESSURE
			...
			&& pDph_t->syl_pressure == 0)
		{
			...
			//beginning of the first phone hence low pressure.
					...
					&& pDph_t->syl_pressure == 0)
				{
					...
					last_real_phon = pDph_t->nphonetot-2;
					}
				else
				{
					...
					if( pDph_t->syl_pressure == 0
						...
						if(pKsd_t->lang_curr == LANG_german)
						{
							if(phone_feature(pDph_t, pDph_t->allophons[last_real_phon ]) & FOBST)
						...
						else
						{
							if(phone_feature(pDph_t, pDph_t->allophons[last_real_phon ]) & FOBST)
								...
								{
									//research by slifka 
								if(phone_feature(pDph_t, pDph_t->allophons[last_real_phon ]) & FVOICD)
								{
									if((pDph_t->tcumdur-pDphsettar->nframb>0))
										pDph_t->syl_pressure = (((pDph_t->pressure-pDph_t->syl_pressure) -500)/(value-pDphsettar->nframb));
								}
								else
								{
									...
									if((pDph_t->tcumdur-pDphsettar->nframb>0))
										pDph_t->syl_pressure = (((pDph_t->pressure-pDph_t->syl_pressure) -450)/(value-pDphsettar->nframb));
								}
								}
								else
								{
					...
					{
						if(pDph_t->syl_pressure > 0)
						printf("Pressure drop from last syl %d \n",pDph_t->syl_pressure);
					}
#endif
	...
	if(pDph_t->allophons[pDph_t->nphone+1] == USP_Q )
	{
		if(pDph_t->tcum >= pDph_t->allodurs[pDph_t->nphone]>>1)
		{
			pDph_t->pressure_drop += 30;                   
	...
	else 	if(pDph_t->allophons[pDph_t->nphone] == USP_Q )
	{
			if(pDph_t->pressure_drop > 0)
				pDph_t->pressure_drop -= 40;
			else
				pDph_t->pressure_drop =0;		
	...
	if(phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone)]) & FVOICD)
				{
					...
					if(pDph_t->phonestep==6)
					{
						pDph_t->dcstep=-1;
						pDph_t->uestep=-1;
					}else if(pDph_t->phonestep==0)
					{
...
/* DEBUG Code  verifies variables that should never hit 
	...
	if ((pDph_t->allophons[ pDph_t->nphone+1] == GEN_SIL)
		...
		if (pDph_t->PHSwapOut != NULL) 
		{
			...
			fprintf(pDph_t->PHSwapOut,"%4d ",pDph_t->parstochip[OUT_AL]);
			fprintf(pDph_t->PHSwapOut,"%4d ",pDph_t->parstochip[OUT_AB]);
			fprintf(pDph_t->PHSwapOut,"%4d ",pDph_t->parstochip[OUT_AN]*10);
			fprintf(pDph_t->PHSwapOut,"%4d ",pDph_t->parstochip[OUT_CNK*10]);
			fprintf(pDph_t->PHSwapOut,"%4d ",pDph_t->parstochip[OUT_PS]);
			fprintf(pDph_t->PHSwapOut,"%4d ",pDph_t->parstochip[OUT_DC]*10);
			fprintf(pDph_t->PHSwapOut,"%4d ",pDph_t->parstochip[OUT_UE]*10);
			fprintf(pDph_t->PHSwapOut,"%4d ",(pDph_t->parstochip[OUT_PH]& PVALUE));
			fprintf(pDph_t->PHSwapOut,"\r\n");
#endif
		}
...
#ifdef NOWOUT
					fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_PH]);
					fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_T0]);
					fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_F1]);
					fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_F2]);
					fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_F3]);
					fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_AG]);
					fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_AB]);
					fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_AL]);
			fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_ABLADE]);
					fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_PS]);
			fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_CNK]);
			fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_DC]);
				fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_UE]);
			fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_UE]);
			fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_OQ]);
			fscanf(pDph_t->PHSwapIn,"%4d ",&pDph_t->parstochip[OUT_CNK]);
			fscanf(pDph_t->PHSwapIn,"\r\n");
