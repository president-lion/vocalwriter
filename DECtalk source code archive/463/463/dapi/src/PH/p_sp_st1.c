 ...
 *
 * *****************************************************************/
void sp_back_smooth_rules (LPTTS_HANDLE_T phTTS, short shrib, short feacur,
 ...
 *
 * *****************************************************************/
void sp_special_rules (LPTTS_HANDLE_T phTTS, short fealas, short feacur, short feanex,
							  short struclm2, short struccur, short pholas, short struclas)
{
	short                   bdur, vot; /* MVP made as a local variable */
...
*/
	...
	if ((((fealas & FPLOSV) IS_PLUS)
		 ...
		 && ((feacur & FSONOR) IS_PLUS ))) 
	{
		PAP.pspesh = 48;	/* EAB 10/23/98 FOr Latin Spanish very low ideally 0 for ptk
...
//		vot = NF30MS;				   /* Asp dur for /p,t,k/ before stressed sonor */
		if(sp_place[pholas & PVALUE] & FLABIAL)
			vot = NF25MS;
