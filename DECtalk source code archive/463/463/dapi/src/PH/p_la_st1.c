	...
	else if (pDphsettar->par_type IS_AV_OR_AH)
	{
		...
		else
		{
			...
			if(phone_temp == LAP_LL)
				tartemp=10;
			if(phone_temp == LAP_J)
				tartemp=45;/* make stronger for castillion*/
		}
	}
 ...
 *
 * *****************************************************************/
void la_back_smooth_rules (LPTTS_HANDLE_T phTTS, short shrib, short feacur,
 ...
 *
 * *****************************************************************/
void la_special_rules (LPTTS_HANDLE_T phTTS, short fealas, short feacur, short feanex,
							  short struclm2, short struccur, short pholas, short struclas)
{
	short                   bdur, vot; /* MVP made as a local variable */
...
*/
	...
	if ((((fealas & FPLOSV) IS_PLUS)
		 ...
		 && ((feacur & FSONOR) IS_PLUS ))) 
	{
		PAP.pspesh = 43;	/* EAB 10/23/98 FOr Latin Spanish very low ideally 0 for ptk
		 //but we'll add that level later */
		if (begtyp(pDphsettar->phonex) != 1)
		{
			PAP.pspesh =45;		   /* Stronger asp before +back */
		}
		PAV.pspesh = 0;				   /* Amplitude of voicing during aspir */
		...
		if(la_place[pholas & PVALUE] & FLABIAL)
			...
			vot = NF30MS;				   /* Asp dur for t */
		if ((struccur & FSTRESS_1) IS_MINUS)
		{
			vot = NF15MS;			   /* Vot shorter if vowel not stressed */
			PAP.pspesh -= 3;
		}
