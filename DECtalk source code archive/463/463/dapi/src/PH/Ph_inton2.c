...
/* 'stress `stress "stress                      */
  /* 0,       51-10,     31-10,    161-50 */  /*eab  */
	...
	short cumdur = -6, phocur = 0;		/* MVP : made local */
	short stepcount;
			...
			if( pDph_t->number_words > 2  )
			{
				...
				if((pDph_t->nallotot -nphon) <6)
				{
					if (nphon == n_last_syl_nuc)
					{
						...
						make_f0_command( phTTS,GLIDE, 23, +60, 0, pDph_t->allodurs[nphon], &cumdur, nphon);
						pDph_t->done =1;
					}
				}
			}
			...
			if (pDph_t->had_hatbegin)
			{
				...
				if (pDph_t->f0mode == NORMAL && !pDph_t->special_phrase)
				{
					...
					*/
						...
						{
							delayf0 = (pDph_t->allodurs[gesture_ref] >> 1);
						}
...
{					
						make_f0_command( phTTS,IMPULSE, 1, (short)(pDphsettar->hatsize), delayf0, 30, &cumdur, nphon);
...
{					
						...
						|| ((struccur & FPERNEXT) IS_PLUS))
					{
						...
						targf0 = targf0 - 50; /*reduce last stres per anna */
				...
				else if (pKsd_t->lang_curr == LANG_german)
				{
						...
						make_f0_command( phTTS,IMPULSE, 22, targf0, delayf0, 20, &cumdur, nphon);
...
pDphsettar->nrises_sofar=1;
				...
				
			{
				...
				length = 25;
				//far from end delay drop and slow it
				if(nphon  < pDph_t->nallotot-3)
				{
					length += 10;
					delayf0 += pDph_t->allodurs[nphon];
					//-6 is for f0 delay through filter
				}
					...
					if ((struccur & FBOUNDARY) == FQUENEXT)
					{
						...
						if(pKsd_t->lang_curr != LANG_german )
						{
							if(pDph_t->number_words == 1)
							{
								...
								make_f0_command ( phTTS, IMPULSE, 341,F0_QGesture2-100, pDph_t->allodurs[nphon],24, &cumdur, nphon);
							}
							else
							{
								make_f0_command ( phTTS,IMPULSE, 341,F0_QGesture1, -6,15, &cumdur, nphon);
								make_f0_command ( phTTS, IMPULSE, 341,F0_QGesture2, (pDph_t->allodurs[nphon]>>2)-4,20, &cumdur, nphon);
							}
						...
						else
						{
							...
							{
								make_f0_command ( phTTS, IMPULSE,420,  F0_CGesture1, delayf0,24, &cumdur, nphon);
								make_f0_command ( phTTS, IMPULSE, 420, F0_CGesture2>>1, (short)(pDph_t->allodurs[nphon]),24, &cumdur, nphon); // NAL warning removal
							}
