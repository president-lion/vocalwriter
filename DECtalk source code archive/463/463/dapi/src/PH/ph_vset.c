...
//		pDph_t->p_locus = femloc;	   			/* Use FEMALE tables */
	...
	pDph_t->f0_dep_tilt = pDph_t->curspdef[SPD_FT]+0;			/* FT -> FT */ /* FL in spanish */
	pDph_t->assertiveness = pDph_t->curspdef[SPD_AS] * 41;	/* AS in % -> AS */
/* BATS 711 MOve German to the new method for filter calculation*/
	pDph_t->f0_lp_filter = (1500 + 15 * (pDph_t->curspdef[SPD_QU]+40));	/* QU in % -> lp cuttoff */
	pDph_t->size_hat_rise = (pDph_t->curspdef[SPD_HR]-10) * 10;	/* HR in Hz -> Hz*10    */
	pDph_t->scale_str_rise = pDph_t->curspdef[SPD_SR];		/* SR in Hz -> mult. sc. fac */
	pDph_t->f0minimum = (pDph_t->curspdef[SPD_AP]-65 )* 10;		/* AP -> f0min          */
	...
	pVtm_t->Tiltm = pDph_t->curspdef[SPD_SM]-40;
	
	 //printf("WA?br  %d \n",pDph_t->curspdef[SPD_BR]); 
	pDph_t->spdefb1off = 4096; 
	...
	spdef->fnscale = (200 - 100) * 41;	/* HS -> FNscale        */
	...
	spdef->r4pb = 3400;	/* F7 -> F4p            */
	spdef->r5pb = 4800;	/* F8 -> F5p            */
	spdef->t0jit = 0;
	//3/15/01 Changed to left shift rather than mult.  */
	spdef->r5ca = 65;	/* G1 -> G1 in dB       */
	spdef->r4ca = 65;	/* G2 -> G2 in dB       */
	spdef->r3ca = 66;	/* G3 -> G3 in dB       */
	spdef->r2ca = 60;	/* G4 -> G4 in dB       */


	spdef->r1ca = 70;	/* LO -> LO in dB       */
#ifdef LOWCOMPUTE
	spdef->nopen1 = 5000 + (160 * (100 - 30));	/* RI -> K1  */
	spdef->nopen2 = 20;/* NF -> K2     */
#else
	spdef->nopen1 = 5000 + (160 * (100 - 30));	/* RI -> K1  */
	spdef->nopen2 = 20 * 4;/* NF -> K2     */
#endif
	spdef->aturb = 0;

	spdef->rnpgain = 71;//pDph_t->curspdef[SPD_GN];	/* GN -> GN             */


	spdef->azgain = pDph_t->curspdef[SPD_GV]-5;	/* GV -> GV             */
	spdef->rnpgain = pDph_t->curspdef[SPD_GN];	/* GN -> GN             */


	spdef->afgain = pDph_t->curspdef[SPD_GF]-3;/* GH -> GH         */
	spdef->apgain = pDph_t->curspdef[SPD_GH]-3;	/* GH -> GH         */
//end hack code for new frank
}
...
/* BATS 711 MOve German to the new method for filter calculation*/
	...
	{
		if(pDph_t->spdeftltoff <= 18)
			pDph_t->spdeftltoff += 8;
	}
	 //printf("WA?br  %d \n",pDph_t->curspdef[SPD_BR]); 
...
if((pKsd_t->modeflag & MODE_READING)
		...
		&& !pVtm_t->bDoTuning)
{
	spdef->azgain = pDph_t->curspdef[SPD_GV]-10; //avgain
	spdef->aturb = pDph_t->curspdef[SPD_BR]+10;
...
else
{
	if (pVtm_t->bDoTuning)
	{
		spdef->aturb = pDph_t->curspdef[SPD_BR]+15;
	}
}
}
