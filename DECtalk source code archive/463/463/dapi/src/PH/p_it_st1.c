 ...
 *
 * ******************************************************************/
short it_gettar (LPTTS_HANDLE_T phTTS, int nphone_temp)
{
	PDPH_T                  pDph_t = phTTS->pPHThreadData;
	PKSD_T                  pKsd_t = phTTS->pKernelShareData;
	PDPHSETTAR_ST           pDphsettar = pDph_t->pSTphsettar;

	short                   phone_temp, phlas_temp, phnex_temp, tartemp=0, pphotr, npar, begtypnex;
	short                   phone_feat;
...
//#if !defined HLSYN && !defined NON_HLSYN
 ...
 *
 * *****************************************************************/
void it_forw_smooth_rules(LPTTS_HANDLE_T phTTS, short shrif, short pholas,
 ...
 *
 * *****************************************************************/
void it_back_smooth_rules (LPTTS_HANDLE_T phTTS, short shrib, short feacur,
...
#else
#ifdef ITP_V
			&& pDphsettar->phcur != ITP_V
#endif
			&& pDphsettar->phcur != ITP_Z)
		{
			/* 
			 * All but Y, V, Z
			 */
...
void special_rules (LPTTS_HANDLE_T phTTS, short fealas, short feacur, short feanex, 
...
//	PKSD_T                  pKsd_t = phTTS->pKernelShareData;
	...
	if ((feacur & FBURST) IS_PLUS)
	{
		...
		else
		{
			...
			if (((feanex & (FNASAL | FPLOSV)) != 0)
				&& it_place[pDphsettar->phcur & PVALUE] >= it_place[pDphsettar->phonex])
				bdur = 0;
		}
	...
	{
		switch (it_place[pDphsettar->phcur & PVALUE])
		{
			case FDENTAL:
			case FLABIAL:
				PAP.pspesh = 53 ;	/* eab */
	...
	if ((feacur & FVBAR) IS_PLUS)
	{
		...
		if (pDphsettar->phcur != ITP_G
			&& (pDphsettar->phonex == ITP_I
				|| pDphsettar->phonex == ITP_E))
			PAV.pspesh = 20;  */
	}
}
 ...
 *
 * *****************************************************************/
void it_special_rules (LPTTS_HANDLE_T phTTS, short fealas, short feacur, short feanex,
							  short struclm2, short struccur, short pholas, short struclas)
{
	short                   bdur, vot; /* MVP made as a local variable */
...
*/
		 ...
		 && ((feacur & FSONOR) IS_PLUS ))) 
	{
		...
		{
			PAP.pspesh = 58;		   /* Stronger asp before +back */
		}
		

		vot = NF30MS;				   /* Asp dur for /p,t,k/ before stressed sonor */
		if(it_place[pholas & PVALUE] & FLABIAL)
			vot = NF20MS;
		else if(it_place[pholas & PVALUE] & FVELAR)
			vot = NF60MS;
		else
