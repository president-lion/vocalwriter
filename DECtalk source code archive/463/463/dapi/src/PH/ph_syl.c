...
{
#if defined SPANISH || defined MULTIPLE_LANGUAGES_LOADED
	int	vc, hit = 0;
#endif
#if defined ENGLISH_US || defined ENGLISH_UK || defined MULTIPLE_LANGUAGES_LOADED	// NAL warning removal
	int l;
	int k=0;
#endif
#if !defined FRENCH && !defined GERMAN || defined MULTIPLE_LANGUAGES_LOADED
	int j;
#endif
	int i,off;
...
#if defined MULTIPLE_LANGUAGES_LOADED
	switch(pKsd_t->lang_curr)
	{
	case LANG_english:
		ascky_check=(char *)us_ascky_check;
		syl_vowels=(char *)us_syl_vowels;
		syl_cons=(char **)us_syl_cons;
		common_affixes=(char **)us_common_affixes;
		break;
	case LANG_french:
		ascky_check=(char *)fr_ascky_check;
		syl_vowels=(char *)fr_syl_vowels;
		syl_cons=(char **)fr_syl_cons;
		common_affixes=(char **)fr_common_affixes;
		break;
	case LANG_german:
		ascky_check=NULL;
		syl_vowels=NULL;
	...
	case LANG_spanish:
		...
		syl_cons=sp_syl_cons;
		common_affixes=(char **)sp_common_affixes;
		break;
	case LANG_british:
		ascky_check=(char *)uk_ascky_check;
		syl_vowels=(char *)uk_syl_vowels;
		syl_cons=(char **)uk_syl_cons;
		common_affixes=(char **)uk_common_affixes;
		break;
	case LANG_latin_american:
		ascky_check=la_ascky_check;
		syl_vowels=la_syl_vowels;
		syl_cons=la_syl_cons;
		common_affixes=(char **)la_common_affixes;
		break;
	...
	for (i = 0; ph[i] != WBOUND; i--)
	{
		...
		{
#if defined MULTIPLE_LANGUAGES_LOADED || defined ENGLISH_US
...
#endif
#if defined MULTIPLE_LANGUAGES_LOADED || defined ENGLISH_UK
...
#endif
#if defined MULTIPLE_LANGUAGES_LOADED || defined SPANISH
...
/* printf(" %s ons %d \n",syl_cons[j],len); */
		...
		while (true)
		{
			if (ascky_check[(*tp) & PVALUE] == 0)
			{
				...
				if(pKsd_t->lang_curr == LANG_english || pKsd_t->lang_curr == LANG_british || pKsd_t->lang_curr == LANG_french)
					tp++;
				else if(pKsd_t->lang_curr == LANG_spanish || pKsd_t->lang_curr == LANG_latin_american)
					tp--;
#else
#if defined ENGLISH_US || defined ENGLISH_UK || defined FRENCH
				tp++;
#endif      
#ifdef SPANISH
				tp--;
