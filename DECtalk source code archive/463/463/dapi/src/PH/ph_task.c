 ...
 * 079	MGS		11/23/2004		Added full phoneme protection code for all langauges
 */
...
//#if defined (WIN32) || defined (__osf__) || defined (__linux__) || defined VXWORKS || defined _SPARC_SOLARIS_
		 ...
		 */
		if ((buf[0] & PVALUE) == SPECIALWORD && (pKsd_t->lang_curr == LANG_english || pKsd_t->lang_curr == LANG_british))
		{
			pDph_t->docitation = 1;
		}
		/* debug eab */
	...
	/* sets wordclass but I want it marked throughout the word because I don't know where it started*/
		...
		if ((buf[0] & PFONT) == (PFCONTROL << PSFONT))
		{
			if (buf[0] == WORD_CLASS) {
				...
				{
					pDph_t->number_verbs++;
					//German doesn't mark helper verbs as functor- we may nee to redo the dicitonary POS
					if( pKsd_t->lang_curr == LANG_german )
...
/* printf("This is the float number %f\n\r",9.0/5.0); */
		...
		/* no syllablification in german */
		/* debug switch */
		if ((pKsd_t->logflag & LOG_SYLLABLES) || DT_DBG(PH_DBG,0x004) && pKsd_t->lang_curr != LANG_german)
			logsyllable (phTTS);
		if (pKsd_t->sayflag == SAY_SYLLABLE && pKsd_t->lang_curr != LANG_german)
		{
...
//		if (((buf[0] & 0xff)>=121)  && ((buf[0] & 0xff) <=129))
		...
		if (((buf[0] & 0xff)>=PHO_SYM_TOT)  && ((buf[0] & 0xff) <=129))
			...
			continue;
#endif
		if ((buf[0]&PFONT) && (temp=(buf[0]&0xff))<100)
		{
			switch ((buf[0]&PFONT)>>8)
			{
			case 0:
			case PFCONTROL:
				break;
			case PFUSA:
				if (temp>=US_TOT_ALLOPHONES)
					return(-1);
				break;
			case PFUK:
				if (temp>=UK_TOT_ALLOPHONES)
					return(-1);
				break;
			case PFGR:
				if (temp>=GR_TOT_ALLOPHONES)
					return(-1);
			case FRGC:
				break;
			case PFFR:
				if (temp>=FR_TOT_ALLOPHONES)
					return(-1);
				break;
			case PFSP:
				if (temp>=SP_TOT_ALLOPHONES)
					return(-1);
				break;
			case PFLA:
				if (temp>=LA_TOT_ALLOPHONES)
					return(-1);
				break;
			case PFIT:
				if (temp>=IT_TOT_ALLOPHONES)
					return(-1);
				break;
			default:
				
#ifdef PH_DEBUG
				printf("error, invalid langauage specified\n");
#endif
				;
...
short *GetPhVdefParams(LPTTS_HANDLE_T phTTS, UINT index, int langauge)
{
	...
	case LANG_french:
		switch (index)
		{
		case 0:	 memcpy(params, fr_paul, sizeof(fr_paul));     break;
		case 1:	 memcpy(params, fr_betty, sizeof(fr_betty));   break;
		case 2:	 memcpy(params, fr_harry, sizeof(fr_harry));   break;
		case 3:	 memcpy(params, fr_frank, sizeof(fr_frank));   break;
		case 4:	 memcpy(params, fr_kit, sizeof(fr_kit));       break;
		case 5:	 memcpy(params, fr_ursula, sizeof(fr_ursula)); break;
		case 6:	 memcpy(params, fr_rita, sizeof(fr_rita));	 break;
		case 7:	 memcpy(params, fr_wendy, sizeof(fr_wendy));	 break;
		case 8:	 memcpy(params, fr_dennis, sizeof(fr_dennis)); break;
		case 9:  memcpy(params, fr_paul, sizeof(fr_paul));		 break;
		case 10: memcpy(params, fr_betty, sizeof(fr_betty));		 break;
		case 11: memcpy(params, fr_harry, sizeof(fr_harry));		 break;
		case 12: memcpy(params, fr_frank, sizeof(fr_frank));       break;
		case 13: memcpy(params, fr_kit, sizeof(fr_kit));			 break;
		case 14: memcpy(params, fr_ursula, sizeof(fr_ursula));	 break;
		case 15: memcpy(params, fr_rita, sizeof(fr_rita));	     break;
		case 16: memcpy(params, fr_wendy, sizeof(fr_wendy));	     break;
		case 17: memcpy(params, fr_dennis, sizeof(fr_dennis));	 break;
		case 18: memcpy(params, fr_paul, sizeof(fr_paul));		 break;
		}
		break;
	default:
		switch (index) 
		{	// CAB Removed warnings by typecast
		case 0:	 memcpy(params, us_paul, sizeof(us_paul));     break;
		case 1:	 memcpy(params, us_betty, sizeof(us_betty));   break;
		case 2:	 memcpy(params, us_harry, sizeof(us_harry));   break;
		case 3:	 memcpy(params, us_frank, sizeof(us_frank));   break;
		case 4:	 memcpy(params, us_kit, sizeof(us_kit));       break;
		case 5:	 memcpy(params, us_ursula, sizeof(us_ursula)); break;
		case 6:	 memcpy(params, us_rita, sizeof(us_rita));	 break;
		case 7:	 memcpy(params, us_wendy, sizeof(us_wendy));	 break;
		case 8:	 memcpy(params, us_dennis, sizeof(us_dennis)); break;
		case 9:  memcpy(params, us_paul, sizeof(us_paul));		 break;
		case 10: memcpy(params, us_betty, sizeof(us_betty));		 break;
		case 11: memcpy(params, us_harry, sizeof(us_harry));		 break;
		case 12: memcpy(params, us_frank, sizeof(us_frank));       break;
		case 13: memcpy(params, us_kit, sizeof(us_kit));			 break;
		case 14: memcpy(params, us_ursula, sizeof(us_ursula));	 break;
		case 15: memcpy(params, us_rita, sizeof(us_rita));	     break;
		case 16: memcpy(params, us_wendy, sizeof(us_wendy));	     break;
		case 17: memcpy(params, us_dennis, sizeof(us_dennis));	 break;
		case 18: memcpy(params, us_paul, sizeof(us_paul));		 break;
		}
	}
	return params;
}
						  ...
						  SPDEFS **ppspLoLimit, SPDEFS **ppspHiLimit, SPDEFS **ppspDefault)
{
	...
	*ppspDefault = (SPDEFS *) malloc(sizeof(SPDEFS));
	...
	if (pKsd_t->uiSampleRate < 8763) /* 8-bit */
		switch (speaker_num)
		{	// CAB Removed warnings by typecast
			case 0:	cur_speaker = (short *)fr_paul;   break;
			case 1:	cur_speaker = (short *)fr_betty;  break;
			case 2:	cur_speaker = (short *)fr_harry;  break;
			case 3:	cur_speaker = (short *)fr_frank;  break;
			case 4:	cur_speaker = (short *)fr_dennis; break;
			case 5:	cur_speaker = (short *)fr_kit;    break;
			case 6:	cur_speaker = (short *)fr_ursula; break;
			case 7:	cur_speaker = (short *)fr_rita;   break;
			case 8:	cur_speaker = (short *)fr_wendy;  break;
	...
	if (pKsd_t->uiSampleRate < 8763) /* 8-bit */
		switch (speaker_num)
		{	// CAB Removed warnings by typecast
			case 0:	cur_speaker = (short *)us_paul;   break;
			case 1:	cur_speaker = (short *)us_betty;  break;
			case 2:	cur_speaker = (short *)us_harry;  break;
			case 3:	cur_speaker = (short *)us_frank;  break;
			case 4:	cur_speaker = (short *)us_dennis; break;
			case 5:	cur_speaker = (short *)us_kit;    break;
			case 6:	cur_speaker = (short *)us_ursula; break;
			case 7:	cur_speaker = (short *)us_rita;   break;
			case 8:	cur_speaker = (short *)us_wendy;  break;
