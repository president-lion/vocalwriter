...

{
	...
	{
		ploc = plocu( (fonobst + (IT_TOT_ALLOPHONES * (sontyx - 1) ) ) );
		//printf ("setloc  fonobst : %x, sontyx %d, ploc %d\n", 
    	  //       fonobst, sontyx, ploc);
		if(pDph_t->malfem == MALE)
		{
			pDph_t->p_locus = (short *)it_maleloc;
		}
		else
		{
			pDph_t->p_locus = (short *)it_femloc;	   	
		}
	}
...
#ifdef nnot_yet
	if ((phone_feature( pDph_t,fonobst) & FPLOSV)
		&& ((place(fonobst)) & BLADEAFFECTED)
		&& (nfonobst > nfonsonor ))   
	{
		pDphsettar->durtran = mstofr(pDph_t->p_locus[ploc + 2])- 5;
	//	printf("yikes\n");
		if (pDphsettar->durtran < 0)
			pDphsettar->durtran  =  0;
	if((pDph_t->sentstruc[pDph_t->nphone ] & FBOUNDARY) == FWBNEXT)
		{
			if(fonsonor == USP_IY) //or other language equiv....
			{
				prcnt = 100;
			}
			else
