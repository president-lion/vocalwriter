...
/* This file is only for US ENGLISH */
#ifndef ENGLISH_US
#define ENGLISH_US
#endif
#ifndef ENGLISH
...
void phinton_classic (LPTTS_HANDLE_T phTTS)
{
...
//#endif
				...
				if (pDph_t->malfem == MALE)
					...
					if( pDph_t->number_words == 1 && stresscur != FEMPHASIS)
		  ...
		  make_f0_command ( phTTS,IMPULSE, 2, targf0, delayf0,20, &cumdur, nphon);
...
#endif
				{
					...
					return to baseline offset by f0delta In further investigation it was really
						...
						delayf0 = (pDph_t->allodurs[nphon]) - NF180MS;
						/* But not too early */
						if (delayf0 < NF25MS)
							delayf0 = NF25MS;
#ifndef FRENCH
			...
			if (((struccur & FBOUNDARY) == FCBNEXT)
...
#endif // #ifndef FRENCH
					{
						...
						if ((struccur & FBOUNDARY) == FQUENEXT)
						{
							...
							if(pDph_t->number_words == 1)
							{
								//make_f0_command ( phTTS,IMPULSE, 41,F0_QGesture1+30, delayf0-2,24, &cumdur, nphon);
								make_f0_command ( phTTS, IMPULSE, 41,F0_QGesture2+50, delayf0,24, &cumdur, nphon);
							}
							else
							{
								//make_f0_command ( phTTS,IMPULSE, 41,F0_QGesture1, -2,14, &cumdur, nphon);
								make_f0_command ( phTTS, IMPULSE, 41,F0_QGesture2+50, delayf0,24, &cumdur, nphon);
							}
						...
						make_f0_command (phTTS, IMPULSE,4, F0_CGesture1, delayf0,0, &cumdur,nphon);
						make_f0_command (phTTS, IMPULSE, 4, F0_CGesture2, pDph_t->allodurs[nphon],0, &cumdur,nphon);
...
//#ifndef (GERMAN || SPANISH) 
	...
	if (( phonex == GEN_SIL)&& (phone_feature( pDph_t,phocur) & FPLOSV) )
			if( (phone_feature( pDph_t,phocur) & FBURST) )
	{
													/* p t k b d g */
