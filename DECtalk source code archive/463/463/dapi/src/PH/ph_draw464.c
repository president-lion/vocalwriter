...
//			printf(" at phone %s \n", phprint( pDph_t->allophons[pDph_t->nphone] ) );
	...
	if(!(pDph_t->allofeats[pDph_t->nphone+1] & FSTRESS))
	{
			...
			if(DT_DBG(PH_DBG,0x080))
			{
							printf("Unvoiced obstruent next unstressed so  glottis to %d and ap to %d\n",
								pDph_t->target_ag,pDph_t->target_ap);
			}
#endif	
					}
			...
			&& !(pDph_t->allofeats[pDph_t->nphone ] & FDUMMY_VOWEL))
		{
					...
					else
			{
				...
				if(DT_DBG(PH_DBG,0x080))
			{
					printf("in a FSON1 ag tar to pVtm_t->NOM_V  %d \n", pDph_t->target_ag  );	
	...
	if (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1]) & FOBST)
			...
			&& phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FCONSON)
		{
			...
			pDph_t->target_tb= 1000;
				...
				pDph_t->target_ag = 2700;
				pDph_t->agspeed = 2;
#ifdef PH_DEBUG
				if(DT_DBG(PH_DBG,0x080))
				{
					printf("HX so  glottis  target  at %d \n", pDph_t->target_ag);
				}
#endif
		...
		&&  pDph_t->allophons[pDph_t->nphone+1] != pDph_t->allophons[pDph_t->nphone+2])
{
	...
	if(pDph_t->allodurs[pDph_t->nphone] < 7)
	{
		...
		if (!(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FOBST))
		{
		if(pDph_t->parstochip[OUT_F1] > 400 || pDph_t->parstochip[OUT_F1] < 650)
		{
			pDph_t->parstochip[OUT_B1] += 80;
			//increasre bandwidth as things are about to happen
			}
...
//chnage to a 20 ms closure
		...
		if(pDph_t->allophons[pDph_t->nphone] == USP_DH
			...
			{
				if (pDph_t->phonestep <  (pDph_t->allodurs[pDph_t->nphone]-10))
				{
#ifdef PH_DEBUG
					if(DT_DBG(PH_DBG,0x080))
					{
			...
			else
			{
				...
				if (pDph_t->phonestep <  2)
				{
					...
					if(DT_DBG(PH_DBG,0x080))
					{
						printf(" Do a small closure for dh or th in word final \n");
					}
#endif
					//test
					pDph_t->target_b= 0;
				...
				if(	(pDph_t->allofeats[pDph_t->nphone] & FTYPESYL) == FMONOSYL)
		{
					...
					if(pDph_t->tcum <= (pDph_t->allodurs[pDph_t->nphone]>>2)+vot_time)
			{
				...
				else //msut be multi syl delay til after stressed syl if single word
			{
					...
					if((pDph_t->allofeats[pDph_t->nphone] & FSTRESS)
						&& pDph_t->number_words == 1)
						{
							...
							if(DT_DBG(PH_DBG,0x080))	
							{
							printf(" Delaying end of phrase on single word til after stressed syl\n");
				...
				{	
					...
					pDph_t->delta_area_g +=  pVtm_t->EndOfPhrase_Spread;
								...
								if(DT_DBG(PH_DBG,0x080))
								{
						printf(" Spreading glotis at final stressed syl delta value %d and pressure_droping also %d target %d\n",pDph_t->delta_area_g,pDph_t->pressure_drop,pDph_t->target_ag);
...
abort_til_later:	if(pDph_t->nphone == 0)
				...
				else
				{
					...
					if( pDph_t->syl_pressure == 0
						...
						if(pKsd_t->lang_curr == LANG_german)
						{
							...
							{
								if((pDph_t->tcumdur-pDphsettar->nframb>0))
										pDph_t->syl_pressure = (((pDph_t->pressure) -600)/(value-pDphsettar->nframb));
							}
						}
						else
						{
								...
								else
								{
										...
										if((pDph_t->tcumdur-pDphsettar->nframb>5))
											pDph_t->syl_pressure = (((pDph_t->pressure) -800)/(value-pDphsettar->nframb));
								}
								}
								else
								{
	...
	pDph_t->parstochip[OUT_ATB] = pDph_t->area_tb;
			//	printf(" %d %d \n",pDph_t->parstochip[OUT_AB],pDph_t->area_b);
					...
					if(pDph_t->nphone >=1)
					{
						...
						if(pDph_t->pressure_burst > 0)
							if(((phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone])) & FSYLL) IS_PLUS)
								if(pDph_t->pressure_burst > 0)
									pDph_t->pressure_burst -=90;
								else
									pDph_t->pressure_burst =0;
					}
					//printf("%d %d %d \n",pDph_t->pressure, pDph_t->pressure_burst,tmp);
#endif
