...
#ifdef HLSYN
#ifdef ENGLISH
char OFFSETFILE[] = "p_us_vdf_tunehl.h";
char *message = "\n\nNow tuning ENGLISH_US HL\n";
int language=LANG_english;
#endif // ENGLISH

#ifdef ENGLISH_UK
char OFFSETFILE[] = "p_uk_vdf_tunehl.h";
char *message = "ENGLISH_UK HL";
int language=LANG_british;
#endif // ENGLISH_UK

#ifdef SPANISH
char OFFSETFILE[] = "p_sp_vdf_tunehl.h";
char *message = "SPANISH_SP HL";
int language=LANG_spanish;
#endif // SPANISH

#ifdef ITALIAN
char OFFSETFILE[] = "p_it_vdf_tunehl.h";
char *message = "ITALIAN HL";
int language=LANG_italian;
#endif // SPANISH

#ifdef SPANISH_LA
char OFFSETFILE[] = "p_la_vdf_tunehl.h";
char *message = "SPANISH_LA HL";
int language=LANG_latin_american;
#endif // SPANISH_LA

#ifdef GERMAN
char OFFSETFILE[] = "p_gr_vdf_tunehl.h";
char *message = "GERMAN HL";
int language=LANG_german;
#endif // GERMAN

#ifdef FRENCH
char OFFSETFILE[] = "p_fr_vdf_tunehl.h";
char *message = "FRENCH HL";
int language=LANG_french;
#endif // FRENCH

#else  // HLSYN

#ifdef ENGLISH
char OFFSETFILE[] = "p_us_vdf_tuneint.h";
char *message = "\n\nNow tuning ENGLISH_US Int\n";
int language=LANG_english;
#endif // ENGLISH

#ifdef ENGLISH_UK
char OFFSETFILE[] = "p_uk_vdf_tuneint.h";
char *message = "ENGLISH_UK Int";
int language=LANG_british;
#endif // ENGLISH_UK

#ifdef SPANISH
char OFFSETFILE[] = "p_sp_vdf_tuneint.h";
char *message = "SPANISH_SP Int";
int language=LANG_spanish;
#endif // SPANISH

#ifdef ITALIAN
char OFFSETFILE[] = "p_it_vdf_tuneint.h";
char *message = "Italian Int";
int language=LANG_italian;
#endif // SPANISH

#ifdef SPANISH_LA
char OFFSETFILE[] = "p_la_vdf_tuneint.h";
char *message = "SPANISH_LA Int";
int language=LANG_latin_american;
#endif // SPANISH_LA

#ifdef GERMAN
char OFFSETFILE[] = "p_gr_vdf_tuneint.h";
char *message = "GERMAN Int";
int language=LANG_german;
#endif // GERMAN

#ifdef FRENCH
char OFFSETFILE[] = "p_fr_vdf_tuneint.h";
...
void DoAutotune()
{
	...
	DefaultParams = TextToSpeechGetPhVdefParams(ttsHandlePtr, nextVoice,language);
	...
	printf("File size: %lu\n", ulInputCount);
		...
		ttsHandlePtr->hTuneContinueEvent=OP_CreateEvent(TRUE,FALSE);
...
//		bSynthIsRunning = TRUE; // allow buffers..
		...
		OP_ResetEvent(ttsHandlePtr->hTuneContinueEvent);
		...
		while (!bDone)
		{
			...
			if(vtdData.bDidOverload)
			{
				...
				OP_SetEvent(ttsHandlePtr->hTuneContinueEvent);
	...
	TextToSpeechCloseInMemory(ttsHandlePtr);
		...
		if (g_bDidOverload)
		{
			...
			if (checkmax(vtdData.purevmax,MAX_FRIC_TRIGGER) && gTunerUnion.tp.gv>0 )
			{
				gTunerUnion.tp.gv--;
				gTunerUnion.tp.gf--;
				gTunerUnion.tp.gh--;
			} else
			if (checkmax(vtdData.r2pd1,MAX_FRIC_TRIGGER) && gTunerUnion.tp.gv>0 )
			{
				gTunerUnion.tp.gv--;
				gTunerUnion.tp.gf--;
				gTunerUnion.tp.gh--;
			} else
			if (checkmax(vtdData.r3pd1,MAX_FRIC_TRIGGER) && gTunerUnion.tp.gv>0 )
			{
				gTunerUnion.tp.gv--;
				gTunerUnion.tp.gf--;
				gTunerUnion.tp.gh--;
			} else
			if (checkmax(vtdData.r4pd1,MAX_FRIC_TRIGGER) && gTunerUnion.tp.gv>0 )
			{
				gTunerUnion.tp.gv--;
				gTunerUnion.tp.gf--;
				gTunerUnion.tp.gh--;
			} else
			if (checkmax(vtdData.r5pd1,MAX_FRIC_TRIGGER) && gTunerUnion.tp.gv>0 )
			{
				gTunerUnion.tp.gv--;
				gTunerUnion.tp.gf--;
				gTunerUnion.tp.gh--;
			} else
			if (checkmax(vtdData.r6pd1,MAX_FRIC_TRIGGER) && gTunerUnion.tp.gv>0 )
			{
				gTunerUnion.tp.gv--;
				gTunerUnion.tp.gf--;
				gTunerUnion.tp.gh--;
...
//			}
	...
	OP_DestroyEvent(ttsHandlePtr->hTuneContinueEvent);
...
if(TUNEERROR)
{
	...
	for(i=0;i<19;i++)
	{
		...
		if(!create_empty_file && i != 20) // Val has no defaults
		{
			if (i==18)
			{
				DefaultParams = TextToSpeechGetPhVdefParams(ttsHandlePtr, 9,language); // set to paul
			}
			else
			{
				DefaultParams = TextToSpeechGetPhVdefParams(ttsHandlePtr, i,language);
			}
		}
		fprintf(fpOffsetFile, "\nconst short\t\t\t%s%s[] =\n{\n",lang_tag,speaker);
		...
		{
			if(!create_empty_file &&  (i!=20) && (j>=GF && j<=LO)	)// get differences for just these values
			{
				if (i==18)
				{
					fprintf(fpOffsetFile, "\t%d,\n", TunedTable[9][j] - DefaultParams[j]); // set to paul
				}
				else
				{
...
void DoFullAutoTune()
{
	...
	char *prefixes[] = {"[:np]","[:nb]","[:nh]","[:nf]","[:nk]","[:nu]","[:nr]","[:nw]",
		"[:nd]","[:np]","[:nb]","[:nh]","[:nf]","[:nk]","[:nu]","[:nr]","[:nw]","[:nd]"};
	
	fprintf(fpResultFile, "***  Doing FULL TUNE  ***\n\n");
	
	for(ii=0;ii<19;ii++)		// Initialize tables for full tune
	{
