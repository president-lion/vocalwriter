...
echo .
echo **********************************************
echo Usage: dt_tune [language] [version] [length] [concurrent] [emptyfiles]
echo .
echo Language must be on of following:
echo          us,  uk,  sp,  la,  gr, fr, it, all
echo .
...
echo . 
echo Emptyfiles
echo            -e to create empty tune files
echo **********************************************
goto end
...
echo tunecheck -t %length%tune_%lang%.txt -l %lang%_ %emptyfiles%

if %CONCUR%==1 (
	start "Tunecheck - Tuning!  %lang%,  Version: %version%,   %length%tune_%lang%.txt" tunecheck -t %length%tune_%lang%.txt -l %lang%_ %emptyfiles%
	goto do_next
)

:single
tunecheck -t %length%tune_%lang%.txt -l %lang%_ %emptyfiles%
