...
extern const unsigned char *arpabet_arrays[];
...
BOOL UserDic( HWND hWindow, UINT uiDictEntries, UINT uiTextLength, TCHAR *fnam_t, TCHAR *fnam_d )
{
	...
	arpabet = (unsigned char *)arpabet_arrays[main_phon_lang];
	...
	for(i=0; i<language_size; i+=2)
	{
		if(p1 == language_prefixes[i] && p2 == language_prefixes[i+1])
		{
			if(line[curr_char] == '_')
			{
				...
				arpabet = (unsigned char *)arpabet_arrays[phon_lang];
				return TRUE;
			}
		}
	}
