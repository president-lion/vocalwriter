...
#endif
{
	...
	*a = handle;
	...
	if (def_lang->funcs.mod == NULL)	return MMSYSERR_ERROR;
#ifdef WIN32
	ret= def_lang->funcs.TextToSpeechStartup(a,&(handle->phTTS),c,d);
#else
	ret= def_lang->funcs.TextToSpeechStartupEx(&(handle->phTTS),b,c,d,e);
#endif
return(ret);
}
...
{	LPLANG_HANDLE handle;
	...
	if (handle->cur_lang == NULL)	return MMSYSERR_ERROR;
	if (handle->cur_lang->funcs.mod == NULL)	return MMSYSERR_ERROR;
//	(unsigned int (*)())handle->cur_lang->funcs.TextToSpeechSpeak = (MMRESULT ((_cdecl*)())) GetProcAddress(handle->cur_lang->funcs.mod,"TextToSpeechSpeak");
	ret=handle->cur_lang->funcs.TextToSpeechSpeak(handle->phTTS,b,c);
	return(ret);
}
...
short  *TextToSpeechGetPhVdefParams(LPTTS_HANDLE_T a, UINT index, int language)
{	LPLANG_HANDLE handle;
	...
	return handle->cur_lang->funcs.TextToSpeechGetPhVdefParams(handle->phTTS, index,language);
}
...
#if !defined __osf__ && !defined __linux__ && !defined _SPARC_SOLARIS_ && !defined __QNX__
