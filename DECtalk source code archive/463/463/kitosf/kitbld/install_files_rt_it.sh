...
echo "IT_dict:"$DICTIONARIES"dtalk_fr.dic" >> /etc/DECtalk.conf
echo "IT_udict:udict_fr.dic" >> /etc/DECtalk.conf
echo "LANG:it,Italian" >> /etc/DECtalk.conf

grep $LIBS /etc/ld.so.conf >/dev/null || {
  echo $LIBS >>/etc/ld.so.conf; /sbin/ldconfig >/dev/null 2>/dev/null;  }
