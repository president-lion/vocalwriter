...
if "%dtlang%"=="ITALIAN" goto del_demo
if %democopy_debug%==1 pause
rem bug
...
copy ..\dapi\%bld_path%\dtdemo\it\release\dtdemo.exe ..\%arch_dir%\%demo_dir%\dtdem_it.exe
if NOT exist ..\%arch_dir%\%demo_dir%\dtdem_it.exe echo File .\%arch_dir%\%demo_dir%\dtdem_it.exe not build >> %blddrv%%bldpath%\build_log\errors\kit%index%.log
:skip_dtdemo
...
copy ..\dapi\%bld_path%\dtalkdic\it\release\dtalk_it.dic ..\%arch_dir%\%demo_dir%\dtalk_it.dic
if NOT exist ..\%arch_dir%\%demo_dir%\dtalk_it.dic echo File .\%arch_dir%\%demo_dir%\dtalk_it.dic not build >> %blddrv%%bldpath%\build_log\errors\kit%index%.log
:skip_dic
:skip_copy
if %democopy_debug%==1 pause
